...
this.Control[iCurrent+22]=this.dw_8
end on
...
gb_2.width  = newwidth  - gb_2.x - 32

dw_2.width  = newwidth  - 1083

gb_4.width  = newwidth  - gb_2.x - 32

dw_4.width  = newwidth  - dw_4.x - 32
dw_4.height = newheight - dw_4.y - 32

dw_6.width  = dw_4.width  - dw_3.width
...
string ls_coilno, ls_place, ls_wcno, ls_suppno
long   ll_row, ll_find
dec{1} ld_wid

dw_2.accepttext()		// 코일작업지시
dw3.accepttext()		// 원코일

if dw_2.rowcount() < 1 then RETURN
if dw3.rowcount()  < 1 or cbx_1.checked = false then RETURN

ls_place  = dw_2.object.wc_place[1]		// 유형
...
if ls_place = "I" then
	...
	if isnull( ls_suppno ) or ls_suppno = "" then
...
// 저장소 코일이동여부 체크
////////////////////////////////////////////
if rb_4.checked = true then	// 원코일내역
	if ls_place = "I" then
		ls_wcno = dw_2.object.wc_no[1]		// 사내저장소
	else
		ls_wcno = dw_2.object.supp_no[1]		// 외주저장소
	end if
	
	// coil, sheet
	for ll_row = 1 to dw3.rowcount()
		ls_coilno = dw3.object.coil_no[ll_row]
      SELECT cnt = sum(a.cnt) INTO :ll_find 
		  FROM ( // open_close O:재고, M:계획, P:작업지시, S:외주가공, C:사용완료
				 SELECT cnt = count(*) FROM coilmst 
				  WHERE loc_no = :ls_wcno and open_close = 'M' and coil_no  = :ls_coilno	// 계획
				 UNION ALL
				 SELECT cnt = count(*) FROM sheetmst 
				  WHERE loc_no = :ls_wcno and open_close = 'M' and sheet_no = :ls_coilno	// 계획
				 ) A;
		if ll_find > 0 then
		else
			if ls_place = "I" then // 사내저장소
			else
				MessageBox("확인", "해당하는 작업장에 코일이 '계획' 상태로 존재하지 않습니다.")
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
if MessageBox("확인", "선택한 내역을 저장 하시겠습니까?", Exclamation!, OKCancel!, 2) = 2 then RETURN
/////////////////////////////////////////////////////////////////////////////////////////////////////////
