...
type rb_1 from radiobutton within w_osmsale_r
end type
type rb_2 from radiobutton within w_osmsale_r
end type
type dw_imwon_sales from datawindow within w_osmsale_r
end type
type rb_3 from radiobutton within w_osmsale_r
end type
...
datawindowchild idwc_cust, idwc_user, idwc_rotate, idwc_coll, idwc_imwon_user
...
this.Control[iCurrent+35]=this.dw_imwon_sales
this.Control[iCurrent+36]=this.rb_3
...
event pb_retrieve::ue_1;String  ls_month, ls_top_level, ls_cust_no
Decimal lr_ym, lr_ys, lr_mm, lr_ms, lr_bm, lr_yb, lr_yy
Decimal lr_aim_inv, lr_aim_del, lr_aim_mod, lr_aim_coll, lr_pre_jan
...
for ll_row = 1 to ll_rows 	
	...
	lr_pre_jan = (lr_ym + lr_yy  - lr_mm) - (lr_ys - lr_ms) // 전월잔액 -> 이월금액 명칭 변경
	
	if isnull(lr_pre_jan) then
		lr_pre_jan = 0
	end if
	...
	dw_1.setitem(ll_row1, "m_jan",        lr_mm - lr_ms + lr_pre_jan)        // 월말잔액
	dw_1.setitem(ll_row1, "m_m",          lr_mm)                        // 금월매출
	dw_1.setitem(ll_row1, "m_s",          lr_ms)                        // 금월수금
	
     /*월말잔액 -> 이월금액 2020.04.21 jowonsuk*/
	ls_cust_no = dw_2.object.data[ll_row, 1]
	
	select  sum(do_total) sale				//do_cash
		into :lr_bm
	from do
	where  datepart(year, do_date) = :ls_yyyy
		and   datepart(month, do_date) = :l_mm
		and  cust_no = :ls_cust_no
		and ewol = 'Y';	
	
	
	dw_1.setitem(ll_row1, "bm_jan",       lr_bm)                        // 전월잔액 -> 이월금액 명칭 변경  2020.04.21 jowonsuk
	...
	dw_1.setitem(ll_row1, "rotate",    dw_2.object.data[ll_row, 91]) // 회전월		
	
	if rb_1.checked = false then
		dw_1.setitem(ll_row1, "gubun",    dw_2.object.data[ll_row, 92]) // 정부
	end if
	
	/*2018.12.26 jowonsuk  반영*/
	...
	dw_3.setitem(ll_row1, "pre_jan",          lr_pre_jan)	
	dw_3.setitem(ll_row1, "m_m",          lr_mm)
	dw_3.setitem(ll_row1, "y_y",          lr_bm)                          //이월금액  2020.04.21 jowonsuk
	...
	dw_3.setitem(ll_row1, "rotate",    dw_2.object.data[ll_row, 91]) // 회전월
	
	if rb_1.checked = false then			
		dw_3.setitem(ll_row1, "gubun",    dw_2.object.data[ll_row, 92]) // 정부
	end if		
...
// 잔액제로 제외
if dw_1.dataobject = "d_osmsaletotal_t" or dw_1.dataobject = "d_osmsaletotal_saleimwon_t" then
...
event pb_retrieve::ue_all;string ls_top_level
decimal lr_ym, lr_ys, lr_mm, lr_ms, lr_bm, lr_yb, lr_yy
decimal lr_aim_inv, lr_aim_del, lr_aim_mod, lr_aim_coll, lr_pre_jan
long    ll_row,  ll_row1, ll_column, l_toplevel

string ls_yyyy, ls_cust_no
int      l_mm
...
for ll_row = 1 to dw_2.rowcount()
	...
	lr_pre_jan = (lr_ym + lr_yy  - lr_mm) - (lr_ys - lr_ms) // 전월잔액
   ...
   /*월말잔액 -> 이월금액 2020.04.21 jowonsuk*/
	...
	dw_1.setitem(ll_row1, "bm_jan",       lr_bm)                        // 전월잔액-> 이월금액
	dw_1.setitem(ll_row1, "y_m",          lr_ym)                        // 연간매출
	dw_1.setitem(ll_row1, "y_s",          lr_ys)                        // 연간수금
	...
	dw_3.setitem(ll_row1, "cust_name",    dw_2.object.data[ll_row, 31]) // 거래처
	dw_3.setitem(ll_row1, "m_m",          lr_mm)
	dw_3.setitem(ll_row1, "pre_jan",       lr_pre_jan)        // 월말잔액	
	dw_3.setitem(ll_row1, "y_y",          lr_bm)							// 전월잔액	 ->이월금액  2020.04.21 jowonsuk
	dw_3.setitem(ll_row1, "m_s",          lr_ms)
	dw_3.setitem(ll_row1, "y_m",          lr_ym)
	dw_3.setitem(ll_row1, "y_s",          lr_ys)
	dw_3.setitem(ll_row1, "m_aim_inv",    lr_aim_inv)                   // 월매출목표
...
/*2018.10.02 jowonsuk 임원담당자 추가  */
if rb_1.checked then	
	ls_userid = dw_sales.object.user[1]

	dw_2.dataobject = "d_osmsaletotal_h"	
	dw_5.dataobject = "d_cdcustomer_grp"	
...
elseif rb_2.checked then
	...
	dw_2.dataobject = "d_osmsaletotal_imwon_h"		
	dw_5.dataobject = "d_cdcustomer_imwon_grp"	
	dw_2.SetTransObject(SQLCA)			
	dw_5.SetTransObject(SQLCA)			
...
elseif rb_3.checked then
	...
	dw_2.dataobject = "d_osmsaletotal_saleimwon_h"		 // d_osmsaletotal_saleimown_h svn에서 삭제 가능시 삭제 필요
	dw_5.dataobject = "d_cdcustomer_saleimwon_grp"	
	dw_2.SetTransObject(SQLCA)			
	dw_5.SetTransObject(SQLCA)		
	
	if ls_userid = '' or isnull(ls_userid) or ls_userid = '%' then
		  MessageBox("확인","담당자 지정은 필수사항입니다.")
		RETURN	
	end if 	
...
// 수금등급 필터
dw_1.setfilter( "colllevel >= '" + ls_colllevel + "' and ( if( isnull( mid(top_level,1,1) ), '%', mid(top_level,1,1) ) like '"+ ls_top_level1 + "') and ( if( isnull( mid(top_level,2,2) ), '%', mid(top_level,2,2) ) like '" + ls_top_level2 + "')" );
dw_1.filter()
dw_3.setfilter( "colllevel >= '" + ls_colllevel + "' and ( if( isnull( mid(top_level,1,1) ), '%', mid(top_level,1,1) ) like '"+ ls_top_level1 + "') and ( if( isnull( mid(top_level,2,2) ), '%', mid(top_level,2,2) ) like '" + ls_top_level2 + "')" );
dw_3.filter()

/* 2018.10.4 jowonsuk 소트 */
if rb_1.checked then
...
elseif rb_3.checked then
	dw_1.SetSort("gubun A, user_name A, toplevel A, cust_sort A, custname_sort A")	
	dw_1.Sort()	

	dw_3.SetSort("gubun A, user_name A, toplevel A, cust_sort A, custname_sort A")	
	dw_3.Sort()	
...
boolean vscrollbar = true
integer limit = 7
string item[] = {"% 전체","大 등급","中 등급","小 등급","신 등급","X  등급"}
borderstyle borderstyle = stylelowered!
end type
...
event clicked;dw_sales.visible = true
dw_sales.X = 2939
dw_sales.Y = 140
dw_imwon_sales.visible = false
dw_imwon_sales.X = 2939
dw_imwon_sales.Y = 140


dw_1.dataobject = "d_osmsaletotal_t"		 // d_osmsaletotal_saleimown_h svn에서 삭제 가능시 삭제 필요
dw_3.dataobject = "d_osmsaletotal_r"	
...
event clicked;//dw_sales.visible = false
dw_sales.visible = true
dw_sales.X = 2939
dw_sales.Y = 140
//dw_imwon_sales.visible = true
dw_imwon_sales.X = 2939
dw_imwon_sales.Y = 140

dw_1.dataobject = "d_osmsaletotal_saleimwon_t"		 // d_osmsaletotal_saleimown_h svn에서 삭제 가능시 삭제 필요
dw_3.dataobject = "d_osmsaletotal_saleimwon_r"	
dw_1.SetTransObject(SQLCA)			
dw_3.SetTransObject(SQLCA)		
...
dw_1.dataobject = "d_osmsaletotal_saleimwon_t"
dw_3.dataobject = "d_osmsaletotal_saleimwon_r"	
dw_1.SetTransObject(SQLCA)			
dw_3.SetTransObject(SQLCA)		
