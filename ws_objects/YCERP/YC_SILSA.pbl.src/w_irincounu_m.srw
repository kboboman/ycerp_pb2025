...
type st_3 from statictext within w_irincounu_m
end type
type ddlb_fld from dropdownlistbox within w_irincounu_m
end type
type ddlb_dwtitles from dropdownlistbox within w_irincounu_m
end type
type st_2 from statictext within w_irincounu_m
end type
type ddlb_op from dropdownlistbox within w_irincounu_m
end type
type sle_value from singlelineedit within w_irincounu_m
end type
type cb_2 from commandbutton within w_irincounu_m
end type
type cb_3 from commandbutton within w_irincounu_m
end type
type cb_4 from commandbutton within w_irincounu_m
end type
type cb_5 from commandbutton within w_irincounu_m
end type
type cb_1 from commandbutton within w_irincounu_m
end type
...
event clicked;//
long   ll_sqlcode, ll_cnt
string ls_userid, ls_locno, ls_toid, ls_usernm[2], ls_sqlerrtext

dw_3.accepttext()
dw_5.accepttext()

ls_userid = dw_3.object.user_id[1]
ls_locno  = dw_5.object.loc_no[1]

if isnull(ls_userid) OR isnull(ls_locno) then RETURN

SELECT count(*) INTO :ll_cnt FROM incounu WHERE user_id = :ls_userid AND loc_no = :ls_locno;
if isnull(ll_cnt) OR ll_cnt < 1 then RETURN

if MessageBox("확인", "현재 저장소의 품목들을 [" + gs_username + "] 에게 품목복사 합니까?", Exclamation!, OKCancel!, 2) = 1 then
	if MessageBox("확인", "기존 저장소의 [" + gs_username + "] 품목들을 모두 삭제합니까?", Exclamation!, OKCancel!, 2) = 1 then
		DELETE incounu WHERE user_id = :gs_userid AND loc_no = :ls_locno;
	end if
	
	INSERT INTO incounu
	SELECT :gs_userid, item_no, qa, loc_no, save_qty 
	  FROM incounu
	 WHERE user_id = :ls_userid AND loc_no = :ls_locno;
	if sqlca.sqlcode <> 0 then
		ls_sqlerrtext = sqlca.sqlerrtext
		MessageBox("오류","품목복사중 오류 발생했습니다.~r~n" + ls_sqlerrtext, Exclamation!)
		RETURN
	else
		COMMIT;
	end if
	
	MessageBox("확인","품목복사 완료.", Exclamation!)
