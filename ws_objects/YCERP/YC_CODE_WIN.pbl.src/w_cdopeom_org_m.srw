...
type rb_1 from radiobutton within w_cdopeom_org_m
end type
type rb_2 from radiobutton within w_cdopeom_org_m
end type
type cb_6 from commandbutton within w_cdopeom_org_m
end type
end forward
...
if rb_1.checked then	
	dw_1.dataobject = "d_cdopeom_org_m"	
	dw_3.dataobject = "d_cdmycustomer_s"	
	...
	dw_1.dataobject = "d_cdopeom_org_imwon_m"		
	dw_3.dataobject = "d_cdmycustomer_imwon_s"	
	dw_1.SetTransObject(SQLCA)			
	dw_3.SetTransObject(SQLCA)			
...
fontfamily fontfamily = modern!
string facename = "굴림체"
string text = "(부)영업조직생성"
end type

event clicked;
integer Net

Net = MessageBox("경고", "기존 데이터를 초기화하고, 새로 생성하시겠습니까?", Exclamation!, OKCancel!, 2)

IF Net = 1 THEN 

	delete opeom_imwon_org;
	
	insert into opeom_imwon_org
	select ROW_NUMBER() OVER(ORDER BY a.user_id) seq, null area, a.user_id area_mgr, null sector, null sector_mgr, null team, null team_mgr, null tbl_info, null user_id, null user_name, null rank, null bigo
	from
	(
	  SELECT  area_mgr user_id
	  FROM opeom_org
		 union
	  SELECT sector_mgr
	  FROM opeom_org
	  ...
	  SELECT user_id
	  FROM opeom_org 
	) a inner join login b on a.user_id = b.user_Id and b.use_flag = 'Y'
	order by a.user_id;
