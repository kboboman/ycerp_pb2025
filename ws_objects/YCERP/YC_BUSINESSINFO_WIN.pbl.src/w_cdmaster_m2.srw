$PBExportHeader$w_cdmaster_m2.srw
$PBExportComments$시행자 관리 (2000/11/07,서재복)
forward
global type w_cdmaster_m2 from w_inheritance
end type
type cb_1 from commandbutton within w_cdmaster_m2
end type
type cb_2 from commandbutton within w_cdmaster_m2
end type
type sle_1 from singlelineedit within w_cdmaster_m2
end type
type st_1 from statictext within w_cdmaster_m2
end type
type cb_3 from commandbutton within w_cdmaster_m2
end type
end forward
...
integer width = 3643
integer height = 2008
string title = "발주처등록(w_cdmaster_m)"
...
global w_cdmaster_m2 w_cdmaster_m2
...
type pb_save from w_inheritance`pb_save within w_cdmaster_m2
integer x = 3191
integer y = 68
end type
...
type dw_1 from w_inheritance`dw_1 within w_cdmaster_m2
integer x = 41
integer y = 308
integer width = 3543
integer height = 1556
...
type st_title from w_inheritance`st_title within w_cdmaster_m2
string tag = "시행자 관리"
integer y = 36
integer width = 1138
integer textsize = -20
...
type st_tips from w_inheritance`st_tips within w_cdmaster_m2
end type

type pb_compute from w_inheritance`pb_compute within w_cdmaster_m2
boolean visible = false
integer taborder = 90
end type

type pb_print_part from w_inheritance`pb_print_part within w_cdmaster_m2
boolean visible = false
end type

type pb_close from w_inheritance`pb_close within w_cdmaster_m2
integer x = 3383
integer y = 68
...
type pb_print from w_inheritance`pb_print within w_cdmaster_m2
integer x = 2999
integer y = 68
...
type pb_cancel from w_inheritance`pb_cancel within w_cdmaster_m2
boolean visible = false
integer taborder = 120
end type

type pb_delete from w_inheritance`pb_delete within w_cdmaster_m2
integer x = 2807
integer y = 68
...
type pb_insert from w_inheritance`pb_insert within w_cdmaster_m2
integer x = 2615
integer y = 68
...
FOR ll_count = 1 TO dw_1.rowcount()
	...
	if l_status = newmodified! then
		IF isnull(dw_1.object.comcode[ll_count]) OR dw_1.object.comcode[ll_count] <= "" THEN
			messagebox("입력오류", string(ll_count) + " 번째 시행자코드을 입력하십시오")
			dw_1.setcolumn("comcode")
			dw_1.setfocus()
			return
		END IF
		IF isnull(dw_1.object.comname[ll_count]) OR dw_1.object.comname[ll_count] <= "" THEN
			messagebox("입력오류", string(ll_count) + " 번째 시행자명을 입력하십시오")
...
//end if
//dw_1.object.comcode[ll_row] = 'M' + right( "000" + string( li_int + 1), 3)
//dw_1.setcolumn('comname')
//dw_1.setfocus()
//
string ls_comcode
select isnull(max(comcode),'M001') into :ls_comcode
from bdcom
where comcode like 'M%';
...
//end if
dw_1.object.comcode[ll_row] = 'M' + RightA( "000" + string( li_int + 1), 3)
dw_1.setcolumn('comname')
dw_1.setfocus()
...
type pb_retrieve from w_inheritance`pb_retrieve within w_cdmaster_m2
integer x = 2423
integer y = 68
...
type gb_3 from w_inheritance`gb_3 within w_cdmaster_m2
boolean visible = false
integer x = 32
integer y = 272
integer width = 2386
...
type gb_2 from w_inheritance`gb_2 within w_cdmaster_m2
integer x = 27
integer y = 260
integer width = 3575
integer height = 1628
...
type gb_1 from w_inheritance`gb_1 within w_cdmaster_m2
integer x = 2391
integer y = 28
integer width = 1207
long textcolor = 0
...
type dw_2 from w_inheritance`dw_2 within w_cdmaster_m2
boolean visible = false
integer x = 1211
integer y = 56
integer width = 187
