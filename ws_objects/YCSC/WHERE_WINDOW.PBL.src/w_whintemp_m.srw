...
if l_status = New! or l_status = NewModified! then
	...
	SELECT isnull(IDENT_CURRENT ('intemp'), 0) + 1 INTO :ll_seq 
	  FROM login
	 WHERE user_id = :gs_userid;
	dw_1.object.intemp_no[1] = LeftA(gs_area, 1) + 'T' + string(dw_1.object.intemp_date[1], 'yyyymmdd') + "-" + RightA( "000" + trim(string(ll_seq)),3)
	if dw_1.object.qty[1] = 0 or isnull(dw_1.object.qty[1]) then
		MessageBox("확인","가입고수량을 입력하십시요")
		dw_1.setcolumn('qty')
		dw_1.setfocus()
		return
	end if
...
if wf_update1(dw_1,"N") = false then return
...
string text = "※ 가입고 사유에 맞는 발생원인을 선택하여 주시가 바랍니다."
