...
choose case ll_error
	...
	case else
		...
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, &
					ll_error, LANG_NEUTRAL, ls_errmsg, LenA(ls_errmsg), 0)
		If RightA(ls_errmsg, 2) = "~r~n" Then
			ls_errmsg = LeftA(ls_errmsg, LenA(ls_errmsg) - 2)
		End If
end choose
...
ll_pos = PosA(ls_ipaddress, ":")
ls_ipaddress = LeftA(ls_ipaddress, ll_pos - 1)
...
If lb_rtn Then
	ls_encoded = LeftA(ls_encoded, lul_buflen - 2)
Else
	SetNull(ls_encoded)
End If
...
ll_SeparatorLen = LenA(as_Separator)

ll_PosEnd = PosA(as_String, as_Separator, 1)

Do While ll_PosEnd > 0
	as_OutArray[ll_Counter] = MidA(as_String, ll_PosStart, ll_PosEnd - ll_PosStart)
	ll_PosStart = ll_PosEnd + ll_SeparatorLen
	ll_PosEnd = PosA(as_String, as_Separator, ll_PosStart)
	ll_Counter++
Loop

as_OutArray[ll_Counter] = RightA(as_String, LenA(as_String) - ll_PosStart + 1)
