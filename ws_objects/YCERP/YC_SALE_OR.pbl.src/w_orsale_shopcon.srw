...
string ls_custno, ls_salecontrol, ls_orderno, ls_orguser, ls_chkyn
int    li_seq 
   ...
   ////////////////////////////////////////////////////////////////////////////////////////////	
	...
	if sqlca.sqlcode = 0 then
		...
		if sqlca.sqlcode = 0 then
			...
			ROLLBACK;
			MessageBox("확인", "쇼핑몰접수(상세) 오류가 발생했습니다.~r~n요청번호: " + ls_orderno)
			RETURN
		end if
	else
		ROLLBACK;
		MessageBox("확인", "쇼핑몰접수(헤더) 오류가 발생했습니다.~r~n요청번호: " + ls_orderno)
		RETURN
	end if
