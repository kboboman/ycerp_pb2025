...
type st_saleamt from statictext within w_orsaleprice_m
end type
...
this.Control[iCurrent+40]=this.pb_8
this.Control[iCurrent+41]=this.pb_9
...
/* 2012-09-03(이재형) : 주문단가 업데이트 관련 불필요사항 주석처리함
	...
	elseif dw_2.object.sale_confirm_flag[1] <> "N" then
		...
		if ls_credit_level = 'Z' or ls_credit_level = 'F' then
			...
			if MessageBox("확인","이 수주내용은 현금거래, 여신한도액 0, 여신한도내, 채권팀 거래처가 아닌~n~n" + & 
				...
				ls_s_msg = "[송천]수주초과:" + string(lr_total,"#,##0") + " " + ls_s_cust_name + "," + trim(ls_s_scene_desc)
				ls_s_msg = GF_HANGLE_TR(ls_s_msg,80)
								
				choose case left(ls_s_hp_no,3)
	...
	SELECT top 1 datediff( day, :ls_date, getdate() ), convert(char(10),dateadd( day, 60, :ls_date ),111)
	  INTO :li_today1, :ls_date60  FROM login;
...
/* 2012-09-03(이재형) : 주문단가 업데이트 관련 불필요사항 주석처리함
	...
	if l_status = newmodified! or l_status = new! then
		...
		ls_area      = left(dw_6.object.area[1],1)
		...
		if isnull(ls_sale_no) or ls_sale_no = '' then
			...
			li_seq = integer(mid(ls_sale_no, 12, 3))
			li_seq = li_seq + 1
			ls_seq = '00' + string(li_seq)
			ls_sale_no1 = left(ls_sale_no1, 11) + right(ls_seq, 3)
		end if
...
/*2022-7-4 jowonsuk 현장에서 사는 사람들은 거래처단가 적용. 시작*/		
select count(*)
  into :ll_count
from customer
where cust_no = :ls_cust
 and register_no = '999-99-99999';
/*2022-7-4 jowonsuk 현장에서 사는 사람들은 거래처단가 적용. 끝*/		
dw_3.SetRedraw(false)
for ll_row = 1 to UpperBound(lstr_where.str1)                  
   ...
   lr_price    = GF_CUSTPRICE(ls_scene_no, ls_cust_no, ls_com_qa, ls_item_no, ls_qa)   // d_wfprice (현장->거래처->표준)
