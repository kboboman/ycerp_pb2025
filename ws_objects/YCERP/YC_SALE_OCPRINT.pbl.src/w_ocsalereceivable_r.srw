...
type st_6 from statictext within w_ocsalereceivable_r
end type
type ddlb_fld from dropdownlistbox within w_ocsalereceivable_r
end type
type ddlb_dwtitles from dropdownlistbox within w_ocsalereceivable_r
end type
type st_7 from statictext within w_ocsalereceivable_r
end type
type sle_value from singlelineedit within w_ocsalereceivable_r
end type
type cb_1 from commandbutton within w_ocsalereceivable_r
end type
type cb_2 from commandbutton within w_ocsalereceivable_r
end type
type cb_3 from commandbutton within w_ocsalereceivable_r
end type
type cb_5 from commandbutton within w_ocsalereceivable_r
end type
type ddlb_op from dropdownlistbox within w_ocsalereceivable_r
end type
type pb_2 from picturebutton within w_ocsalereceivable_r
end type
end forward
...
this.Control[iCurrent+13]=this.dw_11
this.Control[iCurrent+14]=this.em_limit
this.Control[iCurrent+15]=this.st_time
...
string  ls_level_userid, ls_level, ls_3mon
...
ll_row = dw_1.retrieve( li_year, li_month, is_level, ls_level_userid, ld_rate, is_3mon )
dw_1.setredraw(true)

SELECT top 1 replace(convert(char(3),DATEDIFF(ss, :ld_time, getdate()) / 60) + '분',' ','') + ' ' + 
             replace(convert(char(3),DATEDIFF(ss, :ld_time, getdate()) % 60) + '초',' ','') 
  INTO :ls_time 
  FROM login;
...
f_usetips(st_tips,"회전월반영",parent, li_x, li_y)
...
UPDATE customer
 SET rotate = b.invrate
 FROM customer A JOIN (
											 SELECT  cust_no      = a.cust_no                                                                     
													, invrate = convert(numeric(4,1), round(a.invrate,1) )        /* 회전율 */                         
											  FROM (
																		SELECT cust_name    = c.cust_name                                                     
																			  , cust_no      = c.cust_no                                                                                                
																			  , inv_rem      = isnull(dbo.uf_getopeom_Srem( c.cust_no, datepart(year, getdate()), datepart(month, getdate() ) )  ,0)           /* 월말 미수잔액 */
																			  , invrate      = isnull(dbo.uf_getopeom_rate( c.cust_no, datepart(year, getdate()), datepart(month, getdate() ) )  ,0)           /* 회전율 */       
																			  , pinv_avr     = isnull(dbo.uf_getopeom_Sinv( c.cust_no, (datepart(year, getdate()) -1), 12)  ,0) / 12      /* 평균매출전년 */ 
																			  , cinv_avr     = isnull(dbo.uf_getopeom_Sinv( c.cust_no, datepart(year, getdate()), datepart(month, getdate() ))  ,0) / datepart(month, getdate() ) /* 평균매출당년 */                                                                                
																			  , pinv_rem     = isnull(copeom.balbf_amount, 0)                                            /* 전년미수잔액 */ 
																		  FROM customer  c  with(nolock)                                                
																				 LEFT OUTER JOIN (select cust_no, eom_year, balbf_amount from opeom with(nolock) where datepart(year, eom_year) = datepart(year, getdate()) )copeom  ON c.cust_no  = copeom.cust_no 
																		 WHERE c.open_close             = 'O'                               
																			AND c.cust_flag              = 'N'                         
													 ) A 
													 where invrate >= 0                  
														 and abs(inv_rem) + abs(pinv_avr) + abs(cinv_avr) + abs(pinv_rem) <> 0
								 ) B
 ON A.cust_no = B.cust_no;

MessageBox("확인","회전월작업완료")
