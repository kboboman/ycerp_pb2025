...
type st_8 from statictext within w_cddocar_m
end type
type ddlb_fld from dropdownlistbox within w_cddocar_m
end type
type st_9 from statictext within w_cddocar_m
end type
type sle_value from singlelineedit within w_cddocar_m
end type
type cb_filteron from commandbutton within w_cddocar_m
end type
type cb_filteroff from commandbutton within w_cddocar_m
end type
type cb_1 from commandbutton within w_cddocar_m
end type
type cb_2 from commandbutton within w_cddocar_m
end type
type ddlb_dwtitles from dropdownlistbox within w_cddocar_m
end type
type ddlb_op from dropdownlistbox within w_cddocar_m
end type
type rb_1 from radiobutton within w_cddocar_m
end type
type rb_2 from radiobutton within w_cddocar_m
end type
end forward
...
dw_1.height = newheight - gb_1.height - gb_2.height - 60
...
for ll_count = 1 to dw_1.rowcount()
	ls_driver    = dw_1.object.driver[ll_count]
	ls_inout     = dw_1.object.inout[ll_count]
	ldt_sysdate  = dw_1.object.sys_date[ll_count]
	
	if isnull(ls_inout) OR ls_inout = "" then
		MessageBox("확인","출고형태를 입력하시기 바랍니다.")
		RETURN
	end if
	
	if isnull(ls_driver) OR ls_driver = "" then
		MessageBox("확인", string(ll_count) + " 번째 차량NO을 입력하십시오")
	...
	if rb_2.checked then
		...
		ls_car_no = trim(dw_1.object.driver[ll_count])
...
string text = "차량번호란에 전체 차량번호를 넣어주세요(예: 충남32나1234)"
...
event clicked;dw_1.dataobject = 'd_cddocar_m1'
dw_1.settransobject( sqlca)
end event
