$PBExportHeader$w_line_p01.srw
$PBExportComments$생산라인 라벨출력(도장)
forward
global type w_line_p01 from window
end type
type cbx_3 from checkbox within w_line_p01
end type
type cbx_2 from checkbox within w_line_p01
end type
type em_copy from editmask within w_line_p01
end type
type st_7 from statictext within w_line_p01
end type
type em_prt from editmask within w_line_p01
end type
type st_4 from statictext within w_line_p01
end type
type cbx_1 from checkbox within w_line_p01
end type
type cbx_unit from checkbox within w_line_p01
end type
type cbx_next from checkbox within w_line_p01
end type
type em_top from editmask within w_line_p01
end type
type em_left from editmask within w_line_p01
end type
type st_6 from statictext within w_line_p01
end type
type st_5 from statictext within w_line_p01
end type
type st_key from statictext within w_line_p01
end type
type cb_3 from commandbutton within w_line_p01
end type
type cb_2 from commandbutton within w_line_p01
end type
type cb_1 from commandbutton within w_line_p01
end type
type st_2 from statictext within w_line_p01
end type
type dw_2 from datawindow within w_line_p01
end type
type st_1 from statictext within w_line_p01
end type
type dw_1 from datawindow within w_line_p01
end type
type st_3 from statictext within w_line_p01
end type
end forward
...
date   ldt_date
time   lt_Time
int    li_cnt, li_seq, li_division,  li_operation, li_work_per, li_mpsseq, li_operation_pre /* 도장 이전 jowonsuk 2018-05-22*/
...
li_operation_pre  = dw_1.object.operation[al_row]	/* 신규로직 jowonsuk */
...
VALUES (:ldt_sfc_dt,:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,:ldb_work_qty,0,:gs_loc,'RP',0,:ls_wc_no,'현장작업일보[도장]', :gs_userid);
if sqlca.sqlcode < 0 then
	ls_sqlerrtext = sqlca.sqlerrtext
	rollback;
...
/*2018-05-22 jowonsuk 자재 사용 자동 등록 시작 */
/*2020-07-04 jowonsuk 주석처리, 생산계획이전공정으로 처리하는게 맞지만 제품별공정관리를 이용해서 터는걸로 교체
SELECT top 1 ASSY_ITEM, ASSY_QA, UOM
      into :ls_assy_item, :ls_assy_qa, :ls_uom
FROM routtempnew 
 WHERE division  = :li_division 
	AND sale_no   = :ls_sale_no 
	AND seq_no    = :li_seq 
	AND operation = (:li_operation_pre - 1)
  order by division desc; *//* 현재 공정 이전 공정을 자재 사용량으로 사용 한다. */
	
	
///*2018-08-13 jowonsuk 분활시에 아래와 같이 조회 */	
//if sqlca.sqlcode = 100  then /*2018-12-26 jowonsuk sqlca.sqlcode = 100 로 변경  */	
//	
//	SELECT top 1 ASSY_ITEM, ASSY_QA, UOM
//		  into :ls_assy_item, :ls_assy_qa, :ls_uom
//	FROM routtempnew 
//	 WHERE division  =  :li_division + 1
//	     AND sale_no   = :ls_sale_no 
//		AND seq_no    = :li_seq 
//		AND operation = (:li_operation_pre - 1); /* 현재 공정 이전 공정을 자재 사용량으로 사용 한다. */	
//	
//end if
//
//
///*2018-12-26 jowonsuk 분활시에 아래와 같이 조회 */	
//if sqlca.sqlcode = 100  then
//	
//	SELECT top 1 ASSY_ITEM, ASSY_QA, UOM
//		  into :ls_assy_item, :ls_assy_qa, :ls_uom
//	FROM routtempnew 
//	 WHERE division  =  :li_division + 2
//	     AND sale_no   = :ls_sale_no 
//		AND seq_no    = :li_seq 
//		AND operation = (:li_operation_pre - 1); /* 현재 공정 이전 공정을 자재 사용량으로 사용 한다. */	
//	
...
/*	
		 ...
		 select count(*)
		   into :l_count
		 from sfcuse 
		where order_no = :ls_plan_no
			 and wc_no = :ls_wc_no
			 and use_date = CONVERT(DATETIME, CONVERT(CHAR(10), :ldt_sfc_dt,121 ) )
			 and item_no = :ls_assy_item
			 and qa = :ls_assy_qa;
...
/*2018.12.26 jowonsuk 로직 이전 operation이 없어도 털리는 구조 */
/*2019-12-24 jowonsuk 대표공정을 반영할 수 있도록 대표공정이면 규격을 체크하지 않고 완제품 규격을 넣는다. */
/*2020-07-04 jowonsuk 대표공정을 반영할 수 있도록 대표공정이면 규격을 체크하지 않고 완제품 규격을 넣는다. */
string ls_coil_type
long l_count, l_qa_check
...
/*							
								 ...
								 where assembly = :ls_assy_item
								   and represent = 'Y'
*/
								/*2020-08-06 JOWONSUK 자재사용 오류 수정*/
...
if sqlca.sqlcode = 100  then
				 ...
				 values                   (:ls_plan_no, :ls_wc_no, :ldt_sfc_dt /*CONVERT(DATETIME, CONVERT(CHAR(10), :ldt_sfc_dt,121 ) )*/, :ls_assy_item,  :ls_assy_qa, :ls_uom, :ldb_work_qty,   :gs_loc,  :ls_coil_type,     0,           0,              0,   :li_division,                0,   null,            0,            0,           null,       null,              0,   :ls_area_no,   :ls_itemno_color,             null );	 
...
/*		
				...
				update sfcuse
				   set  use_qty = isnull(use_qty,0) + :ldb_work_qty
				where order_no = :ls_plan_no
				    and wc_no = :ls_wc_no
				    and use_date = CONVERT(DATETIME, CONVERT(CHAR(10), :ldt_sfc_dt,121 ) )
				    and item_no = :ls_assy_item
...
*/	 		
		...
		VALUES (getdate(),:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,0,:ldb_work_qty,:gs_loc,'IW',0,:ls_wc_no,'현장자재사용[도장]');
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
date   ldt_date
string ls_wcno, ls_itemno, ls_uom, ls_qa, ls_custno, ls_scene, ls_rem, ls_qa1, ls_saleno, ls_pre_order_no, ls_color
...
//			l_issue_qty = dw_1.object.work_qty[li_int]
			...
			ls_color = dw_1.object.color_name[li_int]	
			
			// 비고색상 출력
			if cbx_3.checked = true then
				ls_color   = dw_1.object.rem[li_int]			// 비고
			else
				ls_color   = dw_1.object.color_name[li_int]	// 색상명
			end if			
			...
			/*2019.03.15 jowonsuk qrcode 라벨 기존 수주번호+5자리순번*/
			...
			insert into label_item   (     req_date,        salesman,       req_seq,      req_flag,      seq,                    order_no,          seq_no,             label_no,          item_no,          qa,       issue_qty,        sys_date,		 print_group,        print_group_seq,         print_no,      use_yn,   mpsorder_order,    order_seq,    color )
						 	  values   ( :l_sys_date,       :gs_userid,                  1,                 '',          1,              :ls_order_no,      :l_seq_no,       :ls_label_no,       :ls_itemno,    :ls_qa,    :l_issue_qty,    :l_sys_date,	   :l_print_group,    :l_print_group_seq,     :l_print_no,              'N',        :ls_mpsorder,          :li_seq,    :ls_color);
...
for l_print_group = l_print_start_group to ll_cnt			 
		...
		ls_qrdata = l_base_data + ls_qrdata	/*2019.03.15 JOWONSUK 끝을 구분하기 위해서 구분자로 $ 다시 뺌 */
...
//			dw_2.setitem( 1, "color"  + string(ll_line,"00"), ls_color )	// 색상
			dw_2.setitem( 1, "color"  + string(ll_line,"00"), ls_rem )	// 비고 2020-06-23 jowonsuk
...
if gs_label = 'OLD'  then
	...
	dw_2.DataObject = 'd_label_165x100_b01_new'
end if
