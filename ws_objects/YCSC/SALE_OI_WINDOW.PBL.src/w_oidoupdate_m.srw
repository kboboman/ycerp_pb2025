...
type st_1 from statictext within w_oidoupdate_m
end type
...
type rb_a4 from radiobutton within w_oidoupdate_m
end type
type rb_a5 from radiobutton within w_oidoupdate_m
end type
...
type st_5 from statictext within w_oidoupdate_m
end type
type em_top from editmask within w_oidoupdate_m
end type
type gb_5 from groupbox within w_oidoupdate_m
end type
type gb_8 from groupbox within w_oidoupdate_m
end type
...
public subroutine wf_print (datawindow arg_dw);long   ll_count, ll_idx, ll_int,  ll_cnt, ll_line_cnt
...
for ll_count = 1 to dw_2.rowcount()
	if ll_idx = 0 then
      ...
      arg_dw.object.do_no[ll_int]   = dw_1.object.do_no[1]
	   ...
	   select register_no into :ls_cap_name 
		  from customer
...
//    dw_1.object.do_cash[1] = dw_1.object.do_cash[1] + dw_2.object.rlse_amount[li_int]
////원본 20070927
////    dw_1.object.do_rate[1] = dw_1.object.do_rate[1] + truncate(dw_2.object.rate_cash[li_int] + 0.0000001, 0)	 
//   if dw_2.object.rate_cash[li_int] < 0 then
//		dw_1.object.do_rate[1] = dw_1.object.do_rate[1] + truncate(dw_2.object.rate_cash[li_int] - 0.0000001, 0)	 
//	else
//		dw_1.object.do_rate[1] = dw_1.object.do_rate[1] + truncate(dw_2.object.rate_cash[li_int] + 0.0000001, 0)	 
//	end if
...
for li_int = 1 to dw_2.rowcount()
   ...
   if dw_2.object.rate_cash[li_int] < 0 then
		dw_1.object.do_rate[1] = dw_1.object.do_rate[1] + truncate(dw_2.object.rate_cash[li_int] - 0.0000001, 0)	 
	elseif dw_2.object.rate_cash[li_int] > 0 then
		dw_1.object.do_rate[1] = dw_1.object.do_rate[1] + truncate(dw_2.object.rate_cash[li_int] + 0.0000001, 0)	 
	end if
next
...
this.Control[iCurrent+26]=this.rb_a4
this.Control[iCurrent+27]=this.rb_a5
this.Control[iCurrent+28]=this.cbx_8
this.Control[iCurrent+29]=this.cbx_9
...
long    ll_row,   ll_inauditrow
int     li_count, li_chk,    li_int,   li_seq,     ll_do_seq, ll_saleseq
...
if GF_PERMISSION("거래명세서_매출이월", gs_userid) = "Y" then			// 한미란만 존재함
...
IF li_count >= 1 THEN
   ...
   IF li_chk = 1 THEN 
		...
		for ll_row = 1 to dw_2.rowcount()
			...
			if ld_remissue + dw_2.object.ori_ship[ll_row] < dw_2.object.ship_qty[ll_row] then
		...
		if ls_case <> 'C' and ls_case <> 'D' then
			for ll_row = 1 to dw_2.rowcount()
				...
				if dw_2.object.ori_ship[ll_row] <> dw_2.object.ship_qty[ll_row] OR &
					dw_1.GetItemdatetime(1, "issue_date", Primary!, TRUE) <> dw_1.GetItemdatetime(1, "issue_date", Primary!, FALSE) then
					//입출고내역저장
		...
		if lb_ewol = true then		// 출고일자 변경
			// 매출이월
         INSERT INTO do_ewol
			SELECT * FROM do     WHERE do_no = :ls_do_no1;			
			// 매출이월상세
         INSERT INTO dodet_ewol
			SELECT * FROM dodet  WHERE do_no = :ls_do_no1;
			if SQLCA.SQLCode = 0 then
				COMMIT;
            MessageBox("확인","이월처리 작업완료.")
         else
				ROLLBACK;
            MessageBox("확인","이월처리 작업중 오류가 발생하였습니다.~n~n" + SQLCA.SQLErrText)
			end if
         ...
         // 소급분 삭제하기
			if lb_goback = true then
				...
				if SQLCA.SQLCode = 0 then
					COMMIT;
					MessageBox("확인","소급분 삭제 작업완료.")
				else
					ROLLBACK;
					MessageBox("확인","소급분 삭제처리 작업중 오류가 발생하였습니다.~n~n" + SQLCA.SQLErrText)
				end if
			end if
		...
		ls_merge = '출하번호:' + is_f_do_no + ' 의 자료가 ' + gs_username + ' (' + gs_userid + ')에 의해 변경되었습니다.'
		...
		INSERT INTO do_log (log_date, log_seq, gs_userid, do_no, user_id, do_area_no, do_case, do_date, issue_date, ship_amount, ship_flag, do_cash, do_rate, do_total, do_jobdesc, cust_no, spot_code)
		VALUES (convert(datetime, convert(varchar(10), getdate(), 112)), :li_seqno, :gs_userid, :is_f_do_no, :is_f_do_user_id, :is_f_do_area_no, :is_f_do_case, :id_f_do_date, :id_f_issue_date, :idc_f_ship_amount, :is_f_ship_flag, :idc_f_do_cash, :idc_f_do_rate, :idc_f_do_total, null, :is_cust_no, :is_spot_code);
		li_seqno++
		INSERT INTO do_log (log_date, log_seq, gs_userid, do_no, user_id, do_area_no, do_case, do_date, issue_date, ship_amount, ship_flag, do_cash, do_rate, do_total, do_jobdesc, cust_no, spot_code)
		VALUES (convert(datetime, convert(varchar(10), getdate(), 112)), :li_seqno, :gs_userid, :is_t_do_no, :is_t_do_user_id, :is_t_do_area_no, :is_t_do_case, :id_t_do_date, :id_t_issue_date, :idc_t_ship_amount, :is_t_ship_flag, :idc_t_do_cash, :idc_t_do_rate, :idc_t_do_total, :ls_do_jobdesc, :is_cust_no, :is_spot_code);
...
choose case dwo.name
	case 'price', 'uom', 'ship_qty'
		if this.object.sale_tax_sign[row] = 'Y' then
...
event dw_2::itemerror;call super::itemerror;choose case dwo.name
	case 'stock_qty'
...
event dw_2::editchanged;call super::editchanged;this.AcceptText()
end event
...
if cbx_6.checked = true then	// 명세
	if rb_a4.checked = true then
		dw_9.dataobject = 'd_oigiving_r_21'
		dw_9.Modify('DataWindow.Print.Paper.Size = 9')
...
if cbx_7.checked = true then	// 인수
	if rb_a4.checked = true then
		dw_9.dataobject = 'd_oigiving_r_22'
		dw_9.Modify('DataWindow.Print.Paper.Size = 9')
		dw_9.modify("DataWindow.print.margin.left = " + ls_left )
		dw_9.modify("DataWindow.print.margin.top  = " + ls_top  )
		dw_9.Modify('DataWindow.Print.Orientation = 1')
...
if cbx_8.checked = true then	// 출고
	if rb_a4.checked = true then
		dw_9.dataobject = 'd_oigiving_r_23'
		dw_9.Modify('DataWindow.Print.Paper.Size = 9')
		dw_9.modify("DataWindow.print.margin.left = " + ls_left )
		dw_9.modify("DataWindow.print.margin.top  = " + ls_top  )
		dw_9.Modify('DataWindow.Print.Orientation = 1')
...
if cbx_9.checked = true then	// 납품
	if rb_a4.checked = true then
		dw_9.dataobject = 'd_oigiving_r_24'
		dw_9.Modify('DataWindow.Print.Paper.Size = 9')
		dw_9.modify("DataWindow.print.margin.left = " + ls_left )
		dw_9.modify("DataWindow.print.margin.top  = " + ls_top  )
		dw_9.Modify('DataWindow.Print.Orientation = 1')
...
event pb_delete::clicked;string ls_do_no, ls_case, ls_sms_yn, ls_s_area_no, ls_s_tel, ls_do_jobdesc
long   ll_row, ll_cnt
date   ldt_issue_date
dwitemstatus l_status, l_jobdesc_status
...
ls_sms_yn = dw_1.object.sms_yn[dw_1.getrow()]
ls_s_area_no = dw_1.object.do_area_no[dw_1.getrow()]
if ls_sms_yn = '' or isnull(ls_sms_yn) then
	ls_sms_yn = 'N'
end if
if MessageBox("확인", "출하 취소내역을 영업/업체/현장 담당자에게 문자메세지로 전송 하시겠습니까?", question!, YesNo!, 2) = 2 then
	ls_sms_yn = 'N'
else
	ls_sms_yn = 'Y'
end if
...
if ls_sms_yn = 'Y' then
	...
	SELECT top 1 b.cust_name, c.scene_desc, convert(varchar(10), getdate(), 111),
	...
	WHERE a.do_no       = :ls_do_no;
		
	ls_s_msg = '[송천-출하취소]' + ls_s_cust_name + '의 ' + ls_s_scene_desc + '에 ' +  ls_do_no  + ' ' +  RightA(ls_s_do_date,5)
...
if wf_update2(dw_2, dw_inaudit,"Y") = true then
	...
	ls_merge = '출하번호:' + is_f_do_no + ' 의 자료가 ' + gs_username + ' (' + gs_userid + ')에 의해 삭제 되었습니다.'
	INSERT INTO sysmsg ( flogin,	tlogin, msgdate, msg, del ) VALUES ( :gs_userid, 'hanmr', getdate(), :ls_merge,'N');
	COMMIT;
	...
	li_seqno++
	INSERT INTO do_log (log_date, log_seq, gs_userid, do_no, user_id, do_area_no, do_case, do_date, issue_date, ship_amount, do_cash, do_rate, do_total, do_jobdesc, cust_no, spot_code)
	VALUES (convert(datetime, convert(varchar(10), getdate(), 112)), :li_seqno, :gs_userid, convert(varchar, :li_seqno - 1) + '번 삭제처리', null, null, null, null, null, 0, 0, 0, 0, :ls_do_jobdesc, :is_cust_no, :is_spot_code);
	COMMIT;
	...
	if ls_sms_yn = 'Y' then
		...
		IF SMSSERVER <> "192.168.1.5" THEN
			Transaction SQLCA1
			If Not IsValid(SQLCA1) Then SQLCA1 = CREATE Transaction
		
			SQLCA1.DBMS       = "MSS Microsoft SQL Server 8.x"
			SQLCA1.Database   = "SMS"
			SQLCA1.LogID      = "db_user"		// INSERT Only
			SQLCA1.LogPass    = ""
			SQLCA1.ServerName = SMSSERVER
			SQLCA1.UserID     = ""
			SQLCA1.DBPass     = ""
			SQLCA1.Lock       = "ReadOnly"
			SQLCA1.DbParm     = "CursorLock='ReadOnly',AppName='YCSC',Host= 'SMS LOGIN"
			SQLCA1.AutoCommit = False 
		
			CONNECT USING SQLCA1;
			IF SQLCA1.SQLCode = -1 THEN
				 MessageBox("SMS단문", "SMS단문 Database와 연결이 되지 않습니다.~r~n" + &
							  "에러코드: " + String(SQLCA1.SQLDBCode ) + "~r~n" + &
							  "에러내용: " + SQLCA1.SQLErrText, StopSign!, OK!, 1 )
			END IF
			If SQLCA1.SQLCode <> 0 then 
				  MessageBox( "MS-SQL DB Connect ERROR", SQLCA1.SQLErrText ) 
				//HALT CLOSE 
			End If 

			TRAN_CALLBACK = ls_s_stel
			TRAN_STATUS   = '1'
			//GW SMS=====================================
		END IF
		...
		if ls_s_hp_no = '' or isnull(ls_s_hp_no) then
			...
			choose case SMSSERVER
				case '192.168.1.5'
					...
					VALUES ( GetDate(), '0', '00', :ls_s_hp_no, :ls_s_stel, :ls_s_msg);

				case 'www.yoo-chang.co.kr', '218.159.85.109'
					If SQLCA1.SQLCode = 0 Then 
						TRAN_PHONE    = ls_s_hp_no
						TRAN_MSG      = ls_s_msg
      ...
      // 현장담당
		if ls_s_scene_tel = '' or isnull(ls_s_scene_tel) then
			...
			choose case SMSSERVER
				case '192.168.1.5'
					INSERT INTO SC_TRAN ( TR_SENDDATE, TR_SENDSTAT, TR_RSLTSTAT, TR_PHONE, TR_CALLBACK, TR_MSG)
					VALUES ( GetDate(), '0', '00', :ls_s_scene_tel, :ls_s_hp_no, :ls_s_msg);

				case 'www.yoo-chang.co.kr', '218.159.85.109'
					if SQLCA1.SQLCode = 0 then 
						TRAN_PHONE    = ls_s_scene_tel
						TRAN_MSG      = ls_s_msg
      ...
      // 거래처 담당
		if ls_s_cust_tel = '' or isnull(ls_s_cust_tel) then
		else
			choose case SMSSERVER
				case '192.168.1.5'
					INSERT INTO SC_TRAN ( TR_SENDDATE, TR_SENDSTAT, TR_RSLTSTAT, TR_PHONE, TR_CALLBACK, TR_MSG)
					VALUES ( GetDate(), '0', '00', :ls_s_cust_tel, :ls_s_hp_no, :ls_s_msg);
...
for ll_row = 1 to UpperBound(lstr_where.str1)						
	...
	dw_2.object.sale_tax_sign[ll_cnt] = trim(lstr_where.str7[ll_row])
...
event clicked;string ls_cust, ls_scene, ls_dono, ls_name
