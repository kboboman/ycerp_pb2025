...
public function string wf_coil (integer arg_row);long ll_row, ll_count, l_len, l_pos, l_seq
int li_col
...
//ls_date = ls_code1 + ls_code2 + mid(string(dw_1.object.rcpt_date[1], 'yyyymmdd'), 3) + "%"
...
ls_date = ls_code1 + ls_code2 + MidA(string(dw_1.object.insp_date[1], 'yyyymmdd'), 3)
setnull(ls_coil)
l_len = 0 
...
if l_len <= 3 then
	...
	for ll_row = 1 to dw_1.rowcount()
		if not isnull(dw_1.object.sheet_no[ll_row]) then
			...
			l_pos = PosA( dw_1.object.sheet_no[ll_row] , '-' )
			...
			if LeftA(ls_coil,2) = LeftA(dw_1.object.sheet_no[ll_row], 2) &
				and long(MidA(ls_coil,3,9)) < long(MidA(dw_1.object.sheet_no[ll_row],3, l_pos)) then
...
dw_1.object.item_name[1] = ist_code.name
...
choose case dwo.name
	case 'item_no'
		...
		ls_item = this.object.item_no[ll_row]		
		this.object.item_name[ll_row] = ls_itemname  
		if ls_item <> '' then
