...
string ls_date, ls_yyyymm, ls_mon
...
//dw_1.width  = this.width  - 3410
dw_1.height = this.height - 564

//dw_3.y      = dw_4.y + dw_4.height + 10
dw_3.width  = this.width  - 1532

dw_4.width  = this.width  - 1532
dw_4.height = this.height - 1300
...
event pb_insert::clicked;string ls_cust_no, ls_date, ls_year, ls_tmpid, ls_cust_name
long   ll_year
...
ll_row    = dw_3.retrieve( ls_cust, ls_yyyymm )	// 수금상세 조회
if ll_row < 1 then
	for ll_y = 1 to 5
...
//
//long    ll_row, ll_chk
//decimal ll_cash, ll_cash1, ll_bill, ll_bill1, ll_setoff, ll_setoff1
//string  ls_bigo, ls_bigo1
//
//ll_chk  = 0
//for ll_row = 1 to 5
//	// 수정값, 원본값
//	ll_cash    = dw_3.object.cash[ll_row]
//   ll_cash1   = dw_3.object.cash1[ll_row]
//	ll_bill    = dw_3.object.bill[ll_row]
//   ll_bill1   = dw_3.object.bill1[ll_row]
//	ll_setoff  = dw_3.object.setoff[ll_row]
//   ll_setoff1 = dw_3.object.setoff1[ll_row]
//	
//	ls_bigo    = dw_3.object.bigo[ll_row]
//   ls_bigo1   = dw_3.object.bigo1[ll_row]
//
//	if ll_cash   <> ll_cash1   then ll_chk = ll_chk + 1
//	if ll_bill   <> ll_bill1   then ll_chk = ll_chk + 1
//	if ll_setoff <> ll_setoff1 then ll_chk = ll_chk + 1
//	if ls_bigo   <> ls_bigo1   then ll_chk = ll_chk + 1
//next
//
//if ll_chk > 0 then
//	if wf_update2(dw_3, dw_4, "N") = false then
//		MessageBox("확인","수금상세 저장중 오류가 발생했습니다.",exclamation!)
//		RETURN
//	end if		
//end if
//
...
if ls_cust3 = ls_cust4 then
	...
	if lr_amt3 = lr_amt4 then
		...
		MessageBox("확인",ls_cust3 + "~n~n" + ls_cust4 + "~n~n" + "수금상세 내역이 당월 수금수정에 반영되지 않았습니다. 확인바랍니다.")
	end if
else
...
event clicked;string ls_custno, ls_custnm
