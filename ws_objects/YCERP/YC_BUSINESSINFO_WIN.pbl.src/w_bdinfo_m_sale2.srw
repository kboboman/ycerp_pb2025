$PBExportHeader$w_bdinfo_m_sale2.srw
$PBExportComments$설계영업진행관리[2001/01/18,서재복]
forward
global type w_bdinfo_m_sale2 from w_inheritance
end type
type dw_plist from datawindow within w_bdinfo_m_sale2
end type
type pb_insert2 from picturebutton within w_bdinfo_m_sale2
end type
type pb_delete2 from picturebutton within w_bdinfo_m_sale2
end type
type gb_4 from groupbox within w_bdinfo_m_sale2
end type
type pb_print3 from picturebutton within w_bdinfo_m_sale2
end type
type dw_print5 from datawindow within w_bdinfo_m_sale2
end type
type tab_1 from tab within w_bdinfo_m_sale2
end type
...
type cb_wf_ret from commandbutton within w_bdinfo_m_sale2
end type
end forward
...
integer width = 4000
integer height = 2908
string title = "판매영업진행입력(w_bdinfo_m_sale2)"
...
global w_bdinfo_m_sale2 w_bdinfo_m_sale2
...
datawindow bd_getdw1, bd_getdw, dw4, dw5, dw3, dw6
string is_scenesql
...
if gs_winname = 'w_oecustdiary' or gs_winname = 'w_bdinfo_m1' then
//	gs_winname = ''
...
event closequery;call super::closequery;if gs_winname = 'w_oecustdiary' or gs_winname = 'w_bdinfo_m1' then
	gs_winname = ''
end if
end event

type pb_save from w_inheritance`pb_save within w_bdinfo_m_sale2
integer x = 3561
end type

event pb_save::clicked;string ls_nullvalue, ls_existvalue
long ll_row1, ll_row2, ll_row3, ll_cnt1, ll_cnt2, ll_cnt4, ll_cnt5, ll_cnt3, ll_cnt6
long ll_existcount
...
//		select count(builder) into :ll_existcount from bdinfodet_sale1
//		where project = :ls_nullvalue;

		l_status = dw3.GetItemStatus(ll_row3, 0, Primary!)
...
//		end if
		...
		ll_cnt3 = wf_modifycount(dw3) 
		ll_cnt6 = wf_modifycount(dw6) 
		
		if ll_cnt3 > 0  or ll_cnt6 > 0 then	 
			wf_update2(dw3,dw6,"Y") 
			IF l_status = newModified! OR l_status = new! THEN
				dw_plist.retrieve(is_build_no, is_divcd, is_speccd, is_project, is_sprog, id_fixeddate1, id_fixeddate2, is_dprog, is_cprog, is_city, &
				is_person, is_dperson, is_ddiv, is_cperson, is_cdiv, is_bperson, is_bdiv, is_sperson, is_sdiv, is_organcd, is_designcd)
			end if
...
type dw_1 from w_inheritance`dw_1 within w_bdinfo_m_sale2
...
type st_title from w_inheritance`st_title within w_bdinfo_m_sale2
integer y = 36
integer height = 164
integer textsize = -20
string text = "판매영업 진행입력"
end type

type st_tips from w_inheritance`st_tips within w_bdinfo_m_sale2
end type

type pb_compute from w_inheritance`pb_compute within w_bdinfo_m_sale2
boolean visible = false
integer x = 4087
integer y = 64
...
type pb_print_part from w_inheritance`pb_print_part within w_bdinfo_m_sale2
boolean visible = false
integer x = 4233
integer y = 64
...
type pb_close from w_inheritance`pb_close within w_bdinfo_m_sale2
integer x = 3753
...
type pb_print from w_inheritance`pb_print within w_bdinfo_m_sale2
boolean visible = false
integer x = 3739
integer y = 64
...
type pb_cancel from w_inheritance`pb_cancel within w_bdinfo_m_sale2
boolean visible = false
integer x = 3927
integer y = 64
...
type pb_delete from w_inheritance`pb_delete within w_bdinfo_m_sale2
integer x = 3369
...
string ls_delbuildno, ls_delspec_code, ls_builder
dec ld_delseq
...
//		end if
		...
		if bd_getdw1 = dw3 then
			long ll_dw3row
			ll_dw3row = dw3.getrow()
			if ll_dw3row < 1 then return
//			ls_delbuildno = dw3.object.build_no[ll_dw3row]
//			ld_delseq = dw3.object.seq[ll_dw3row]
			ls_builder = dw3.object.builder[ll_dw3row]

			delete FROM bdinfodet_sale2
			where build_no = :ls_delbuildno
			and 	seq = :ld_delseq 
			and	builder = :ls_builder;
			if sqlca.sqlcode = 0 then
				...
				rollback;
				messagebox('경고 NO_2', '삭제 오류가 발생 되었습니다.  전산실에 문의 바랍니다.')
			end if
		else
			if dw6.getrow() < 1 then return
			dw6.deleterow(0)
			wf_delete1(dw6,"Y")
		end if
...
type pb_insert from w_inheritance`pb_insert within w_bdinfo_m_sale2
integer x = 3177
integer taborder = 180
end type

event pb_insert::clicked;decimal ld_seq_no
long ll_row, ll_dw4row, ll_dw5row,  ll_dw3row, ll_dw6row, ll_seq
string ls_build_no
...
//		dw_2.setfocus()
		if bd_getdw1 = dw3 then
			long ll_dw3setrow
			dw3.accepttext()
			ll_dw3row = dw3.insertrow(0)
			dw3.object.build_no[ll_dw3row] = dw_2.object.bdinfodet_build_no[1]
			dw3.object.seq[ll_dw3row] = dw_2.object.bdinfodet_seq[1]
			ls_build_no = dw_2.object.bdinfodet_build_no[1]
			ll_seq = dw_2.object.bdinfodet_seq[1]
			dw3.scrolltorow(ll_dw3row)
			dw6.reset()

//			select isnull(seq_no,0) into :ll_dw3setrow from bdinfodet2
//			where build_no = :ls_build_no
//			and   seq = :ll_seq;
//			if isnull(ll_dw3setrow) or ll_dw3setrow = 0 then
//				ll_dw3setrow = 1
//			else
//				ll_dw3setrow = ll_dw3setrow + 1
//			end if
//			dw3.object.seq_no[ll_dw3row] = ll_dw3setrow
//			if ll_dw3row > 1 then
//				if dw3.object.builder[ll_dw3row - 1] = dw3.object.builder[ll_dw3row] then
//					dw3.deleterow(ll_dw3row)
//					messagebox('확인', '자재코드는 1줄씩 입력하고 저장하세요.')
//					return
//				end if
//			end if
			dw3.setcolumn('builder')
			dw3.setfocus()
		else
			if dw3.getrow() >= 1 then
				dw3.accepttext()
				ll_dw6row = dw6.insertrow(0)
				dw6.object.build_no[ll_dw6row] = dw_2.object.bdinfodet_build_no[1]
				dw6.object.seq[ll_dw6row] = dw_2.object.bdinfodet_seq[1]
				dw6.object.builder[ll_dw6row] = dw3.object.builder[dw3.getrow()]
				dw6.object.seq_no[ll_dw6row] = ll_dw6row
				dw6.object.i_date[ll_dw6row] = gf_today() //gdt_today
				dw6.scrolltorow(ll_dw6row)
				dw6.setcolumn('i_date')
				dw6.setfocus()
			end if
		end if		
...
type pb_retrieve from w_inheritance`pb_retrieve within w_bdinfo_m_sale2
integer x = 2793
...
type gb_3 from w_inheritance`gb_3 within w_bdinfo_m_sale2
string tag = "736"
integer x = 741
integer y = 216
integer width = 3232
...
type gb_2 from w_inheritance`gb_2 within w_bdinfo_m_sale2
boolean visible = false
integer x = 3749
integer y = 948
integer width = 512
...
type gb_1 from w_inheritance`gb_1 within w_bdinfo_m_sale2
integer x = 2752
integer width = 1216
end type

type dw_2 from w_inheritance`dw_2 within w_bdinfo_m_sale2
integer x = 763
integer y = 268
integer width = 3182
integer height = 564
integer taborder = 60
string dataobject = "d_bdlist_1_s_new2"
...
string dataobject = "d_bdinfo_input_l_sale"
...
dw3.reset()
dw6.reset()
dw_2.retrieve( ld_agseq )
dw3.retrieve( ld_agseq )
...
event rowfocuschanged;string ls_builder
long ll_seq
...
dw6.retrieve( ll_seq, ls_builder )

end event

event retrieveend;string ls_builder
long ll_seq
if rowcount < 1 then return

ls_builder = this.object.builder[this.getrow()] 
ll_seq = this.object.seq[this.getrow()] 
...
event doubleclicked;choose case dwo.name
	case 'builder'
		...
		if l_status = newModified! or l_status = new! then
			openwithparm(w_whcustret_w, ist_whcustret)
			ist_whcustret = Message.PowerObjectParm
			if ist_whcustret.cust_chk = 'Y' then
//				ist_whcustret.cust_name
				this.object.builder[row] = ist_whcustret.cust_no
			end if
		end if
end choose
end event

event clicked;string ls_builder
long ll_seq
...
integer taborder = 130
string title = "none"
string dataobject = "d_bdinfodet2_item_m_sale"
boolean hscrollbar = true
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
...
integer taborder = 120
string title = "none"
string dataobject = "d_bdinfodet2_m_sale"
boolean hscrollbar = true
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
