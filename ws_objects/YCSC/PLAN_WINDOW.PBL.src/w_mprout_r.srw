...
event clicked;string ls_qa,ls_routqa,ls_item,ls_qa1,ls_assy,ls_assyqa,ls_wc,ls_temp
...
for i = ll_rowcount to 1
	...
	for j = i to 1
	   ...
	   insert into rout
	     (item_no,qa,operation,concurrent,wc_no,rout_desc,setup_time,queue_time,run_time,delivery_time,
         assembly,assy_qa,bom_no,qpa,work_per)
	   values (:ls_item,:ls_qa,:li_seq,null,:ls_wc,null,:lr_setup,0,:lr_run,0,:ls_assy,:ls_assyqa,0,:lr_qpa,:lr_work);
	   if sqlca.sqlcode < 0 then
	      messagebox('오류','라우트테이블에 인서트 에러가 발생했습니다.',exclamation!)
		   rollback;
		   return
	   end if
	next
...
string  ls_qa, ls_item, ls_qa1, ls_assy, ls_uom, ls_source, ls_qalike, ls_itemname, ls_temp
...
event clicked;// 일괄처리
string  ls_qa, ls_routqa, ls_item, ls_qa1, ls_assy, ls_wc
long    i, j
real    lr_setup, lr_run, lr_qpa, lr_work
integer li_seq
...
for i = 1 to dw_1.rowcount()
	if dw_1.object.chk[i] = "N"   then continue		// 미선택
// if isnull(dw_1.object.rout_qa[i]) = false then continue
	if isnull(dw_1.object.rout_qa[i]) then
	else
		continue
	end if
