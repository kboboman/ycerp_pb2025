...
type cbx_edit from checkbox within tabpage_2
end type
...
type cbx_erp from checkbox within w_orsms_m
end type
type cb_8 from commandbutton within w_orsms_m
end type
type cb_9 from commandbutton within w_orsms_m
end type
type st_info from statictext within w_orsms_m
end type
...
this.Control[iCurrent+30]=this.cbx_erp
this.Control[iCurrent+31]=this.cb_8
this.Control[iCurrent+32]=this.cb_9
this.Control[iCurrent+33]=this.st_info
...
if GF_Permission( "SMS발신내용관리", gs_userid ) = "Y" then
	...
	cbx_erp.visible    = true		// ERP 전송
end if

string ls_office, ls_officenm

setnull(ls_officenm)

SELECT rtrim(item_nm), rtrim(BIGO) INTO :ls_officenm, :ls_office 
FROM codemst
WHERE type = 'SMSSEND' AND item_cd = :gs_userid AND use_yn = 'Y';
 
if isnull(ls_office) OR ls_office = "" then
	MessageBox("확인","발신번호 미 등록시 문자전송할 수 없습니다.")
	pb_save.enabled = false
else
	pb_save.enabled = true

	choose case ls_office
		case "0226489600"
			ls_officenm = "관리본부"
		case "0226481998"
			ls_officenm = "고객센터"
		case "0226901999"
			ls_officenm = "서부영업소(화곡)"
		case "0426357002"
			ls_officenm = "중부호남영업지점"
		case "0317551600"
			ls_officenm = "동부영업소(성남)"
		case "0226404675"
			ls_officenm = "기술영업부"
		case "0314999600"
			ls_officenm = "시화공장"
		case "0313559631"
			ls_officenm = "화성공장"
		case else
			if isnull(ls_officenm) or ls_officenm = "" then
				ls_officenm = "발신불가"
				pb_save.enabled = false
			end if
	end choose
	
	st_info.text    = ls_officenm
	em_from.text    = ls_office
end if


if isvalid(istr_where) then
	istr_where.str1 = ""
	em_to.text   = GF_ReplaceAll(istr_where.str1, "-","")
	if em_to.text = "   -    -    " then
	else
		sle_toname.text = istr_where.name
	end if
end if
// 전송내역 조회
...
for ll_row = 1 to dw_1.rowcount()	
	...
	if ls_phone = "" or isnull(ls_phone) then
		...
		lb_mmssend = GF_MMSSEND( gs_userid, ls_sendhp, ls_phone, ls_phonename, ls_message )
	end if
next

if lb_mmssend = true then
...
if cbx_erp.checked = true then
	dw_2.retrieve( "system", ls_sdate, string(today(),"yyyy/mm/dd"), ls_admin )
else
...
choose case dwo.name
	...
	case "tr_msg"
	   mle_msg.text = gf_replaceall(this.object.tr_msg[row],CharA(13),"~r")
	   mle_msg.text = gf_replaceall(this.object.tr_msg[row],CharA(10),"~n")
end choose
...
for ll_row = 1 to tab_1.tabpage_2.dw_5.rowcount()
	tab_1.tabpage_2.dw_5.object.chk[ll_row] = ls_chk
next
...
tab_1.tabpage_2.cbx_edit.checked = true
tab_1.tabpage_2.cbx_edit.triggerevent( clicked! )
...
event clicked;//
string	ls_phoneno, ls_phonenm, ls_pathname
long		ll_row, ll_rc
integer  iRet

ll_rc = dw_1.ImportClipBoard()		// 1번 라인 SKIP
if ll_rc > 0 then
	...
	for ll_row =  1 to dw_1.rowcount()
		if PosA(dw_1.object.phone_no[ll_row],"-") > 0 then
			dw_1.object.phone_no[ll_row]   = gf_replaceall(dw_1.object.phone_no[ll_row],"-","")
			dw_1.object.phone_name[ll_row] = gf_replaceall(dw_1.object.phone_name[ll_row],"-","")
		end if
	next
	MessageBox("확인","엑셀 붙여넣기 완료" )
end if
...
fontfamily fontfamily = modern!
string facename = "굴림체"
string text = "수신번호 <-> 수신자명"
end type

event clicked;//
string	ls_phoneno, ls_phonenm
long		ll_row, ll_rc
integer  iRet
...
for ll_row = 1 to dw_1.rowcount()
	ls_phoneno = dw_1.object.phone_no[ll_row]
	ls_phonenm = dw_1.object.phone_name[ll_row]
	
	dw_1.object.phone_no[ll_row]   = ls_phonenm
	dw_1.object.phone_name[ll_row] = ls_phoneno	
next

MessageBox("확인","수신번호 <-> 수신자명 변경 완료" )
