...
type cb_6 from commandbutton within w_mpsliting_m
end type
type sle_order_no from singlelineedit within w_mpsliting_m
end type
type sle_1 from singlelineedit within w_mpsliting_m
end type
end forward
...
SELECT :ls_chkcoil,  wid,    item_no,   qa,    loc_no,  :ld_date, unit_qty, length,  
		 :ai_cnt, :ai_cnt,	color_no, supp_no, flag, '분할', 0, open_close, rem, rcpt_qty, rcpt_len, etc_no, :as_coil
  FROM coilmst		
...
is_chkcoil = ls_chkcoil /*2021.01.18 jowonsuk 추가 이코일을 지울로직을 위해서 여기서 코일번호받아서 coilorderdet의 rem5에 분할 코일번호을 저장한다. */
return true
...
this.Control[iCurrent+10]=this.tab_2
this.Control[iCurrent+11]=this.st_1
this.Control[iCurrent+12]=this.cb_6
this.Control[iCurrent+13]=this.sle_order_no
...
long    ll_row	, l_los_qty
string  ls_item_no, ls_qa, ls_qa1, ls_qa2, ls_qa3, ls_qa4, ls_flag, ls_loc
...
//================================================================================================
	...
	/*2019.06.10 jowonsuk 디테일부터 먼저 점검하고 마스터를 점검하고 저정하는 로직 변경 
	  기존에 마스터부터 저장하면 coilmst에 선반영되다고 오류메세지 만나면 문제 발생 
	*/
/////////////////////////
// 스리팅 용도 등록
/////////////////////////
...
if l_status = New! or l_status = NewModified! then 
	...
	/*2019.06.10 jowonsuk 32767(smallint) 넘어가면 에러메세지*/	
	l_los_qty = dw3.object.los_qty[1]	
	
	if l_los_qty >= 32767 then
		MessageBox('오류','로스중량이 너무 큽니다. 다시 입력하십시요.',exclamation!)
		dw3.setcolumn('los_qty')
		dw3.setfocus()
		RETURN
	end if	
...
//	ldt_date = date(dw3.object.order_date[1])
	ldt_date = datetime(dw3.object.order_date[1])
	ls_loc   = dw3.object.loc_no[1]
...
for ll_row = 1 to dw_2.rowcount()	
	...
	// 115X-0004X0000X00000
	if ls_qa <> "" then
		GF_SPLIT( ls_qa, "X", ls_qa_split )
		if LenA(ls_qa_split[1]) <> 3 or LenA(ls_qa_split[2]) <> 4 or LenA(ls_qa_split[3]) <> 4 or LenA(ls_qa_split[4]) <> 5 then
			Messagebox('확인','제품 규격을 정확히 표기하시기 바랍니다.(마이너스(-) 표기안됨)')
			RETURN
		end if
	else
		Messagebox('확인','제품 규격을 정확히 표기하시기 바랍니다.(공난처리 안됨)')
		RETURN
	end if
	
	if ls_sheet <> 'L' then
		if ls_slitemnm = '' or IsNull(ls_slitemnm) then
			Messagebox('확인','스리팅 용도 제품명은 공난처리 안됩니다.!!')
			RETURN
		end if
		if MidA(ls_slitemnm,1,1) <> 'E' then
			if ls_slqa  = '' or IsNull(ls_slqa) then
				Messagebox('확인','스리팅 용도 규격은 공난처리 안됩니다.!!')
				RETURN
			end if
		end if
	end if
...
//		else
//		  ls_order = trim(ls_order)
//		  li_num   = integer(right(ls_order,2))
//		  li_num++
//		  ls_num   = right('000' + string(li_num),2)
//		  ls_order = ls_area + ls_date + "-" + ls_num
//		  dw3.object.order_no[1] = ls_order
//		end if
//	end if
...
for ll_row = 1 to dw4.rowcount()
	...
	if l_status = New! or l_status = NewModified! then 
		...
		if dw4.object.cnt[ll_row] < dw4.object.coilmst_qcnt[ll_row] then		// 갯수 < 재고수량
			...
			dw4.object.rem[ll_row] = is_chkcoil
...
if dw4.accepttext() = -1 then return
...
event dw_1::clicked;//
string ls_order, ls_proc_flag
long   ll_row

if row < 1 then return

ls_order = trim(dw_1.object.order_no[row])
ls_proc_flag = trim(dw_1.object.proc_flag[row])

sle_order_no.text = ls_order
...
choose case tab_1.SelectedTab
	case 2	// 코일합체내역
		...
		if l_status = New! or l_status = NewModified! then 
			...
			if sqlca.sqlcode <> 0 then
				messagebox('오류3',"코일마스터에 자료가 없습니다.~r~n" + sqlca.sqlerrtext, exclamation!)
				rollback;
				return
			end if
...
string ls_item,ls_data,ls_qa,ls_uom,ls_itemname,ls_loc,ls_data1,ls_supp,ls_wc
...
choose case dwo.name
	case 'item_no'
		   ls_data = data
			...
			this.object.ga_item_name[ll_row] = ls_itemname
...
choose case dwo.name
	case "r_item_no", "gb_item_name"
		...
		if lstr_select.chk = "Y" then
			...
			this.object.gb_item_name[row] = lstr_select.name[1]
...
event clicked;long   ll_row, ll_dw2row, ll_front, ll_seq
...
if ll_seq > 1 then
	...
	dw_2.object.ga_item_name[ll_row] = dw_2.object.ga_item_name[ll_row - 1]
else
	...
	if sqlca.sqlcode = 0 then 
		...
		into
			...
			,:ls_itemname			 
		from 
			groupitem a inner join groupitem b on a.item_no = b.assembly_item_no and b.item_no = :ls_item
		using sqlca;
	...
	dw_2.object.ga_item_name[ll_row]		= ls_itemname
...
fontfamily fontfamily = modern!
string facename = "굴림체"
string text = "로스중량을 0로"
end type

event clicked;// 로스중량을 0로
dw3.accepttext()
if dw3.rowcount() < 1 then return
...
event itemchanged;//
string ls_data, ls_item, ls_itemname, ls_qa1, ls_null, ls_loc_no
long   ll_row, l_row
dec{2} ld_null
...
choose case dwo.name
	case 'coilmst_item_no'
		...
		this.object.coilmst_loc_no[ll_row] = 'WW01'
		ls_loc_no= 'WW01'
		
		is_where  = ' and item_no = ~~"' + ls_data + '~~" ' 
		is_where  = is_where + ' and coilmst.loc_no = ~~"' + ls_loc_no + '~~" ' 		
		is_clause = "DataWindow.Table.Select='"  + is_select + is_where + "'"
		idwc_coil.modify(is_clause)
		idwc_coil.settransobject(sqlca)
		idwc_coil.retrieve()
		
		this.setcolumn('coilmst_loc_no')
		this.setfocus()
		
	case 'coilmst_loc_no'
		ls_data = data
		this.object.rt[ll_row]                  = ld_null
		this.object.cnt[ll_row]                  = ld_null		
		this.object.coil_no[ll_row]             = ls_null
		
		ls_item = this.object.coilmst_item_no[ll_row]
		
		is_where  = ' and item_no = ~~"' + ls_item + '~~" ' 
		is_where  = is_where + ' and coilmst.loc_no = ~~"' + ls_data + '~~" ' 		
		is_clause = "DataWindow.Table.Select='"  + is_select + is_where + "'"
		idwc_coil.modify(is_clause)
		idwc_coil.settransobject(sqlca)
		idwc_coil.retrieve()
	...
	case 'coil_no'
		 ...
		 this.object.coilorderdet_length[ll_row]   = idwc_coil.getitemnumber(idwc_coil.getrow(),'length')		/*2019.01.24 jowonsuk 코일생산계획헤더의 길이*/ 
		 ...
		 if dw_2.rowcount() > 0  then
			...
			for l_row = 1 to  dw_2.rowcount()				
				dw_2.object.thin[l_row] = this.object.rt[ll_row]
				dw_2.object.length[l_row] = this.object.coilmst_length[ll_row]								
			next 
...
GF_SPLIT( ls_qa, "X", ls_qa_split )
if LenA(ls_qa_split[1]) <> 3 or LenA(ls_qa_split[2]) <> 4 or LenA(ls_qa_split[3]) <> 4 or LenA(ls_qa_split[4]) <> 5 then
	Messagebox('확인','제품 규격은 마이너스(-)를 표기할 수 없습니다.')
	Return
end if
...
event clicked;/* 2018.08.30 jowonsuk
   이수원차장 요청 건
    반복 등록으로 복사 기능 반영 

*/
string ls_item, ls_order
string ls_coil_no, ls_item_no, ls_qa, ls_proc_flag, ls_rem, ls_sheet, ls_ga_item_name, ls_loc_no, ls_status, ls_r_Item_no, ls_gb_item_name, ls_r_qa, ls_sqlerrtext
long l_seq, l_wid, l_unit_qty, l_cnt, l_length,  ll_row, l_row
dec l_thin, l_swid


dw3.accepttext()

DECLARE coilsliting_cursor CURSOR FOR 
  SELECT  coilsliting.seq         
			 , coilsliting.coil_no     
			 , coilsliting.wid         
			 , coilsliting.item_no     
			 , coilsliting.qa          
			 , coilsliting.unit_qty    
			 , coilsliting.cnt         
			 , coilsliting.length      
			 , coilsliting.proc_flag   
			 , coilsliting.rem         
			 , coilsliting.sheet       
			 , ga.item_name            
			 , coilsliting.loc_no      
			 , thin = convert(decimal(3,2), substring(qa,1,1) + '.' + substring(qa, 2,2))
			 , coilsliting.status   
			 , swid = (case when sheet = 'C' or sheet = 'L' then convert(decimal(5,1), substring(qa,5,4) + '.' + substring(qa, 10,1))  
								 when sheet = 'S'                then convert(decimal(5,0), substring(qa,5,4)) 
						  end)
			 , coilsliting.r_item_no
			 , gb.item_name         
			 , coilsliting.r_qa
		 FROM coilsliting inner join groupitem ga on coilsliting.item_no     = ga.item_no 
			                        left outer join  groupitem gb on coilsliting.r_item_no = gb.item_no
		WHERE ( coilsliting.order_no    = :ls_order  );	
		
		
ls_order = trim(sle_order_no.text)

OPEN coilsliting_cursor;

FETCH coilsliting_cursor INTO :l_seq, :ls_coil_no, :l_wid, :ls_item_no, :ls_qa, :l_unit_qty, :l_cnt, :l_length, :ls_proc_flag, :ls_rem, :ls_sheet, :ls_ga_item_name, :ls_loc_no, 
										:l_thin, :ls_status, :l_swid, :ls_r_Item_no, :ls_gb_item_name, :ls_r_qa;
		...
		Do While sqlca.sqlcode = 0	
			...
			if sqlca.sqlcode < 0 then
				...
				ROLLBACK;				
				MessageBox('오류','SELECT 중 오류가 발생했습니다.~r~n', exclamation!)
				RETURN 0
			end if		
			
			l_row = dw_2.insertrow(0)		
			
			dw_2.object.seq[l_row] = l_seq
			dw_2.object.coil_no[l_row] = ls_coil_no
			dw_2.object.wid[l_row] = l_wid		
			dw_2.object.item_no[l_row] = ls_item_no					
			dw_2.object.qa[l_row] = ls_qa								
			dw_2.object.unit_qty[l_row] = l_unit_qty								
			dw_2.object.cnt[l_row] = l_cnt	
			
			if  (isnull(dw3.object.coilmst_length[1]) = false) and dw3.rowcount() > 0 then
				l_length = dw3.object.coilmst_length[1]
			else
				l_length = 0
			end if
			
			dw_2.object.length[l_row] = l_length
/*			dw_2.object.proc_flag[l_row] = ls_proc_flag							*/
			dw_2.object.rem[l_row] = ls_rem
			dw_2.object.sheet[l_row] = ls_sheet			
			dw_2.object.ga_item_name[l_row] = ls_ga_item_name						
			dw_2.object.loc_no[l_row] = ls_loc_no
			
			if (isnull(dw3.object.rt[1]) = false) and dw3.rowcount() > 0 then
				l_thin = dw3.object.rt[1]
			else
				l_thin = 0
			end if
			
			dw_2.object.thin[l_row] = l_thin
			dw_2.object.coilsliting_status[l_row] = ls_status
			dw_2.object.sheet[l_row] = ls_sheet
			dw_2.object.r_Item_no[l_row] = ls_r_Item_no
			dw_2.object.gb_item_name[l_row] = ls_gb_item_name
			dw_2.object.r_qa[l_row] = ls_r_qa
			dw_2.object.swid[l_row] = l_swid
			
			string ls_c_wid, ls_c_thin, ls_c_wid2, ls_c_qa
			dec    ld_yrate
			
			ls_c_wid = trim(string(integer(truncate(dw_2.object.swid[l_row],0)),"0000"))
			
			if isnull(ls_c_wid) or trim(ls_c_wid) = '' &
				or truncate(dw_2.object.swid[l_row],0) <> dw_2.object.swid[l_row] then
				if dw_2.object.sheet[l_row] = "S" then
					ls_c_wid2 = RightA("0000" + string(integer(truncate(dw_2.object.length[l_row],0))), 4)
				else
					ls_c_wid2 = LeftA(string(integer( &
					mod(dw_2.object.swid[l_row], truncate(dw_2.object.swid[l_row],0))* 10)) + "0000", 4)
				end if
			else
				if dw_2.object.sheet[l_row] = "S" then
					ls_c_wid2 = RightA("0000" + string(integer(truncate(dw_2.object.length[l_row],0))), 4)
			...
			ls_c_thin = trim(string(dw_2.object.thin[l_row] * 100 ,"###"))
			ls_c_qa   = RightA("000" + ls_c_thin, 3) + "X" +ls_c_wid + "X" + ls_c_wid2 + "X00000"
			
			dw_2.object.qa[l_row] = ls_c_qa
			
			if dw_2.object.sheet[l_row] = "S" then
				if MidA(dw_2.object.item_no[l_row], 3,1 ) = "A" then
					ld_yrate = id_arate
				else
					ld_yrate = id_srate
				end if // sheet 단중 = 두께 * 폭 * 길이 * 요율(스틸 = 8.12, 알미늄 = 2.72)
				
				if dw3.rowcount() > 0 and dw3.object.wid[1] > 0 and dw_2.object.unit_qty[l_row] > 0 then
					dw_2.object.cnt[l_row] = truncate(dw_2.object.swid[l_row] / dw3.object.wid[1] &
													 * ld_yrate / dw_2.object.unit_qty[l_row] / 1000000,0)
				end if
			end if			
