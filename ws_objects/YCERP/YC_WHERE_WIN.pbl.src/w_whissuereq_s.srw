...
event clicked;// 조회
string ls_area, ls_acceptuser, ls_defno, ls_locno

dw_1.reset()

setnull(ls_area)
SELECT issuereq_area INTO :ls_area FROM login WHERE user_id = :gs_userid;

if gs_userid = '1999010s' then ls_area = '%'
...
il_row = dw_3.retrieve( '%', date(em_1.text), 'Y', '%', '%' ) // 'Y' : 확정된것만 조
