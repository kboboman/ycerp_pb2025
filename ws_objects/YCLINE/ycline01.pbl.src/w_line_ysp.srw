$PBExportHeader$w_line_ysp.srw
$PBExportComments$생산라인 라벨출력(YSP)
forward
global type w_line_ysp from window
end type
type em_prt from editmask within w_line_ysp
end type
type st_7 from statictext within w_line_ysp
end type
type cbx_unit from checkbox within w_line_ysp
end type
type cbx_1 from checkbox within w_line_ysp
end type
type em_top from editmask within w_line_ysp
end type
type em_left from editmask within w_line_ysp
end type
type st_6 from statictext within w_line_ysp
end type
type st_5 from statictext within w_line_ysp
end type
type st_4 from statictext within w_line_ysp
end type
type st_key from statictext within w_line_ysp
end type
type cb_3 from commandbutton within w_line_ysp
end type
type cb_2 from commandbutton within w_line_ysp
end type
type cb_1 from commandbutton within w_line_ysp
end type
type st_2 from statictext within w_line_ysp
end type
type dw_2 from datawindow within w_line_ysp
end type
type st_1 from statictext within w_line_ysp
end type
type dw_1 from datawindow within w_line_ysp
end type
type st_3 from statictext within w_line_ysp
end type
end forward
...
VALUES (:ldt_sfc_dt,:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,:ldb_work_qty,0,:gs_loc,'RP',0,:ls_wc_no,'현장작업일보[YSP]', :gs_userid);
if sqlca.sqlcode < 0 then
	ls_sqlerrtext = sqlca.sqlerrtext
	rollback;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
date   ldt_date
string ls_wcno, ls_custno, ls_scene, ls_packno, ls_packno1, ls_mpsorder, ls_color, ls_manager
string ls_itemno, ls_itemnm, ls_uom, ls_qa, ls_qa13, ls_qa5, ls_rem, ls_prt
string ls_sqlerrtext, ls_left, ls_top
...
SELECT top 1 convert(char(10), getdate(), 112) INTO :ls_date FROM login;
...
if ls_manager <> "" then ls_manager = ls_manager + " (인)"
...
if dw_1.enabled = false then
	...
	ls_qa     = dw_1.object.qa[1]
     ls_qa13   = LeftA(dw_1.object.qa[1],13)		
	...
	VALUES (:ls_packno1, :ls_wcno, :ls_custno, :ls_scene, :ldt_date, :ls_itemno, :ls_qa13,
	...
	if SQLCA.SqlCode <> 0 then
		...
		for li_int = 1 to li_rowcnt
         ...
         li_seq      = dw_1.object.order_seq[li_int]
			...
			ls_qa5      = RightA(dw_1.object.qa[li_int],5)
			...
			VALUES ( :ls_packno1, :li_int, :ls_qa5, :ls_rem, :ld_qty, :ls_mpsorder, :li_seq );
			if SQLCA.SqlCode <> 0 then
				ls_sqlerrtext = sqlca.sqlerrtext
				rollback;
...
dw_2.object.lb_manager[1]    = ""	//  ls_manager
dw_2.object.lb_color[1]      = ""   //  ls_color

dw_2.object.lb_packno1[1]    = "*" + ls_packno1 + "*"
dw_2.object.lb_packno2[1]    = "*" + ls_packno1 + "*"
dw_2.object.lb_packno3[1]    = ls_packno1

//dw_2.object.ks[1]            = ls_ks
//dw_2.object.ks1[1]           = ls_ks1
//dw_2.object.ks2[1]           = ls_ks2
//dw_2.object.ks3[1]           = ls_ks3
//dw_2.object.ksd1[1]          = ls_ksd
...
//	ls_qa13     = left(ls_qa,13) /* jowonsuk 2020.05.20까진 길이미반영 */
	ls_qa13     = LeftA(ls_qa,19) /* jowonsuk 2020.05.21       길이를 반영 */
		
	ls_qa13     = GF_ReplaceAll(ls_qa13,"X", " X ")
	ls_qa5      = RightA(ls_qa,5)
	ld_qty      = dw_1.object.unit_qty[li_int]
	ls_rem      = dw_1.object.sale_no[li_int]

	dw_2.setitem( 1, 'lb_qa'     + string(li_int), ls_qa13)		// 규격
	dw_2.setitem( 1, 'lb_qty'    + string(li_int), ld_qty )		// 수량
	dw_2.setitem( 1, 'lb_rem'    + string(li_int), ls_rem )		// 비고
next

li_page_qty = dw_1.GetItemNumber(1, 'page_qty')			// 출력장수					
...
Do While ll_selrow <> 0
	...
	if ls_cust  <> w_line.dw_1.object.cust_no[ll_selrow] OR &
		ls_scene <> w_line.dw_1.object.scene_code[ll_selrow] then
		MessageBox("선택오류(YSP)", "선택한 항목중에 현장 또는 거래처가 다른것이 있습니다.~n~n" &
					+ "같은 현장 또는 거래처만 선택하십시요.") 
...
event clicked;//
if this.checked = true then
	dw_2.dataobject = "d_label_a4_ysp"
	dw_2.SetTransObject(sqlca)
else
	dw_2.dataobject = "d_label_165x100_ysp"
	dw_2.SetTransObject(sqlca)
end if
...
type dw_2 from datawindow within w_line_ysp
string tag = "d_label_a4_ysp"
integer x = 421
integer y = 880
integer width = 3282
integer height = 976
string title = "d_label_165x100_ysp"
string dataobject = "d_label_165x100_ysp"
boolean border = false
