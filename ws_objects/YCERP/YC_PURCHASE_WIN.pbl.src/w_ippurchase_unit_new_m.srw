$PBExportHeader$w_ippurchase_unit_new_m.srw
$PBExportComments$구매입고시계열관리(2020/02/24, 조원석)
forward
global type w_ippurchase_unit_new_m from w_inheritance
end type
type st_3 from statictext within w_ippurchase_unit_new_m
end type
type ddlb_fld from dropdownlistbox within w_ippurchase_unit_new_m
end type
type ddlb_dwtitles from dropdownlistbox within w_ippurchase_unit_new_m
end type
type st_4 from statictext within w_ippurchase_unit_new_m
end type
type sle_value from singlelineedit within w_ippurchase_unit_new_m
end type
type cb_1 from commandbutton within w_ippurchase_unit_new_m
end type
type cb_2 from commandbutton within w_ippurchase_unit_new_m
end type
type cb_3 from commandbutton within w_ippurchase_unit_new_m
end type
type cb_5 from commandbutton within w_ippurchase_unit_new_m
end type
type ddlb_op from dropdownlistbox within w_ippurchase_unit_new_m
end type
type tab_1 from tab within w_ippurchase_unit_new_m
end type
...
type st_2 from statictext within w_ippurchase_unit_new_m
end type
type st_5 from statictext within w_ippurchase_unit_new_m
end type
type em_3 from editmask within w_ippurchase_unit_new_m
end type
type em_2 from editmask within w_ippurchase_unit_new_m
end type
type rb_1 from radiobutton within w_ippurchase_unit_new_m
end type
type rb_2 from radiobutton within w_ippurchase_unit_new_m
end type
type dw_7 from datawindow within w_ippurchase_unit_new_m
end type
end forward
...
global w_ippurchase_unit_new_m w_ippurchase_unit_new_m

type variables
st_print  i_print
integer  i_tabpage
string    is_dw, is_purchase_code, is_purchase_name, is_yyyy, is_yyyymm
end variables
...
public function integer wf_graph_create1 ();string ls_purchase_name, ls_purchase_name1, ls_purchase_name2,  ls_yyyymm_name, ls_fr_date, ls_to_date, ls_yyyymm
decimal l_unit_amt1, l_unit_amt2, l_unit_amt3, l_unit_amt4, l_unit_amt5, l_null
	...
	myoleobject.application.workbooks(1).Worksheets(1).Activate
	myoleobject.application.workbooks(1).Worksheets(1).range("A2:F" + string(l_row)).select
	myoleobject.application.workbooks(1).Worksheets(1).paste
	 
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.setSourcedata(myoleobject.application.workbooks(1).WorkSheets(1).Range("A2:F" + string(l_row) ))
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.SeriesCollection(1).name = '열연내수가'
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.SeriesCollection(2).name = '열연수출가'	
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.SeriesCollection(3).name = '냉연수출가'		
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.SeriesCollection(4).name = 'GI포두'		
	myoleobject.application.workbooks(1).Worksheets("Sheet3").ChartObjects(1).Chart.SeriesCollection(5).name = '철근수출가'			
...
on w_ippurchase_unit_new_m.create
int iCurrent
call super::create
...
this.Control[iCurrent+18]=this.dw_7
end on

on w_ippurchase_unit_new_m.destroy
call super::destroy
destroy(this.st_3)
...
where use_yn = 'Y'		
and unit_amt is not null
and purchase_code in ('00001', '00002', '00003', '00004', '00006');
...
type pb_save from w_inheritance`pb_save within w_ippurchase_unit_new_m
integer x = 4233
integer y = 60
end type
...
type dw_1 from w_inheritance`dw_1 within w_ippurchase_unit_new_m
...
type st_title from w_inheritance`st_title within w_ippurchase_unit_new_m
integer x = 91
integer y = 40
integer width = 1358
string text = "구매자재 단가 현황(new)"
end type

type st_tips from w_inheritance`st_tips within w_ippurchase_unit_new_m
end type

type pb_compute from w_inheritance`pb_compute within w_ippurchase_unit_new_m
boolean visible = false
integer x = 4709
integer y = 212
...
type pb_print_part from w_inheritance`pb_print_part within w_ippurchase_unit_new_m
boolean visible = false
integer x = 3406
end type

type pb_close from w_inheritance`pb_close within w_ippurchase_unit_new_m
integer x = 4425
integer y = 60
...
type pb_print from w_inheritance`pb_print within w_ippurchase_unit_new_m
integer x = 4041
integer y = 60
...
type pb_cancel from w_inheritance`pb_cancel within w_ippurchase_unit_new_m
boolean visible = false
integer x = 4471
integer y = 240
...
type pb_delete from w_inheritance`pb_delete within w_ippurchase_unit_new_m
integer x = 3849
integer y = 60
...
type pb_insert from w_inheritance`pb_insert within w_ippurchase_unit_new_m
integer x = 3657
integer y = 60
...
type pb_retrieve from w_inheritance`pb_retrieve within w_ippurchase_unit_new_m
integer x = 3465
integer y = 60
...
if i_tabpage = 1 then
	...
	if rb_1.checked then
		...
		wf_graph_create1()
//		tab_1.tabpage_1.p_photo.width  = 8635
//		tab_1.tabpage_1.p_photo.height = 3396			
	elseif rb_2.checked then
		wf_graph_create2()			
//		tab_1.tabpage_1.p_photo.width  = 8645
//		tab_1.tabpage_1.p_photo.height = 3392		
	end if
...
type gb_3 from w_inheritance`gb_3 within w_ippurchase_unit_new_m
boolean visible = false
integer x = 2565
integer y = 8
integer width = 823
...
type gb_2 from w_inheritance`gb_2 within w_ippurchase_unit_new_m
integer y = 244
integer width = 3913
integer height = 168
integer taborder = 90
...
type gb_1 from w_inheritance`gb_1 within w_ippurchase_unit_new_m
integer x = 3415
integer y = 16
integer width = 1248
integer height = 212
...
type dw_2 from w_inheritance`dw_2 within w_ippurchase_unit_new_m
boolean visible = false
integer x = 1353
integer y = 72
integer width = 119
...
dw_7.dataobject = 'd_purhase_unit_amt_r2'
dw_7.settrans(sqlca)
end event
...
event clicked;string  ls_fr_yyyymm, ls_to_yyyymm
	...
	from dbo.purchase_unit_amt
	where use_yn = 'Y'		
	and kor_amt is not null
	and purchase_code in ('00004', '00005');

em_2.text = LeftA(ls_fr_yyyymm,4) + '/'	 + RightA(ls_fr_yyyymm,2)
em_3.text = LeftA(ls_to_yyyymm,4) + '/'	 + RightA(ls_to_yyyymm,2)

dw_7.dataobject = 'd_purhase_unit_amt_r3'
dw_7.settrans(sqlca)
end event
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_purhase_unit_amt_r2"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
