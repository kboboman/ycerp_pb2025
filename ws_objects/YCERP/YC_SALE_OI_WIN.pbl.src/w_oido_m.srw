...
type pb_1 from picturebutton within w_oido_m
end type
type cbx_5 from checkbox within w_oido_m
end type
type cbx_6 from checkbox within w_oido_m
end type
type dw_4 from datawindow within w_oido_m
end type
type cbx_issue from checkbox within w_oido_m
end type
type sle_prt from singlelineedit within w_oido_m
end type
type dw_6 from datawindow within w_oido_m
end type
type cbx_ship from checkbox within w_oido_m
end type
type st_left from statictext within w_oido_m
end type
type em_left from editmask within w_oido_m
end type
type st_9 from statictext within w_oido_m
end type
...
st_print i_print
datawindowchild idwc_scene, idwc_cust, idwc_car, idwc_lot, idwc_locno, idwc_car_no
...
// 주위 : 내용 수정시 w_oido_new_m 같이 수정해야함.
//
*****************************************************/
// 저장
DataWindowChild dwc1, ldwc_driver

string   ls_chk,  ls_do_no, ls_do_no1,  ls_sale_flag, ls_do_date,   ls_seq,     ls_null, ls_scene_code, ls_car_no
...
int      li_int,  li_seq,   ll_row,  ll_int,       li_count,  li_saleseq, l_row, l_id, ii
...
string   ls_prt, ls_source, ls_confirm_flag
...
for ii = 1 to dw_2.rowcount()
	ls_source	= 'N'
	ls_order_no	= ''
	ls_source	= dw_2.getitemstring(ii, 'source')
	ls_order_no = dw_2.getitemstring(ii, 'order_no')
	if ls_source = 'N' then continue
	...
	if gf_null_chk(ls_confirm_flag) then
		ls_confirm_flag	= 'N' 
	end if
	
	if ls_confirm_flag = 'N' then 
		messagebox("확인","수주번호 : " + ls_order_no +"는 미 확정건입니다.~r~n수주 담당자에 문의하세요")
		RETURN
	end if
next	
...
if isnull(ls_timearrival) OR ls_timearrival = "" then // 도착예정
   ls_driverdw1 = dw_1.object.driver[dw_1.getrow()]
	...
	if isnull(ls_driver) or ls_driver = "" then // 차량구분
		MessageBox("저장오류","차량구분은 반드시 선택하시기 바랍니다.",stopsign!)
		RETURN
	end if	
...
/*2020-10-08 jowonsuk 오명준 사원 저장소와 기본출고지 차이 오류 지속적 발생으로 
  오명준사원 작업시 출고지, 저장소 차이나면 메세지 출력
*/
	...
	if dw_2.object.source[ll_cnt] = "Y"  then		
		if (Not isnull(dw_2.object.loc_no[ll_cnt])) AND trim(dw_2.object.loc_no[ll_cnt]) <> "" then	// 저장소
			...
			/*2020-10-08 jowonsuk 오명준 사원 저장소와 기본출고지 차이 오류 지속적 발생으로 
			  오명준사원 작업시 출고지, 저장소 차이나면 메세지 출력
			*/		
			if ls_loc <> dw_2.object.loc_no[ll_cnt] then
				ll_check = ll_check + 1
			end if
...
/*2020-10-08 jowonsuk 오명준 사원 저장소와 기본출고지 차이 오류 지속적 발생으로 
  오명준사원 작업시 출고지, 저장소 차이나면 메세지 출력
*/
IF gs_userid = 'woper369' then
	if ll_check > 0 then
		if MessageBox("확인","기본출고지와 다른 저장소가 있습니다. 계속 진행하시겠습니까?",question!,okcancel!,1) = 2 then RETURN	
	end if
end if	
...
//	set use_yn = 'Y',               /* N->Y N은 임시 -> Y로 되어야 사용한것*/
//		  do_no  = :ls_do_no1,
//		  oi_no   = :ll_row
...
if lb_chk then
	...
	SELECT top 1 b.cust_no
		  ...
		  , isnull(g.saler_phone,'XXX-XXXX-XXXX')
	...
	choose case ls_callback	
		case "0226404628", "0226901999",	"0226489600",	"0226404675",	"0226481998" &
			  ...
			  // 시화공장, 화성공장, 동부영업소(성남), 중부호남영업지점
				...
				//		case 'grldh1120', '1999010s'
//							if ll_gubun = 1 then
//								Messagebox("확인","알림톡 발송 완료")
//							else
//								Messagebox("확인","SMS 발송 완료")
//							end if
//				//	end choose						
...
string ls_car,  ls_curr_code, ls_area, ls_car_info, ls_scene_code, ls_date, ls_car_gbn, ls_car_no, ls_company
...
dec    ld_itemloc_qoh, lr_exp_rate, ld_janqoh, l_ship_amount
...
//			this.object.rem[1] = ls_temp + left(idwc_car.getitemstring(idwc_car.getrow(),"driver") + " " &
//									 + idwc_car.getitemstring(idwc_car.getrow(),"name") + " " &
//									 + idwc_car.getitemstring(idwc_car.getrow(),"hp"), 43)
	...
	//		end if
			...
			/*2019-11-21 jowonsuk 택배 자동으로 내용 등록 되도록 */
			if trim(data) = '경동-화곡' then
					this.object.rem[1] = '경동-화곡 경동택배'
					this.object.do_dr_hp[1] = '02-2665-7336'								
					this.object.sms_yn[1] = 'N'	
					this.object.do_car_gbn[1] = '01'
					this.object.time_arrival[1] = gf_today()
			end if			
...
string   ls_custno,   ls_scene_code, ls_where, ls_sql, ls_sale_flag, ls_curr_code, ls_null, ls_select_car
string   ls_custname, ls_addr, ls_scenename, ls_addr2, ls_order_no, ls_shipto_addr, ls_telno, ls_telnocap, ls_pre_sale_yn
string   ls_car_no, ls_car_gbn, ls_driver_name, ls_driver_tel, ls_cargo_name, ls_sqlerrtext

dec      ld_curr_rate, ld_ship_amount
datetime ldt_dodate
...
//	/*2019.03.27 jowonsuk 배차정보 가지고 오기 */
//	select car_no, car_gbn, driver_name, driver_tel 
//	   into :ls_car_no, :ls_car_gbn, :ls_driver_name, :ls_driver_tel
//	from car_carriage
//	where cardate = convert(char(8), getdate(), 112)
//		and scene_code = :ls_scene_code
//		and car_status = '02';
	/*2019.03.27 jowonsuk 배차정보 가지고 오기 */
//	select car_carriage.car_no, car_carriage.car_gbn, car_carriage.driver_name, car_carriage.driver_tel, docar.cargo_name, car_carriage.st_car_amt
//	   into :ls_car_no, :ls_car_gbn, :ls_driver_name, :ls_driver_tel, :ls_cargo_name, :ld_ship_amount
//	from car_carriage with(nolock) left outer join docar with(nolock) on car_carriage.car_no = docar.driver
//	where car_carriage.cardate = convert(char(8), getdate(), 112)
//		and car_carriage.scene_code = :ls_scene_code
//		and car_carriage.car_status = '02';
...
/*2019.04.12 jowonsuk 배차정보 가지고 오기 */
	...
	OPEN car_cursor;
		...
		FETCH car_cursor INTO :ls_car_no;

		Do While sqlca.sqlcode = 0
	
			ls_select_car = ls_select_car + "'" + ls_car_no + "'" + ', '
	...
	ls_select_car = MidA(ls_select_car, 1, LenA(ls_select_car) - 2)
	
	idwc_car_no.setfilter("driver in (" + ls_select_car + ")")
	idwc_car_no.filter()
	...
	if ls_ship_case = 'T' then	
		...
		elseif gs_area = 'H0001' then /*화곡경우*/	
			dw_1.object.driver[1] = '경동-화곡'
			dw_1.object.rem[1] = '경동-화곡 경동택배'
			dw_1.object.do_dr_hp[1] = '02-2665-7336'
			dw_1.object.ship_flag[1] = ls_ship_flag
			dw_1.object.sms_yn[1] = 'N'	
			dw_1.object.do_car_gbn[1] = '01'
			dw_1.object.time_arrival[1] = gf_today()					
		elseif gs_area = 'Z0001' then /*대전경우*/	
		else			
		end if
	end if	
		

//	dw_1.object.driver[1] =  ls_car_no
//	dw_1.object.do_dr_hp[1] =  ls_driver_tel
//	dw_1.object.do_car_gbn[1] = ls_car_gbn
//	dw_1.object.rem[1] = ls_driver_name + ' ' +  ls_car_no + ' ' + ls_cargo_name
//	dw_1.object.ship_amount[1] = ld_ship_amount
	/**/
