...
ls_chk = "N"
if MessageBox("확인", "회손된 라벨을 재출력 합니다.", Question!, YesNo!, 1) = 1 then
	wf_barcode_print()
...
//
//dw_1.accepttext()
//lt_Time          = time(mid(string(today(),'yyyy/mm/dd hh:mm:ss:fff'),11))
...
//	case 'M'
//		ldb_hebe = Double(Long(Mid(ls_assy_qa, 15, 5)) / 1000.0) * ldb_hebe * ldb_work_qty
//	case else
...
//VALUES (:ldt_sfc_dt,:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,:ldb_work_qty,0,:gs_loc,'RP',0,:ls_wc_no,"작업일보[YSP]");
//if sqlca.sqlcode < 0 then
//	ls_sqlerrtext = sqlca.sqlerrtext
//	rollback;
...
date     ldt_date
string   ls_saleno, ls_custscene, ls_wcno, ls_custno, ls_custname, ls_scene, ls_scenedesc
string   ls_packno, ls_packno1, ls_mpsorder, ls_color, ls_manager
string   ls_itemno, ls_itemnm, ls_uom, ls_qa, ls_qa13, ls_qa5, ls_rem, ls_routnext, ls_prt
...
ls_packno1  = dw_1.object.pack_no[1]		// 패킹번호
...
SetProfileString("YCERP.INI","Barcode", "LEFT", ls_left)
SetProfileString("YCERP.INI","Barcode", "TOP",  ls_top)
SetProfileString("YCERP.INI","Barcode", "PRINT",ls_prt)
...
for ll_row = 1 to ll_dw1cnt
   ...
   // 수주번호가 있으면 주문품(O), 재고품(S)
	...
	ls_scenedesc = dw_1.object.scene_desc[ll_row]
	// ERP품명, BOX품명 출력구분
	ls_itemnm    = dw_1.Object.item_name[ll_row]		// ERP품명
	
	ls_qa        = dw_1.object.qa[ll_row]					// 규격
	...
	dw_2.setitem( 1, "lb_item_name", ls_itemnm)		// 품명
	dw_2.setitem( 1, "lb_color",     ls_color )		// 색상
	if ll_row < 7 then
...
dw_2.object.lb_cust_name[1]    = ls_custname						// 거래처
dw_2.object.lb_scene_name[1]   = ls_scenedesc					// 현장명	
//dw_2.object.lb_totcnt[1]     = ll_dw1cnt						// 건수

dw_2.object.lb_makedate[1]     = string(ldt_getdate,"yyyy/mm/dd")						// 제조일자	
...
em_left.text =ProfileString("YCERP.INI","Barcode","LEFT", " ")
em_top.text  =ProfileString("YCERP.INI","Barcode","TOP",  " ")
em_prt.text  =ProfileString("YCERP.INI","Barcode","PRINT"," ")
...
ll_dw1row = w_mpwork_m1.tab_1.tabpage_2.dw_4.GetRow()
ls_packno = w_mpwork_m1.tab_1.tabpage_2.dw_4.object.pack_no[ll_dw1row]

if isnull(ll_dw1row) OR ll_dw1row < 1 OR isnull(ls_packno) OR ls_packno = "" then
	MessageBox("확인","재출력 대상의 패킹번호가 존재하지 않습니다.")
	close( this)
end if

// 패킹내역 조회
ll_dw1cnt = dw_1.Retrieve( ls_packno )
...
long backcolor = 8421376
string text = "F8:출력  F9:재출력 ESC:취소"
alignment alignment = center!
boolean focusrectangle = false
end type
