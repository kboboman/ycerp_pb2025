...
type st_3 from statictext within w_cdsaler_esero_m
end type
type st_4 from statictext within w_cdsaler_esero_m
end type
...
type st_5 from statictext within w_cdsaler_esero_m
end type
...
type cb_4 from commandbutton within w_cdsaler_esero_m
end type
type cb_5 from commandbutton within w_cdsaler_esero_m
end type
...
if gf_permission('전자세금계산서(관리담당)', gs_userid) = 'Y' then
else
...
string ls_salerid, ls_childnum

dw_1.accepttext()
if dw_1.rowcount() < 1 then return

for ll_count = 1 to dw_1.rowcount()
	ls_salerid = trim(dw_1.object.saler_id[ll_count])
	if isnull(ls_salerid) Or ls_salerid = "" then
		messagebox("입력오류", string(ll_count) + " 번째 담당ID 항목을 확인 하십시오")
		dw_1.setcolumn("saler_id")
		dw_1.setfocus()
		dw_1.scrolltorow(ll_count)
		return
	end if
	
	ls_childnum = trim(dw_1.object.child_number[ll_count])
	if isnumber(ls_childnum) then
	else
		messagebox("입력오류", string(ll_count) + " 번째 종사업장 항목을 확인 하십시오")
		dw_1.setcolumn("child_number")
		dw_1.setfocus()
		dw_1.scrolltorow(ll_count)
		return
	end if
	...
	CHOOSE CASE trim(dw_1.object.saler_bizno[ll_count])
		...
		CASE "140-81-48198"	// 시흥
			IF dw_1.object.area[ll_count] = "DD" or dw_1.object.area[ll_count] = "ZD" THEN
			ELSE
				messagebox("입력오류", string(ll_count) + " 번째 수주항목을 확인 하십시오")
				dw_1.setcolumn("area")
				dw_1.setfocus()
