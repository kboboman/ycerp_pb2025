...
type st_vertical from statictext within w_mproutingchk_r
end type
type st_horizontal from statictext within w_mproutingchk_r
end type
...
this.Control[iCurrent+17]=this.cb_routchange
this.Control[iCurrent+18]=this.cbx_all
this.Control[iCurrent+19]=this.dw_5
this.Control[iCurrent+20]=this.sle_qa
this.Control[iCurrent+21]=this.cbx_not
this.Control[iCurrent+22]=this.cbx_select
this.Control[iCurrent+23]=this.ddlb_qa
end on
...
Long		ll_thisWidth, ll_thisHeight, ll_Width, ll_Height

ll_thisWidth  = this.Width
ll_thisHeight = this.Height

ll_Width      = this.WorkSpaceWidth()
ll_Height     = this.WorkspaceHeight()

gb_2.width  = newwidth  - 91
gb_2.height = newheight - 476

dw_1.width  = newwidth  - 174
dw_1.height = newheight - 1548

dw_3.y      = dw_1.y + dw_1.height
//dw_3.width  = dw_1.width  * 0.3
//dw_3.height = this.height - 2128

dw_4.x      = dw_3.x + dw_3.width
dw_4.y      = dw_3.y + 88
dw_4.width  = dw_1.width  - dw_3.width
dw_4.height = dw_3.height - 88
...
//ll_cnt    = this.object.routcase[row]
...
event pb_retrieve::clicked;string ls_start, ls_end, ls_category
long   ll_row
...
event mouseup;//Check for move in progess
	...
	if MessageBox("확인", "선택한 " + string(ll_cnt,"#,##0") + "개의 품목+규격들의 1차 공정을 '" + &
		...
		for ll_row = 1 to dw_4.rowcount()
			if dw_4.object.chk[ll_row] = "Y" then
				...
				UPDATE rout SET wc_no = :ls_wcno_new
