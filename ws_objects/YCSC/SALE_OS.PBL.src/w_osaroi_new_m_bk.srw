$PBExportHeader$w_osaroi_new_m_bk.srw
$PBExportComments$판매매출관리new(2008/01/17, 정갑용)
forward
global type w_osaroi_new_m_bk from w_inheritance
end type
type gb_7 from groupbox within w_osaroi_new_m_bk
end type
type gb_6 from groupbox within w_osaroi_new_m_bk
end type
type gb_5 from groupbox within w_osaroi_new_m_bk
end type
type dw_4 from datawindow within w_osaroi_new_m_bk
end type
type rb_1 from radiobutton within w_osaroi_new_m_bk
end type
type rb_2 from radiobutton within w_osaroi_new_m_bk
end type
type em_1 from editmask within w_osaroi_new_m_bk
end type
type em_2 from editmask within w_osaroi_new_m_bk
end type
type st_1 from statictext within w_osaroi_new_m_bk
end type
type dw_6 from datawindow within w_osaroi_new_m_bk
end type
type ddlb_1 from dropdownlistbox within w_osaroi_new_m_bk
end type
type dw_7 from datawindow within w_osaroi_new_m_bk
end type
type dw_9 from datawindow within w_osaroi_new_m_bk
end type
type cbx_1 from checkbox within w_osaroi_new_m_bk
end type
type dw_5 from datawindow within w_osaroi_new_m_bk
end type
type cbx_2 from checkbox within w_osaroi_new_m_bk
end type
type dw_3 from datawindow within w_osaroi_new_m_bk
end type
type cbx_3 from checkbox within w_osaroi_new_m_bk
end type
type sle_1 from singlelineedit within w_osaroi_new_m_bk
end type
type rb_3 from radiobutton within w_osaroi_new_m_bk
end type
type rb_4 from radiobutton within w_osaroi_new_m_bk
end type
type gb_4 from gb_3 within w_osaroi_new_m_bk
end type
end forward
...
global w_osaroi_new_m_bk w_osaroi_new_m_bk

type variables
st_print i_print
datawindowchild idwc_cust
string is_sale_flag, is_round = 'N', is_dw6sql, is_dw7sql, is_dw7desc
int ii_an
end variables
...
public subroutine wf_ret ();string ls_cust, ls_scene, ls_where, ls_sql6, ls_sql7
...
//	ls_sql4 = "datawindow.table.select = '" + is_dw4sql + "'"
	ls_sql6 = "datawindow.table.select = '" + is_dw6sql + "'"
	ls_sql7 = "datawindow.table.select = '" + is_dw7sql + "'"
else
	ls_cust = dw_1.object.cust_no[1]
	ll_count = 0
	ls_scene = ''
	FOR ll_cnt = 1 to dw_3.rowcount()
		if dw_3.isselected(ll_cnt) then
			ll_count ++
			ls_scene = ls_scene + "'" + trim(dw_3.object.spot_code[ll_cnt]) + "',"
		end if
	next

	if ll_count < 1 then // 미선택, 기존 d_osaroi_m1, d_osaroi_h1, d_osaroi_s
		ls_scene = "'" + trim(dw_3.object.spot_code[dw_3.getrow()]) + "'"
	ELSE						// 선택(1~N), 신규 d_osaroi_new_m1, d_osaroi_new_h1, d_osaroi_new_s
		ls_scene = trim(ls_scene)
		li_int = LenA(ls_scene)
		ls_scene = LeftA(ls_scene, li_int - 1 )
	END IF

	ls_where = " and do.spot_code IN (" + ls_scene + ")" 
//	ls_sql4 = "datawindow.table.select = '" + is_dw4sql + ls_where + "'"
	ls_sql6 = "datawindow.table.select = '" + is_dw6sql + ls_where + "'"
	ls_sql7 = "datawindow.table.select = '" + is_dw7sql + ls_where + "'"
end if
...
this.Control[iCurrent+21]=this.rb_4
this.Control[iCurrent+22]=this.gb_4
end on

on w_osaroi_new_m_bk.destroy
call super::destroy
destroy(this.gb_7)
destroy(this.gb_6)
destroy(this.gb_5)
...
is_dw7sql = dw_7.describe("datawindow.table.select")
...
type pb_save from w_inheritance`pb_save within w_osaroi_new_m_bk
...
type dw_1 from w_inheritance`dw_1 within w_osaroi_new_m_bk
integer x = 59
integer y = 456
integer width = 859
integer height = 92
...
type dw_2 from w_inheritance`dw_2 within w_osaroi_new_m_bk
integer x = 992
integer y = 248
integer width = 2583
integer height = 488
...
type st_title from w_inheritance`st_title within w_osaroi_new_m_bk
integer x = 37
integer y = 44
integer width = 1120
string text = "판매매출관리"
...
type st_tips from w_inheritance`st_tips within w_osaroi_new_m_bk
end type

type pb_compute from w_inheritance`pb_compute within w_osaroi_new_m_bk
integer x = 2994
...
type pb_print_part from w_inheritance`pb_print_part within w_osaroi_new_m_bk
integer x = 1783
integer y = 44
integer width = 599
...
type pb_close from w_inheritance`pb_close within w_osaroi_new_m_bk
integer x = 3378
...
type pb_print from w_inheritance`pb_print within w_osaroi_new_m_bk
...
type pb_cancel from w_inheritance`pb_cancel within w_osaroi_new_m_bk
boolean visible = false
integer x = 2427
integer taborder = 240
end type

type pb_delete from w_inheritance`pb_delete within w_osaroi_new_m_bk
boolean visible = false
integer x = 2738
integer taborder = 250
end type

type pb_insert from w_inheritance`pb_insert within w_osaroi_new_m_bk
integer x = 2802
...
//mle_1.text = dw_4.describe("datawindow.table.select")
messagebox('0','1')
...
type pb_retrieve from w_inheritance`pb_retrieve within w_osaroi_new_m_bk
boolean visible = false
integer x = 1851
integer taborder = 260
end type

type gb_3 from w_inheritance`gb_3 within w_osaroi_new_m_bk
integer x = 969
integer y = 212
integer width = 2629
integer height = 536
...
type gb_2 from w_inheritance`gb_2 within w_osaroi_new_m_bk
integer x = 27
integer y = 216
integer width = 521
integer height = 152
...
type gb_1 from w_inheritance`gb_1 within w_osaroi_new_m_bk
integer x = 2779
integer width = 809
integer taborder = 0
end type
...
string title = "d_osaroi_new_m1 미사용가능"
string dataobject = "d_osaroi_new_m1"
...
string dataobject = "d_osaroi_new_h1"
boolean livescroll = true
end type
