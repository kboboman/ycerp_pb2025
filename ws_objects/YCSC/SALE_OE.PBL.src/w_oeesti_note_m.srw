...
DECLARE c_item CURSOR FOR  
  SELECT bigo FROM codemst 
   WHERE type = 'SMSWORD' AND use_yn = 'Y' AND item_nm <> '주소' ORDER BY sort;
OPEN c_item;
FETCH c_item INTO :ls_value;
Do While sqlca.sqlcode = 0
	ddlb_2.AddItem( ls_value )
	FETCH c_item INTO :ls_value;
Loop
CLOSE c_item;
...
ls_estino  = istr_where.str1

SELECT esti_tax INTO :ld_estitax FROM estimate_head WHERE estimate_no = :ls_estino;
if ld_estitax > 0 then
	rb_1.checked = false
	rb_2.checked = true
	MessageBox("확인","부가세액이 존재합니다.")
	RETURN
end if

dw_1.object.estimate_no[1] = ls_estino
dw_1.object.seq_no[1] = 1
