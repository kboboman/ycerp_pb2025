...
for ll_row = 1 to dw_3.rowcount()
	//금월매출액, 연간매출액
	lr_yy = dw_3.object.data[ll_row, 2] //전월잔액
	for ll_column = 3 to ll_m
		lr_ym = lr_ym + dw_3.object.data[ll_row, ll_column]  // 연간매출
		if ll_column = ll_m then
			lr_mm = dw_3.object.data[ll_row, ll_column] // 금월매출
		end if
	next
	
	//금월수금액, 연간수금액
	for ll_column = 15 to ll_s
		lr_ys = lr_ys + dw_3.object.data[ll_row, ll_column]
		if ll_column = ll_s then
			lr_ms = dw_3.object.data[ll_row, ll_column]
		end if
	next
