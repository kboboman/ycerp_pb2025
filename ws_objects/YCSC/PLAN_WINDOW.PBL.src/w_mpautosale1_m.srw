...
for ll_row = 1 to arg_cnt
	 // itemjaego: 재고물품 테이블에 존재하는 ITEM 이면 무조건 생산계획을 생성하지 않는다.
	ls_itemno = dw_1.object.itemjaego_item_no[ll_row]
	
	if isnull(ls_itemno) or trim(ls_itemno) = "" then
	else
		// 재고 물품인 경우에는 saledet.sol_qty에 가용량을 넣지 않고
...
for ll_row = 1 to arg_cnt
   ...
   // lr_notqty = dw_1.object.notqty[ll_row]
	...
	// 생산계획여부(saledet_text) 'N' : 불가, 'X' : 가능, 가용량(sol_qty > 0, '부분', '생산')
	if dw_1.object.groupitem_source[ll_row] = 'B' or dw_1.object.saledet_text[ll_row] = 'X' then		
		ll_ins  = dw_4.insertrow(0)
		dw_4.object.msg1[ll_ins] = "오류  - [" + ls_item + "][" + ls_qa1 + "] 구매품은 생산계획을 생성하지 않습니다. " + STRING(now())
		continue
	end if
	...
	if ll_cnt < 1 then
		ll_ins  = dw_4.insertrow(0)
		dw_4.object.msg1[ll_ins] = "오류  - [" + ls_item + "][" + ls_qa1 + "] 물품의 공정이 등록되지[제품별 공정관리(w_cditemwc)] 않았습니다. " + STRING(now())
		rollback;
		continue
	else
	    for j =  1 to ll_cnt
          ...
          ls_assyqa    = trim(dw_3.object.assy_qa[j])		
		    ...
		    if sqlca.sqlcode = 100 or sqlca.sqlcode < 0 then
				ll_ins  = dw_4.insertrow(0)
				dw_4.object.msg1[ll_ins] = "오류  - [" + ls_assyitem + "] 물품구성이[제품별 공정관리(w_cditemwc)] 잘못되었습니다. " + STRING(now())
				rollback;
				ls_error = 'Y'
				exit
		    end if
          ...
          if MidA(ls_assyitem,1,1) = 'P' then
			 ...
			 if li_count = 0 then
			    ...
			    select count(*) into :li_count 
				   from itempattern 
				  where item_no = :ls_assyitem and qa1 = :ls_qa3;
			    if li_count = 0 then
...
//					 MESSAGEBOX("", string(sqlca.sqlerrtext))
					 ...
					 select count(*) into :ll_int3 from routtempnew
					  where operation = :li_operation
						 and wc_no  = :ls_wc   and sale_no  = :ls_order
  					    and seq_no = :li_seq  and division = 1;
				   if ll_int3 > 0 then
			          ...
			          if sqlca.sqlcode < 0 then
							  ll_ins  = dw_4.insertrow(0)
							  dw_4.object.msg1[ll_ins] = "오류5-1  - routtempnew DELETE 중 오류가 발생 하였습니다." + STRING(now())
		                 rollback;
				           ls_error = 'Y'
		             end if					 
			          ...
			          if sqlca.sqlcode < 0 then
							  ll_ins  = dw_4.insertrow(0)
							  dw_4.object.msg1[ll_ins] = "오류5-2  - routtempnew INSERT 중 오류가 발생 하였습니다." + STRING(now())
		                 rollback;
				           ls_error = 'Y'
		             end if					 
					else
...
//			        end if		
                    ...
                    // insert routtempnew #-3
		              ...
		              values (:ldt_duedate,1,:ls_wc,:li_operation,:ls_flag,:li_workper,:ls_order,:li_seq,
			           ...
			           if sqlca.sqlcode <> 0 then
		      	         rollback;
								ll_ins  = dw_4.insertrow(0)
								dw_4.object.msg1[ll_ins] = "오류9 - [" + ls_item + "][" + ls_qa + "] 에 대한 공정구성중 오류가 발생했습니다. " + STRING(now())
				   	      ls_error = 'Y'
				      	    exit
			           end if
               ...
               ////---------------- 생산예약량 설정하는 부분 -------------------------------------------						  
					case else
		             rollback;
							ll_ins  = dw_4.insertrow(0)
							dw_4.object.msg1[ll_ins] = "오류10 - [" + ls_item + "][" + ls_qa + "] 에 대한 공정구성중 오류가 발생했습니다. " + STRING(now())
				       ls_error = 'Y'
				       exit						
				 end choose
          end if
...
//dw_5.x  = 233
//dw_5.y  = 444
//dw_5.width  = this.width  - 485
//dw_5.height = this.height - 652	
dw_5.visible = false
...
dw_5.width  = this.width  - 910
dw_5.height = this.height - 1240
...
//		w_mprout_r.dw_1.setredraw(true)
	...
	case "item"		// 재고내역
		...
		if ll_idx > 0 then	w_mpjaegoc_r.dw_2.scrolltorow(ll_idx)
end choose
...
ll_row = dw_5.retrieve()
if ll_row > 0 then
	dw_5.visible = true
	MessageBox("품목코드 오류!", ls_message)
else
	dw_5.visible = false	
end if
