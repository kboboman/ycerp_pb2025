...
type gb_4 from groupbox within w_apoivc_m
end type
...
type st_11 from statictext within w_apoivc_m
end type
type st_4 from statictext within w_apoivc_m
end type
type rb_1 from radiobutton within w_apoivc_m
end type
type rb_2 from radiobutton within w_apoivc_m
end type
end forward
...
integer width = 3657
integer height = 2008
string title = "자재지급관리(w_apoivc_m)"
...
if gs_userid = '1999010s' then
	...
	messagebox("확인","이 프로그램을 사용하지 않습니다.")
end if
...
event pb_save::clicked;string ls_vc_no, ls_vc_no1, ls_oi_date, ls_paid_type, ls_seq, ls_supp_no, ls_vc_type, ls_month
...
//	messagebox("",ls_vc_no)
   ...
   if dw_2.object.paid_type[1] = 'B' then //어음일 경우
		...
		UPDATE supp SET bill_amount = bill_amount + :lr_amount
   ...
   ls_month = string(dw_2.object.oi_date[1], 'mm')
	...
	UPDATE supp SET balance_amt = balance_amt + :lr_amount
	 WHERE supp_no = :ls_supp_no;
	if sqlca.sqlcode <> 0 then
		rollback;
		messagebox("저장","저장중 오류가 발생하였습니다!",exclamation!)
...
elseif lr_amount <> dw_2.getitemdecimal(1, 'amount', Primary!, TRUE) then //지급액에 변동이 있을 경우
	...
	if dw_2.object.paid_type[1] = 'B' then //어음인 경우
		...
		UPDATE apbill SET bill_amount = bill_amount - :lr_amount
...
event pb_compute::clicked;string ls_supp_no
...
event pb_insert::clicked;string ls_supp_no
...
select balance_amt, bill_amount, buyer 
  into :ist_apoivc.balance_amt, :ist_apoivc.bill_amount, :ist_apoivc.saleman from supp
 where supp_no = :ls_supp_no;
if rb_1.checked = true then
	...
	dw_2.object.amount[1] = dw_3.object.amount[dw_3.getrow()] + dw_3.object.apoiinv_rate[dw_3.getrow()]
end if
...
long textcolor = 33554432
long backcolor = 67108864
string text = "매입자료에의한 추가"
boolean checked = true
end type

event clicked;pb_retrieve.enabled = true
dw_3.visible = true
...
event clicked;pb_retrieve.enabled = false
dw_3.visible = false
dw_3.reset()
end event
