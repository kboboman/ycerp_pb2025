...
ls_database = Upper(Trim(ProfileString("YCERP.INI","Database","DataBase"," ")) + "ERP SYSTEM")
st_5.text   = "유창 YC2010 " + ls_database
...
event clicked;//// SOURCE 통합 전 
//if FileExists("c:\Program Files\ycerpgl\ycerpgl.exe") = true then
//	RUN("c:\Program Files\ycerpgl\ycerpgl.exe " + gs_userid + " YCGW")
//end if
//
end event
...
SELECT COUNT(*) INTO :ll_cnt 
  FROM CODEMST
 WHERE TYPE   IN ('판매관리_입력','판매관리_조회', '판매관리_입력(부분)')
