...
SELECT user1_rank  = (case when a.user1_id = a.user2_id then '' else a.user1_rank end)
  ...
  FROM (
       SELECT user1_rank  = a.rank
            ...
            , user2_rank  = c.rank
            , user2_id    = (case when a.user_id = 'sjh4279' and b.team_manager = 'parkji' then 'sjh4279' else b.team_manager end)
            , user2_name  = (case when a.user_id = 'sjh4279' and b.team_manager = 'parkji' then '신준현'  else c.user_name    end)
            , user3_rank  = (case when a.team_code = '72000' then '실장' else '전무' end)
            , user3_id    = (case when a.team_code = '72000' then 'cwj'
                                  when a.team_code = '74000' then '19840901'
                                  else                            'chahc'
                             end)
            , user3_name  = (case when a.team_code = '72000' then '조우제'
...
if ll_cnt > 0 then
	...
	if lb_chk = true then
		for ll_upcheck = ll_cnt + 1 to 5
			ls_appdt = dw_1.getitemdate( 1, "dc_appdt" + string(ll_upcheck) )			
