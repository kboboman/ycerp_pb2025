$PBExportHeader$w_orsale_webcon.srw
$PBExportComments$Web 주문관리(접수)
forward
global type w_orsale_webcon from w_inheritance
end type
type st_1 from statictext within w_orsale_webcon
end type
type em_1 from editmask within w_orsale_webcon
end type
type em_2 from editmask within w_orsale_webcon
end type
type dw_3 from datawindow within w_orsale_webcon
end type
type dw_4 from datawindow within w_orsale_webcon
end type
type st_4 from statictext within w_orsale_webcon
end type
type st_5 from statictext within w_orsale_webcon
end type
type st_6 from statictext within w_orsale_webcon
end type
type st_8 from statictext within w_orsale_webcon
end type
type ddlb_fld from dropdownlistbox within w_orsale_webcon
end type
type st_9 from statictext within w_orsale_webcon
end type
type sle_value from singlelineedit within w_orsale_webcon
end type
type cb_filteron from commandbutton within w_orsale_webcon
end type
type cb_filteroff from commandbutton within w_orsale_webcon
end type
type cb_1 from commandbutton within w_orsale_webcon
end type
type cb_2 from commandbutton within w_orsale_webcon
end type
type gb_4 from groupbox within w_orsale_webcon
end type
type ddlb_dwtitles from dropdownlistbox within w_orsale_webcon
end type
type st_2 from statictext within w_orsale_webcon
end type
type st_3 from statictext within w_orsale_webcon
end type
type rb_1 from radiobutton within w_orsale_webcon
end type
type rb_2 from radiobutton within w_orsale_webcon
end type
type rb_3 from radiobutton within w_orsale_webcon
end type
type ddlb_op from dropdownlistbox within w_orsale_webcon
end type
type gb_5 from groupbox within w_orsale_webcon
end type
end forward
...
integer width = 4695
integer height = 2360
string title = "웹 접수확인(w_orsale_webcon)"
...
global w_orsale_webcon w_orsale_webcon
...
if GF_Permission("WEB_접수관리", gs_userid) = "Y" then
else
...
type pb_save from w_inheritance`pb_save within w_orsale_webcon
integer x = 3552
integer y = 64
integer taborder = 90
end type
...
for ll_row = 1 to dw_1.rowcount()
	...
	if ls_chkyn = "Y" then
		...
		ls_weborder[ll_dw1cnt] = dw_1.object.order_no[ll_row]		// 웹 주문번호
...
for ll_row = 1 to UpperBound(ls_weborder)
	...
	ls_itemno  = dw_1.object.item_no[ll_dw1cnt]	// 품목
	ls_qa      = dw_1.object.qa[ll_dw1cnt]			// 규격
...
if lb_continue = true then
	dw_1.object.chk_yn[ll_row_save] = "N"
...
for ll_row = 1 to UpperBound(ls_weborder)
	...
	ls_custno  = dw_1.object.cust_no[ll_dw1cnt]  // 거래처
	ls_itemno  = dw_1.object.cust_no[ll_dw1cnt]	// 품목
	ls_qa      = dw_1.object.cust_no[ll_dw1cnt]	// 규격
   ...
   ////////////////////////////////////////////////////////////////////////////////////////////	
	...
	// SALE
	// 2017.11.8 조원석 telno_cap 추가 saleshop에 컬럼 없어 복사시 누락 발생 //
	INSERT INTO sale (order_no, cust_no, scene_code, order_date, due_date, estimate_no, sale_flag, curr_code
	     , cust_po_no, cust_po_date, ship_flag, fob, salesman, confirm_flag, confirm_user, confirm_date
		  ...
		  , plan_date, shipamt_case, area_no, ysp_confirm, ysp_qty, color, type, class, ysp_qty2, org_user, telno_cap )
	SELECT sale_no, cust_no, scene_code, order_date, due_date, estimate_no, sale_flag, curr_code
	     , cust_po_no, cust_po_date, ship_flag, fob, salesman, confirm_flag, confirm_user, confirm_date
		  ...
		  , plan_date, shipamt_case, area_no, ysp_confirm, ysp_qty, color, type, class, ysp_qty2, :gs_userid, telno_cap
	  FROM saleshop
	 WHERE order_no = :ls_orderno;
	if sqlca.sqlcode = 0 then
		// SALEDET
		...
		SELECT :ls_saleno1, seq_no, item_no, qa, uom, order_qty, price, acc_qty, saledet_text
		     , action, sol_qty, loc_no, issue_qty, rlse_date, rlse_qty, rem, 0, plan_close, color
			  , price_be, do_rem, stock_qty, stock_issue, stock_do, price_last, danjung, jungrang 
		  FROM saleshopdet
		 WHERE order_no = :ls_orderno;	
		if sqlca.sqlcode = 0 then
...
type dw_1 from w_inheritance`dw_1 within w_orsale_webcon
integer x = 32
integer y = 456
integer width = 4581
integer height = 1768
...
type st_title from w_inheritance`st_title within w_orsale_webcon
integer x = 32
integer y = 24
integer width = 965
string text = "Web 접수확인"
end type

type st_tips from w_inheritance`st_tips within w_orsale_webcon
end type

type pb_compute from w_inheritance`pb_compute within w_orsale_webcon
boolean visible = false
integer taborder = 100
end type

type pb_print_part from w_inheritance`pb_print_part within w_orsale_webcon
boolean visible = false
integer taborder = 80
end type

type pb_close from w_inheritance`pb_close within w_orsale_webcon
integer x = 3744
integer y = 64
integer taborder = 70
end type
...
type pb_print from w_inheritance`pb_print within w_orsale_webcon
integer x = 3360
integer y = 64
integer taborder = 60
end type
...
type pb_cancel from w_inheritance`pb_cancel within w_orsale_webcon
boolean visible = false
end type

type pb_delete from w_inheritance`pb_delete within w_orsale_webcon
boolean visible = false
end type

type pb_insert from w_inheritance`pb_insert within w_orsale_webcon
boolean visible = false
integer taborder = 130
end type

type pb_retrieve from w_inheritance`pb_retrieve within w_orsale_webcon
integer x = 3168
integer y = 64
integer taborder = 40
end type
...
string ls_saleman, ls_area, ls_sdate, ls_edate, ls_type
long   ll_cnt,   ll_chk
...
dw_1.retrieve( ls_area, ls_saleman, ls_sdate, ls_edate, ls_type )
...
type gb_3 from w_inheritance`gb_3 within w_orsale_webcon
integer x = 1070
integer y = 12
integer width = 1627
integer height = 224
...
type gb_2 from w_inheritance`gb_2 within w_orsale_webcon
integer y = 260
integer width = 4581
integer height = 180
integer taborder = 0
...
type gb_1 from w_inheritance`gb_1 within w_orsale_webcon
integer x = 3141
integer width = 818
integer height = 224
integer taborder = 0
...
type dw_2 from w_inheritance`dw_2 within w_orsale_webcon
integer x = 50
integer y = 168
integer width = 96
integer height = 72
...
dw_1.setsort( "user_name A, order_date A, order_no D, seq_no A, cust_name A, scene_code A" )
dw_1.Sort()
...
dw_1.setsort( "order_no A, seq_no A" )
dw_1.Sort()
