...
type gb_7 from groupbox within w_osaroiup_m
end type
type gb_6 from groupbox within w_osaroiup_m
end type
...
type st_1 from statictext within w_osaroiup_m
end type
type pb_2 from picturebutton within w_osaroiup_m
end type
...
integer width = 4585
integer height = 2820
string title = "판매매출수정 및 취소(w_osaroiup_m)"
...
public subroutine wf_ret ();date ld_sdate, ld_edate
...
dw_2.reset()
dw_4.reset()
dw_3.retrieve(dw_1.object.cust_no[1],ld_sdate,ld_edate)
dw_3.setredraw(true)
dw_2.setredraw(true)
dw_4.setredraw(true)
dw_3.setfocus()
...
this.Control[iCurrent+14]=this.cbx_vat
...
gb_6.height = newheight - 590
dw_3.height = newheight - 652

dw_4.width  = newwidth  - 1042
dw_4.height = newheight - 868
...
select cust_name, captain_name, addr, tel_no into :ls_cust_name, :ls_cap_name, 
       :ls_addr, :ls_tel_no from customer
...
if cbx_1.checked = false then
	...
	dw_7.object.mon[1] = integer(MidA(dw_7.object.inv_date[1], 5, 2))
	dw_7.object.day[1] = integer(MidA(dw_7.object.inv_date[1], 7, 2))
	ll_count = 11 - ll_count
end if

dw_7.object.sp_cnt[1] = string(ll_count) 
if isnull(dw_2.object.citem[1] ) then
	IF dw_2.object.inv_type[1] = 'Y' THEN
	...
	dw_7.object.item_name[1] = dw_2.object.citem[1]
end if
...
event rowfocuschanged;string ls_item_no, ls_item_name
...
event clicked;long ll_count, ll_idx, ll_int, ll_cnt
string ls_cust_name, ls_addr, ls_reg_no, ls_per_no, ls_cap_name, ls_ind_type, ls_ind_item
string ls_inv_date, ls_sp_cnt, ls_cash, ls_vat, ls_item_name, ls_cash1, ls_vat1, ls_total
string ls_cust_no, ls_item_no
dec ld_total, ld_amount, ld_rate
...
//dw_9.object.inv_date[1] = string(dw_2.object.oi_date[1], "YYYY MM DD") //윗줄로 옮김
ld_amount = dw_2.object.aroiinv_cash[1] + dw_2.object.ship_cost[1] - dw_2.object.discount[1]
...
if isnull(dw_2.object.citem[1] ) then
	...
	dw_9.object.item_name[1] = dw_2.object.citem[1]
end if
...
if this.checked = true then
	...
	IF gf_permission('거래명세서_기본공급자', gs_userid) = 'Y' THEN
		...
		// 도로명주소 가져오기
		if string(dw_2.object.oi_date[1],"yyyy/mm/dd") < "2014/01/01" then
		else
