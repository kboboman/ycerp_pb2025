...
tab_1.tabpage_1.dw_3.width  = this.width  - 128
tab_1.tabpage_1.dw_3.height = this.height - 460

tab_1.tabpage_2.dw_4.width  = this.width  - 128
tab_1.tabpage_2.dw_4.height = this.height - 704
...
event pb_insert::clicked;string ls_name, ls_loc
...
dw4.object.src_loc[1]    = ls_loc
dw4.object.tar_loc[1]    = ls_loc
...
event selectionchanged;if GF_PERMISSION('물품대체_관리', gs_userid) = 'Y' then	
...
event itemchanged;string ls_item,ls_str,ls_qa,ls_uom,ls_itemname,ls_loc,ls_str1,ls_supp,ls_wc, ls_str2
...
choose case dwo.name
	case 'src_item'
			...
			select item_no,item_name,uom into :ls_item,:ls_itemname,:ls_uom 
			  from groupitem where item_no = :ls_str;
...
// 정미림(jml)추가 -> // 2007.05.28  김영란(kyr)추가			
			if gs_userid = 'root' or gs_userid = '1999010s' or gs_userid = 'JDS'  or gs_userid = 'jml' or gs_userid = 'kyr' then
			else
				select count(*) into :ll_cnt from groupitem
				 where item_no = :ls_str
					and use_flag = 'C';
				if ll_cnt > 0 then
