...
event clicked;//
//
//-- 2012-11-27(이재형) : system_error 테이블 Insert

string ls_errornum, ls_message, ls_where, ls_object, ls_event, ls_line

ls_errornum		= dw_error.Object.errornum[1]
if isNULL(ls_errornum) then ls_errornum = ''

if ls_errornum <> '' then
	ls_errornum		= dw_error.Object.errornum[1]
	ls_message		= dw_error.Object.message[1]
	ls_where			= dw_error.Object.where[1]
	ls_object		= dw_error.Object.object[1]
	//ls_event			= dw_error.Object.event[1]
	ls_event			= dw_error.getitemstring(1, "event") // dw_error.Object.event[1]
	ls_line			= dw_error.Object.line[1]

	INSERT INTO system_error(user_id, errornum, message, swhere, object, sevent, line) 
		VALUES (:gs_UserID, :ls_errornum, :ls_message, :ls_where, :ls_object, :ls_event, :ls_line) ;
	if sqlca.sqlcode <> 0 then
		messagebox('오류',"INSERT 도중 오류가 발생했습니다..~r~n" + sqlca.sqlerrtext,exclamation!)
		rollback;
	else
		commit;
	end if
