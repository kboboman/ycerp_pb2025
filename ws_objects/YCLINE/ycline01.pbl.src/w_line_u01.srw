$PBExportHeader$w_line_u01.srw
$PBExportComments$생산라인 라벨출력(유압)
forward
global type w_line_u01 from window
end type
type em_prt from editmask within w_line_u01
end type
type st_9 from statictext within w_line_u01
end type
type st_8 from statictext within w_line_u01
end type
type cb_4 from commandbutton within w_line_u01
end type
type cbx_1 from checkbox within w_line_u01
end type
type cbx_unit from checkbox within w_line_u01
end type
type st_7 from statictext within w_line_u01
end type
type cbx_box from checkbox within w_line_u01
end type
type em_top from editmask within w_line_u01
end type
type em_left from editmask within w_line_u01
end type
type st_6 from statictext within w_line_u01
end type
type st_5 from statictext within w_line_u01
end type
type st_key from statictext within w_line_u01
end type
type cb_3 from commandbutton within w_line_u01
end type
type cb_2 from commandbutton within w_line_u01
end type
type cb_1 from commandbutton within w_line_u01
end type
type st_2 from statictext within w_line_u01
end type
type dw_2 from datawindow within w_line_u01
end type
type st_1 from statictext within w_line_u01
end type
type dw_1 from datawindow within w_line_u01
end type
type st_3 from statictext within w_line_u01
end type
end forward
...
if MessageBox("확인", "재고에 반영합니다. 작업수량이 정확합니까?", Question!, YesNo!, 1) = 1 then
	for ll_row = 1 to ll_dw1row
		if wf_sfcday( ll_row ) = False then		// 저장(재고생성)
...
VALUES (:ldt_sfc_dt,:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,:ldb_work_qty,0,:gs_loc,'RP',0,:ls_wc_no,'현장작업일보[유압반]', :gs_userid);
if sqlca.sqlcode < 0 then
	ls_sqlerrtext = sqlca.sqlerrtext
	rollback;
...
date     ldt_date
string   ls_wcno, ls_custno, ls_scene, ls_itemno, ls_itemnm1, ls_itemnm2, ls_uom, ls_qa, ls_qa1, ls_qa23, ls_rem, ls_boxtype
string   ls_packno, ls_packno1, ls_mpsorder, ls_sqlerrtext, ls_left, ls_top, ls_prt, ls_color, ls_manager
string   ls_ks, ls_ks1, ls_ks2, ls_ks3, ls_ksd, ls_value, ls_right
integer  li_i, li_tab
long     ll_row, ll_dw1cnt, ll_unitqty, ll_boxqty

// 라벨출력
integer li_pageqty, li_pageqty2, li_boxqty, li_seq
...
// 품질관리자
SELECT rtrim(work_manager) INTO :ls_manager FROM wc WHERE work_no = :ls_wcno;
...
for ll_row = 1 to ll_dw1cnt
   ...
   // 수주번호가 있으면 주문품(O), 재고품(S)
	...
	// ERP품명, BOX품명 출력구분
	if cbx_box.checked = true then
		ls_value = dw_1.Object.combo_name[ll_row]	// BOX품명
	else	
		ls_value = dw_1.Object.item_name[ll_row]		// ERP품명
	end if
	
	// 품명을 2줄 만들기
	li_tab     = PosA(ls_value, "(", 1)
	ls_itemnm1 = LeftA(ls_value, li_tab - 1)
	ls_itemnm2 = "(" + MidA(ls_value, li_tab + 1)
	
	dw_2.object.lb_item_name[1]  = ls_itemnm1								// 품명1
	dw_2.Object.lb_item_name2[1] = ls_itemnm2						     // 품명2
	
	dw_2.object.lb_color[1]      = dw_1.object.box_color[ll_row]	// 색상
	dw_2.object.lb_qa[1]         = LeftA(dw_1.object.qa[ll_row],13)	// 규격
	dw_2.object.lb_rem[1]        = ""										// 비고
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	// 주문품이면 
	// AL TILE (평판), 080 X 600 X 600 = BOX당 24EA (Ex: 1250 / 24 = 52.08	52장까지는 BOX수량, 53장 잔량)
	// 재고품이면
	// BOX수량으로 출력(잔량 아님)
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	ll_unitqty    = dw_1.object.unit_qty[ll_row]	// 작업수량
	ll_boxqty    = dw_1.object.box_qty[ll_row]		// BOX 수량
	li_pageqty  = dw_1.object.page_qty[ll_row]	// 출력장수
	
	if ll_boxqty > 0 then 
		li_pageqty  = Truncate(ll_unitqty / ll_boxqty + 0.0000001, 0 )
		li_pageqty2 = mod(ll_unitqty, ll_boxqty)
	else
		li_pageqty  = 0
		li_pageqty2 = ll_unitqty
	end if
	
	// BOX갯수 반복 출력
	for li_i = 1 to li_pageqty
		dw_2.object.lb_qty[1] = ll_boxqty
		dw_2.Object.DataWindow.Print.Copies = 1
		dw_2.print()
	next
	
	// 마지막페이지 잔량출력
	if li_pageqty2 > 0 then
		// 주문(O) 잔량수량, 재고(S) BOX수량
		if ls_stock = "S" then li_pageqty2 = ll_boxqty
		
		dw_2.object.lb_qty[1] = li_pageqty2
		dw_2.Object.DataWindow.Print.Copies = 1
		dw_2.print()
	end if
	////////////////////////////////////////////////////////////////////////////////////////////////////////
...
string  ls_wcno, ls_areano, ls_item_name, ls_qa, ls_qa23, ls_boxtype, ls_selitem, ls_selqa, ls_chk, ls_cust, ls_scene
date    ld_date
...
if gs_label = 'OLD'  then
	dw_2.DataObject = 'd_label_165x100_u01'
else
	dw_2.DataObject = 'd_label_165x100_u01_new'
end if
   ...
   lsa_item_name[li_cnt] = w_line.dw_1.object.item_name[ll_selrow]
    ...
    //Close(w_line_f7) //현재 상테에서 종료하고 메인윈도우로 가지 못하는 문제. 
	 //윈도우는 종료해도 스크립트 실행은 끝가지 간다.
		ls_chk = 'Y'
		goto err
	elseif ls_qa <> LeftA(ls_selqa,13) then
		// 유압반은 규격체크하지 않음
	end if
...
DEBUGBREAK()
// 조회후 BOX품목 + 규격으로 BOX당 수량 구하기
ll_dw1cnt = dw_1.Retrieve( ls_wcno, ld_date, ls_areano, lsa_order_no, lia_order_seq )

for ll_row = 1 to ll_dw1cnt
	ls_boxtype = dw_1.object.box_type[ll_row]
	ls_qa23    = MidA(dw_1.object.qa[ll_row], 5, 9)
	
	GF_SPLIT(ls_qa23, "X", lsa_qa)
	ls_qa23    = string(integer(lsa_qa[1])) + "*" + string(integer(lsa_qa[2]))
	
	SELECT isnull(qty,0) INTO :ll_boxqty FROM label_boxmst WHERE box_name = :ls_boxtype AND qa = :ls_qa23;
	dw_1.object.box_qty[ll_row] = ll_boxqty
next
...
event clicked;// 이전방식
if GF_Permission("유압반_이전방식출력", "YCLINE_U01_OLD") = "Y" then
	Open( w_label_prt )	// 유압반 라벨
end if
...
long textcolor = 128
long backcolor = 67108864
string text = "100x130(구양식)"
end type

event clicked;//
if this.checked = true then
	dw_2.dataobject = "d_label_13x10_u01"
	dw_2.SetTransObject(sqlca)
else
...
type dw_2 from datawindow within w_line_u01
string tag = "d_label_13x10_u01"
integer x = 782
integer y = 820
integer width = 2802
integer height = 976
string title = "d_label_165x100_u01"
string dataobject = "d_label_165x100_u01"
boolean border = false
...
choose case dwo.name
	case "unit_qty"
		ll_unitqty  = long(data)			         	// 작업수량
		ll_boxqty   = this.object.box_qty[ll_row]		// BOX 수량
		
		if isnull(ll_boxqty) OR ll_boxqty < 1 then
		else			
			if ll_boxqty > 0 then 
				li_page_qty  = Truncate( ll_unitqty / ll_boxqty + 0.0000001, 0 )
				li_page_qty2 = mod(ll_unitqty, ll_boxqty)
			else
				li_page_qty  = 0
				li_page_qty2 = ll_unitqty
			end if
			
			this.object.page_qty[ll_row] = li_page_qty
		end if
end choose
