...
for ll_row = 1 to arg_cnt
	...
	if trim(dw_1.object.uom[ll_row]) = 'M2' then
		//이광선 차장님의 요구로 단위가 M2인 것은 생산계획조정에 안 들어가게 함
		dw_1.object.saledet_text[ll_row] = 'X'
	end if
next
return true
...
string ls_cust, ls_scene, ls_uom, ls_suju, ls_wc, ls_assyitem, ls_assyqa, ls_flag, ls_area
...
for ll_row = 1 to arg_cnt
	...
	if ll_cnt < 1 then
		...
		dw_4.object.msg1[ll_ins] = "오류1  - [" + ls_item + "][" + ls_qa1 + "] 물품의 공정이 등록되지 않았습니다. " + STRING(now())
		rollback;
		continue
	else
	    for j =  1 to ll_cnt
...
// 실계획수량 = 물품단위당수량 * 미계획수량(수주수량 - (수주가용량 + 출고수량 + 생산계획수량)
          ...
          ls_assyqa  = trim(dw_3.object.assy_qa[j])		
		    ...
		    if sqlca.sqlcode = 100 or sqlca.sqlcode < 0 then
				ll_ins  = dw_4.insertrow(0)
				dw_4.object.msg1[ll_ins] = "오류3  - [" + ls_assyitem + "] 물품구성이 잘못되었습니다. " + STRING(now())
				rollback;
				ls_error = 'Y'
				exit
		    end if
...
IF GF_PERMISSION("자동생산계획생성(YSP)", gs_userid) = "Y" AND gs_area <> "T0001" THEN
	THIS.postevent('ue_1')
	// 자동새산계획 automatic time = 30분 = 30 * 60초 = 1800초
	// Timer(0.05)
ELSE
...
event pb_save::clicked;if this.text = '전체선택' then
	this.text = '선택취소'
	cbx_1.checked = true
