...
type gb_15 from groupbox within w_mpouter_m
end type
type gb_4 from groupbox within w_mpouter_m
end type
...
event pb_save::clicked;long ll_rowcount,i
...
for i = 1 to ll_rowcount
   ...
   update mpsdet set rcpt_type = 'N'
   where plan_no = :ls_plan 
	  and item_no = :ls_assy 
	  and qa = :ls_assyqa;
...
event clicked;long ll_rowcount,ll_cnt,ll_find,ll_row,ll_count
...
string text = "외주대상물품을 선택한 후 [외주]버튼을 누르면 사급,도급쪽으로 데인터가 이동됩니다."
boolean border = true
borderstyle borderstyle = stylelowered!
...
event clicked;if dw3.update() = 1 then
...
dw4.retrieve(ls_sale, dw_area.object.area[1])
...
if ii_sort = 1 then
	ii_sort = 0
	dw_4.SetSort("item_no D, qa A, sale_no A ")
else
	ii_sort = 1
	dw_4.SetSort("item_no A, qa A, sale_no A ")
end if
...
ls_sale = trim(sle_1.text) + "%"
dw5.retrieve(ls_sale, dw_area.object.area[1])
...
if ii_sort = 1 then
	ii_sort = 0
	dw_5.SetSort("item_no D, qa A, sale_no A ")
else
	ii_sort = 1
	dw_5.SetSort("item_no A, qa A, sale_no A ")
end if
