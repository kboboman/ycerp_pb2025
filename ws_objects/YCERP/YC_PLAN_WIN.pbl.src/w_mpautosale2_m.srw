$PBExportHeader$w_mpautosale2_m.srw
$PBExportComments$자동생산계획생성(YSP)(1999/10/29,이인호)
forward
global type w_mpautosale2_m from w_inheritance
end type
type dw_3 from datawindow within w_mpautosale2_m
end type
type dw_4 from datawindow within w_mpautosale2_m
end type
type cbx_1 from checkbox within w_mpautosale2_m
end type
type cb_4 from commandbutton within w_mpautosale2_m
end type
type st_info from statictext within w_mpautosale2_m
end type
type st_2 from statictext within w_mpautosale2_m
end type
type st_1 from statictext within w_mpautosale2_m
end type
type st_vertical from u_splitbar_vertical within w_mpautosale2_m
end type
end forward
...
integer width = 4718
integer height = 2380
string title = "자동생산계획생성:YSP(w_mpautosale2_m)"
...
global w_mpautosale2_m w_mpautosale2_m
...
public function boolean wf_plandet_backup (long arg_cnt)
end prototypes
...
public function boolean wf_alter (long arg_cnt);//
long     ll_row,  ll_ins
string   ls_item, ls_itemnm, ls_qa, ls_qa1
...
for ll_row = 1 to arg_cnt
	...
	if isnull(dw_1.object.itemjaego_item_no[ll_row]) OR trim(dw_1.object.itemjaego_item_no[ll_row]) = "" then
	else
		// 재고물품인 경우에는 saledet.sol_qty에 가용량을 넣지 않고 
		// 출고시에 수동으로 가용량 을 설정후에 출고 할수 있도록함.
		dw_1.object.saledet_text[ll_row] = "X"
		
		ll_ins  = dw_4.insertrow(0)
		dw_4.object.rout_date[ll_ins] = ldt_sysdate
		dw_4.object.area_no[ll_ins]   = "B0001"
		dw_4.object.user_id[ll_ins]   = gs_userid
		dw_4.object.rout_msg[ll_ins]  = "SKIP  - [" + ls_itemnm + "][" + ls_qa1 + "] '재고물품 테이블에 존재함' " + STRING(now())		
	end if
	
	if trim(dw_1.object.uom[ll_row]) = "M2" then
		// 단위가 M2인 것은 생산계획조정에 안 들어가게 함
		dw_1.object.saledet_text[ll_row] = "X"
		...
		dw_4.object.area_no[ll_ins]   = "B0001"
		dw_4.object.user_id[ll_ins]   = gs_userid
		dw_4.object.rout_msg[ll_ins]  = "SKIP  - [" + ls_itemnm + "][" + ls_qa1 + "] '단위가 M2인 것은 생산계획조정 반영안함' " + STRING(now())		
	end if
next
...
//								    = "N" : 첫번째공정이 아닌경우
////////////////////////////////////////////////////////////////////////
long     ll_ins, ll_row, l_qa_check, l_row
string   ls_source, ls_error, ls_order, ls_item, ls_itemnm, ls_qa, ls_rem, ls_qa1, ls_like, ls_color
string   ls_cust, ls_scene, ls_uom, ls_suju, ls_wc, ls_assyitem, ls_assyqa, ls_flag, ls_area, ls_represent
real     lr_qpa,lr_notqty, lr_setuptime, lr_runtime, lr_planqty, lr_qoh, lr_saleqty, lr_prodqty
datetime ldt_duedate
long     ll_cnt, j
int      li_count, li_seq, li_bom, li_workper, li_operation, li_op
string   ls_itemname, ls_mpuom, ls_qa2, ls_str, ls_qa3, ls_temp
string   ls_sos, ls_text
datetime ldt_sysdate
...
//	if dw_1.IsSelected(ll_row) = false then CONTINUE
	...
	setnull( ls_sos ) ; setnull( ls_text )
	
	ls_sos  = dw_1.object.groupitem_source[ll_row]
	ls_text = dw_1.object.saledet_text[ll_row]
	if ls_sos = "B" OR ls_text = "X" then
		ll_ins  = dw_4.insertrow(0)
		dw_4.object.rout_date[ll_ins] = ldt_sysdate
		dw_4.object.area_no[ll_ins]   = "B0001"
		dw_4.object.user_id[ll_ins]   = gs_userid
...
//	ll_cnt  = dw_3.retrieve(ls_item,ls_like)
	if ll_cnt < 1 then
		...
		dw_4.object.area_no[ll_ins]   = "B0001"
		dw_4.object.user_id[ll_ins]   = gs_userid
		dw_4.object.rout_msg[ll_ins]  = "오류1  - [" + ls_itemnm + "][" + ls_qa1 + "] 물품의 공정이 등록되지 않았습니다. " + STRING(now())
		CONTINUE
	else
		for j = 1 to ll_cnt		// 공정단계까지
			lr_qpa       = dw_3.object.qpa[j]     
			...
			ls_assyitem  = trim(dw_3.object.assembly[j])
			ls_assyqa    = trim(dw_3.object.assy_qa[j])		
			ls_flag    = trim(dw_3.object.flag[j])		
		   ...
		   if sqlca.sqlcode = 100 or sqlca.sqlcode < 0 then
			...
			SELECT count(*) INTO :li_count FROM item with(nolock) WHERE item_no = :ls_assyitem and qa = :ls_qa2;
			if li_count = 0 then
				ls_qa3 = MidA(ls_qa2,1,14) + "00000"
				INSERT INTO item (item_no,qa,loc_no,item_name,uom,source,leadtime,count_code,qoh,sales_reserved,
			          prodn_reserved,pur_reserved,reorder,safety_stock,lot,obsolete_code,scrap_qty,
			   ...
			   VALUES(:ls_assyitem,:ls_qa2,'WS00000000',:ls_itemname,:ls_uom,:ls_source,0,'M',0,0,
				...
				SELECT count(*) INTO :li_count FROM itempattern with(nolock) WHERE item_no = :ls_assyitem and qa1 = :ls_qa3;
				if li_count = 0 then
					INSERT INTO itempattern (item_no,qa1) VALUES(:ls_assyitem,:ls_qa3);
					if sqlca.sqlcode < 0 then
						ROLLBACK;
			...
			if j = 1 then // 마지막공정이면(완제품공정)
				...
				if sqlca.sqlcode < 0 then
					int ll_int3
					SELECT count(*) INTO :ll_int3 FROM routtempnew with(nolock)
					 WHERE operation = :li_operation AND wc_no   = :ls_wc
						AND sale_no = :ls_order	AND seq_no = :li_seq AND division = 1;
				   if ll_int3 > 0 then
						DELETE FROM routtempnew
...
/*				
				SELECT item_no INTO :ls_temp FROM itemjaego with(nolock) WHERE item_no = :ls_assyitem AND qa = :ls_qa2;
...
//			        end if				 
						...
						SELECT qoh INTO :lr_qoh FROM itemloc with(nolock)
...
*/						
						...
						if sqlca.sqlcode < 0 then
							...
							dw_4.object.area_no[ll_ins]   = "B0001"
							dw_4.object.user_id[ll_ins]   = gs_userid
							dw_4.object.rout_msg[ll_ins]  = "오류9 - [" + ls_itemnm + "][" + ls_qa + "] 에 대한 공정 구성중 오류가 발생했습니다. " + STRING(now())
...
/*
					case else
						...
						dw_4.object.area_no[ll_ins]   = "B0001"
						dw_4.object.user_id[ll_ins]   = gs_userid
						dw_4.object.rout_msg[ll_ins]  = "오류10 - [" + ls_itemnm + "][" + ls_qa + "] 에 대한 공정 구성중 오류가 발생했습니다. " + STRING(now())
...
if GF_PERMISSION("자동생산계획생성(YSP)", gs_userid) = "Y" AND gs_area <> "T0001" then
	//////////////////////////////////////////////////////////////////
	...
	MessageBox("사업장권한","'자동생산계획생성(YSP)' 권한과, 기본사업장이 일치하지 않습니다.")
end if
...
type pb_save from w_inheritance`pb_save within w_mpautosale2_m
integer x = 2757
integer y = 60
integer width = 375
...
type dw_1 from w_inheritance`dw_1 within w_mpautosale2_m
integer x = 1851
integer y = 328
integer width = 2752
integer height = 1812
integer taborder = 60
string dataobject = "d_mpautosale2_m"
...
int li_int, li_seq
string ls_order, ls_login, ls_qa1
...
type st_title from w_inheritance`st_title within w_mpautosale2_m
integer x = 46
integer width = 1463
integer height = 120
string text = "자동생산계획생성(YSP)"
end type

type st_tips from w_inheritance`st_tips within w_mpautosale2_m
end type

type pb_compute from w_inheritance`pb_compute within w_mpautosale2_m
integer x = 4197
integer y = 60
...
dw_4.object.area_no[ll_dw4row]   = "B0001"
...
type pb_print_part from w_inheritance`pb_print_part within w_mpautosale2_m
boolean visible = false
end type

type pb_close from w_inheritance`pb_close within w_mpautosale2_m
integer x = 4389
integer y = 60
...
type pb_print from w_inheritance`pb_print within w_mpautosale2_m
boolean visible = false
integer x = 2757
integer y = 60
...
type pb_cancel from w_inheritance`pb_cancel within w_mpautosale2_m
boolean visible = false
integer taborder = 130
end type

type pb_delete from w_inheritance`pb_delete within w_mpautosale2_m
integer x = 462
integer y = 228
integer width = 91
integer height = 92
...
type pb_insert from w_inheritance`pb_insert within w_mpautosale2_m
integer x = 37
integer y = 228
integer width = 425
integer height = 92
...
type pb_retrieve from w_inheritance`pb_retrieve within w_mpautosale2_m
integer x = 3781
integer y = 60
integer width = 402
...
type gb_3 from w_inheritance`gb_3 within w_mpautosale2_m
boolean visible = false
integer x = 1792
integer y = 256
integer width = 1641
...
type gb_2 from w_inheritance`gb_2 within w_mpautosale2_m
integer x = 2482
integer y = 16
integer width = 677
integer height = 208
...
type gb_1 from w_inheritance`gb_1 within w_mpautosale2_m
integer x = 3177
integer y = 16
integer width = 1426
integer height = 208
...
type dw_2 from w_inheritance`dw_2 within w_mpautosale2_m
integer x = 2053
integer y = 24
integer width = 123
integer height = 96
...
long textcolor = 16711680
long backcolor = 67108864
string text = "우측 마우스 : 생산계획생성, 재고내역, 공정관리, 메시지보내기"
