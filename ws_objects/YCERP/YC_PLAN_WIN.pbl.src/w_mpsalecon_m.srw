$PBExportHeader$w_mpsalecon_m.srw
$PBExportComments$영업수주생산확정
forward
global type w_mpsalecon_m from w_inheritance
end type
type gb_15 from groupbox within w_mpsalecon_m
end type
type dw_3 from datawindow within w_mpsalecon_m
end type
type dw_4 from datawindow within w_mpsalecon_m
end type
type st_2 from statictext within w_mpsalecon_m
end type
type em_1 from editmask within w_mpsalecon_m
end type
type em_2 from editmask within w_mpsalecon_m
end type
type cbx_1 from checkbox within w_mpsalecon_m
end type
type dw_5 from datawindow within w_mpsalecon_m
end type
type dw_6 from datawindow within w_mpsalecon_m
end type
type rb_all from radiobutton within w_mpsalecon_m
end type
type rb_part from radiobutton within w_mpsalecon_m
end type
type dw_7 from datawindow within w_mpsalecon_m
end type
type em_3 from editmask within w_mpsalecon_m
end type
type dw_8 from datawindow within w_mpsalecon_m
end type
type dw_area from datawindow within w_mpsalecon_m
end type
type rb_2 from radiobutton within w_mpsalecon_m
end type
type rb_3 from radiobutton within w_mpsalecon_m
end type
type st_5 from statictext within w_mpsalecon_m
end type
type cb_13 from commandbutton within w_mpsalecon_m
end type
type st_13 from statictext within w_mpsalecon_m
end type
type ddlb_fld from dropdownlistbox within w_mpsalecon_m
end type
type ddlb_dwtitles from dropdownlistbox within w_mpsalecon_m
end type
type st_14 from statictext within w_mpsalecon_m
end type
type ddlb_op from dropdownlistbox within w_mpsalecon_m
end type
type sle_value from singlelineedit within w_mpsalecon_m
end type
type cb_1 from commandbutton within w_mpsalecon_m
end type
type cb_2 from commandbutton within w_mpsalecon_m
end type
type cb_3 from commandbutton within w_mpsalecon_m
end type
type cb_4 from commandbutton within w_mpsalecon_m
end type
type st_horizontal from u_splitbar_horizontal within w_mpsalecon_m
end type
type st_vertical from u_splitbar_vertical within w_mpsalecon_m
end type
type rb_1 from radiobutton within w_mpsalecon_m
end type
type rb_4 from radiobutton within w_mpsalecon_m
end type
type rb_5 from radiobutton within w_mpsalecon_m
end type
type gb_5 from groupbox within w_mpsalecon_m
end type
end forward
...
integer width = 5061
integer height = 2628
string title = "판매생산확정(w_mpsalecon_m)"
gb_15 gb_15
dw_3 dw_3
dw_4 dw_4
st_2 st_2
...
global w_mpsalecon_m w_mpsalecon_m

type variables
st_sale_ysp ist_ysp
decimal ir_price
string is_yn_flag, is_confirm_flag
datawindowchild idwc_qa,  idwc_item, idwc_cust
string   is_flag, is_won = "WON", is_price_update, is_view_chk = 'Y'
...
SELECT scene_desc INTO :ls_scene_name FROM scene WHERE scene_code    = :ls_scene_code;
dw_2.setredraw(true)

/*2019.01.31 JOWONSUK  추가 */
string ls_area_no
...
/*2019.01.31 JOWONSUK  보완  */
dw_3.retrieve(as_order, ls_area_no)

end subroutine

on w_mpsalecon_m.create
int iCurrent
call super::create
...
this.Control[iCurrent+30]=this.st_horizontal
this.Control[iCurrent+31]=this.st_vertical
...
dw_4.object.confirm_flag.visible = false		// 확정
dw_4.object.input_date.visible = false		// 입력일자
dw_4.setsort( "confirm_date desc, order_no" )
dw_4.sort()
...
// 단가수정 권한
if GF_Permission("판매생산확정_단가수정", gs_userid) = "Y" then
	is_price_update = 'Y'
else
	is_price_update = 'N'
end if
...
// 납기조정 권한
if GF_Permission("판매생산확정_납기조정", gs_userid) = "Y" then
...
string ls_duty
SELECT duty_flag2 INTO :ls_duty FROM login WHERE user_id = :gs_userid ;

if GF_Permission("판매수주생산확정", gs_userid) = "Y" then
...
type pb_save from w_inheritance`pb_save within w_mpsalecon_m
integer x = 4590
...
string   ls_itemno, ls_order_no, ls_tax_sign, ls_conf, ls_area_no, ls_costyn
long     ll_row, ll_row1
datetime ldt_null
dec      lr_amt
...
//
if is_confirm_flag = "Y" AND dw_2.object.sale_confirm_flag[1] = "N" then 
	...
	if li_count > 0 then
		rollback;
		MessageBox("확인","이 주문은 작업지시가 내려진 수주입니다. ~r~n" &
								+ "생산관리자 협의하시기바랍니다.")
		return
	end if
...
// 확정취소가 아닌경우
	...
	if ls_sale_control = "A" then // 매출통제: 완전통제
		MessageBox("저장오류","매출통제 대상 거래처입니다. ~r~n~n" &
...
type dw_1 from w_inheritance`dw_1 within w_mpsalecon_m
integer x = 1294
integer y = 116
integer width = 873
integer height = 96
...
type st_title from w_inheritance`st_title within w_mpsalecon_m
integer x = 0
integer y = 0
integer width = 1006
integer height = 140
...
type st_tips from w_inheritance`st_tips within w_mpsalecon_m
end type

type pb_compute from w_inheritance`pb_compute within w_mpsalecon_m
integer x = 2747
integer y = 64
integer width = 169
integer height = 120
...
if ls_confirm = "T" then	// 판매생산 확정
else
	MessageBox("확인", "판매수주가 미확정 상태입니다.")
...
type pb_print_part from w_inheritance`pb_print_part within w_mpsalecon_m
integer x = 4498
integer y = 288
integer width = 471
integer height = 92
...
type pb_close from w_inheritance`pb_close within w_mpsalecon_m
integer x = 4782
...
type pb_print from w_inheritance`pb_print within w_mpsalecon_m
integer x = 4393
end type
...
type pb_cancel from w_inheritance`pb_cancel within w_mpsalecon_m
integer x = 3817
integer y = 288
integer width = 398
integer height = 92
...
type pb_delete from w_inheritance`pb_delete within w_mpsalecon_m
integer x = 4361
integer y = 288
integer width = 123
integer height = 88
...
IF LeftA(ls_area_no,1) = 'B' THEN		// YSP 사업장(반월)
	...
	if ls_conf = 'Y' then
		messagebox("확인","이 주문은 생산접수가 완료된 주문입니다. ~r~n" &
								+ "YSP생산관리자 협의하시기바랍니다.")
		return
	end if
END IF
...
type pb_insert from w_inheritance`pb_insert within w_mpsalecon_m
integer x = 4229
integer y = 288
integer width = 123
integer height = 88
...
type pb_retrieve from w_inheritance`pb_retrieve within w_mpsalecon_m
integer x = 4197
integer taborder = 80
end type
...
if cbx_1.checked = true then	// 생산확정
	ls_confirm_flag = 'Y'
else
	ls_confirm_flag = 'T'
end if
...
/*ll_row = dw_4.retrieve(ls_cust_no, ld_sorder_date, ld_eorder_date, ls_confirm_flag, dw_area.object.area[1], ls_area_no)*/
/*2019.01.31 JOWONSUK  보완  */
ll_row = dw_4.retrieve(ls_cust_no, ld_sorder_date, ld_eorder_date, ls_confirm_flag,  ls_area_no)
if ll_row < 1 then MessageBox("확인","해당하는 조건의 데이터가 존재하지 않습니다.")
	
end event

type gb_3 from w_inheritance`gb_3 within w_mpsalecon_m
integer x = 2213
integer y = 0
integer width = 1111
integer height = 220
...
type gb_2 from w_inheritance`gb_2 within w_mpsalecon_m
integer width = 4722
integer height = 192
integer taborder = 0
integer textsize = -8
...
type gb_1 from w_inheritance`gb_1 within w_mpsalecon_m
integer x = 4160
integer y = 0
integer width = 841
integer height = 220
...
type dw_2 from w_inheritance`dw_2 within w_mpsalecon_m
integer x = 32
integer y = 440
integer width = 2738
integer height = 1104
integer taborder = 0
string dataobject = "d_mpsalecon_m"
boolean vscrollbar = false
end type

event dw_2::itemchanged;//
string   ls_order_no, ls_credit_level, ls_fob, ls_order_no1
string   ls_conf, ls_area_no, ls_itemno, ls_costyn
long     ll_row
int      li_cnt
...
choose case dwo.name
	case 'sale_confirm_flag'
		if data = 'Y' then	// 생산확정
			...
			if LeftA(ls_order_no,1) = "B" THEN	// YSP수주품
				//
			else                                // 일반품
				// 가공비&타공비 수주확정시
				for ll_row = 1 to dw_3.rowcount()
			...
			IF LeftA(ls_area_no,1) = 'B' THEN		// YSP수주품
				setnull(ls_conf)
...
event dw_2::itemerror;if is_yn_flag = '1' then
	...
	this.object.sale_confirm_flag[1] = 'Y'
...
if this.rowcount() > 0 then
	ls_orderno = this.object.order_no[this.getrow()]
	
	dw_2.retrieve( ls_orderno )
	if dw_2.rowcount() < 1 then RETURN

	dw_2.object.sale_tel_no.TabSequence    = 0
	dw_2.object.ship_flag.TabSequence      = 0
	dw_2.object.sale_ship_case.TabSequence = 0
	dw_2.object.fob.TabSequence            = 0
	dw_2.object.ysp_qty2.TabSequence       = 0
	dw_2.object.sale_ysp_qty.TabSequence   = 0
	dw_2.object.sale_color.TabSequence     = 0
	dw_2.object.sale_class.TabSequence     = 0
	dw_2.object.sale_type.TabSequence      = 0
	dw_2.object.rem.TabSequence            = 0
	dw_2.object.rem1.TabSequence           = 0
	dw_2.object.rem2.TabSequence           = 0
	...
	dw_3.retrieve( ls_orderno, ls_area_no )
	
	dw_3.object.uom.TabSequence   = 0
   dw_3.object.price.TabSequence = 0
	if GF_Permission( "판매생산확정_비고수정", gs_userid ) = "Y" then
	else
...
lst_where.str4 = em_1.text
lst_where.str5 = em_2.text
