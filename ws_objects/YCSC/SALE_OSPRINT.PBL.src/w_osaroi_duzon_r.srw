...
event resize;call super::resize;gb_2.width  = this.width  - 87
gb_2.height = this.height - 452

dw_1.width  = this.width  - 141

dw_2.width  = this.width  - 141
dw_2.height = this.height - 1808

cb_1.width  = dw_1.width
//hpb_1.width = cb_1.width - 23
...
if dwxls.rowcount() > 0 then
   ...
   //===============================================================//
	if MessageBox("확인","회계 자동전표(더존)를 표준서식으로 만들겠습니까?~n~n" + &
		...
		OleExcel.WorkBooks.Open(ls_DocName)
...
MessageBox("확인","매출마감 및 엑셀파일 작업완료.")
...
fontfamily fontfamily = modern!
string facename = "굴림체"
string text = "영업매출 세금계산서를 회계(더존) 자동전표 만들기."
end type

event clicked;//
long	 ll_row, lY, lX, ll_dw2row, ll_slipno
string ls_getdate, ls_esero, ls_partno, ls_teamname, ls_acctdate
...
for ly = 1 to ll_row
	...
	if dw_1.object.chk[ly] = 1 and ls_partno <> "DD" then
		...
		dw_2.object.co_cd[ll_dw2row]     = "6000"       		// '회사코드'         2018.04.30 jowonsuk 변경 1000 -> 6000
