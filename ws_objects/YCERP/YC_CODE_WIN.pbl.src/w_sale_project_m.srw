$PBExportHeader$w_sale_project_m.srw
$PBExportComments$선행영업등록(2018/01/08- 조원석)
forward
global type w_sale_project_m from w_inheritance
end type
type dw_3 from datawindow within w_sale_project_m
end type
type tab_1 from tab within w_sale_project_m
end type
...
type st_4 from statictext within w_sale_project_m
end type
type ddlb_fld from dropdownlistbox within w_sale_project_m
end type
type ddlb_dwtitles from dropdownlistbox within w_sale_project_m
end type
type st_5 from statictext within w_sale_project_m
end type
type sle_value from singlelineedit within w_sale_project_m
end type
type cb_3 from commandbutton within w_sale_project_m
end type
type cb_4 from commandbutton within w_sale_project_m
end type
type cb_5 from commandbutton within w_sale_project_m
end type
type cb_6 from commandbutton within w_sale_project_m
end type
type pb_1 from picturebutton within w_sale_project_m
end type
type st_vertical from u_splitbar_vertical within w_sale_project_m
end type
type st_horizontal from u_splitbar_horizontal within w_sale_project_m
end type
type ddlb_op from dropdownlistbox within w_sale_project_m
end type
type pb_photo_insert from picturebutton within w_sale_project_m
end type
type pb_photo_delete from picturebutton within w_sale_project_m
end type
type dw_4 from datawindow within w_sale_project_m
end type
type st_6 from statictext within w_sale_project_m
end type
type cbx_picture_check from checkbox within w_sale_project_m
end type
type rb_1 from radiobutton within w_sale_project_m
end type
type rb_2 from radiobutton within w_sale_project_m
end type
type rb_3 from radiobutton within w_sale_project_m
end type
end forward
...
integer width = 9061
integer height = 4696
string title = "프로젝트 관리(대형)(w_sale_project_m)"
...
global w_sale_project_m w_sale_project_m

type variables
gs_item ist_item
string  is_dw, is_dw1sql, is_dw3sql, is_project_code
...
this.Control[iCurrent+14]=this.st_horizontal
this.Control[iCurrent+15]=this.ddlb_op
this.Control[iCurrent+16]=this.pb_photo_insert
this.Control[iCurrent+17]=this.pb_photo_delete
this.Control[iCurrent+18]=this.dw_4
this.Control[iCurrent+19]=this.st_6
this.Control[iCurrent+20]=this.cbx_picture_check
this.Control[iCurrent+21]=this.rb_1
this.Control[iCurrent+22]=this.rb_2
this.Control[iCurrent+23]=this.rb_3
end on

on w_sale_project_m.destroy
call super::destroy
destroy(this.dw_3)
destroy(this.tab_1)
destroy(this.st_4)
...
type pb_save from w_inheritance`pb_save within w_sale_project_m
integer x = 4055
...
string ls_project_code, ls_scene_code, ls_select_name, ls_select_tel
...
if i_tabpage = 1 then
	...
	if MessageBox("저장","저장하시겠습니까?",question!,okcancel!,1) = 2 then return
		...
		ls_project_code = trim(tab_1.tabpage_1.dw_5.getitemstring(1, "project_code"))
			
//		if isnull(ls_project_code) or ls_project_code = "" then
//			MessageBox("저장오류","프로젝트는 필수 입력사항입니다!",stopsign!)
//			tab_1.tabpage_1.dw_5.setcolumn('project_code')
...
//		end if		
		...
		if l_status = newmodified! or l_status = new! then			
			...
			setnull(l_project_code_seq)
			
			/*2018.10.15 jowonsuk 5자리로 순번 생성 로직 변경 */			
			SELECT top 1 ISNULL(CONVERT(smallint, project_code),0) + 1
			into :l_project_code_seq	
			from salepre_project with(nolock)
			order by project_code desc;
			
			
			if isnull(l_project_code_seq) then
				ls_project_code = '00001'
			else
				ls_project_code = trim(string(l_project_code_seq, '00000'))
			end if
			
			tab_1.tabpage_1.dw_5.setitem(1, "project_code", ls_project_code)	
			
		
			tab_1.tabpage_1.dw_5.setitem(1, "write_user_id", gs_userid)
			tab_1.tabpage_1.dw_5.setitem(1, "write_date", trim(gf_today2s()) )
			tab_1.tabpage_1.dw_5.setitem(1, "modify_user_id", gs_userid)
			tab_1.tabpage_1.dw_5.setitem(1, "modify_date", trim(gf_today2s()) )			
			tab_1.tabpage_1.dw_5.setitem(1, "project_end_yn", 'Y' )						
...
type dw_1 from w_inheritance`dw_1 within w_sale_project_m
integer x = 27
integer y = 400
integer width = 2194
integer height = 2120
integer taborder = 10
string title = "d_sale_scene_r"
string dataobject = "d_sale_project_r"
...
is_project_code = this.object.project_code[row]

if isnull(is_project_code) or is_project_code = "" then
else	
	...
	if gs_level = '3' then	
		...
		if (this.object.a_person1[row] = gs_userid )  then
			ls_retrieve = 'Y'
		ELSE
			ls_retrieve = 'N'
		end if
...
IF ls_retrieve = 'Y' THEN
	...
	l_row = tab_1.tabpage_1.dw_5.retrieve(is_project_code)
	...
	tab_1.tabpage_3.dw_10.retrieve(is_project_code)
	...
	if cbx_picture_check.checked then
			...
			SELECTBLOB photo 
					 INTO :lb_picture 
					FROM salepre_project WHERE project_code = :is_project_code;			
...
type st_title from w_inheritance`st_title within w_sale_project_m
integer x = 37
integer y = 28
integer width = 1120
integer height = 132
string text = "프로젝트 관리(대형)"
end type

type st_tips from w_inheritance`st_tips within w_sale_project_m
end type

type pb_compute from w_inheritance`pb_compute within w_sale_project_m
...
type pb_print_part from w_inheritance`pb_print_part within w_sale_project_m
boolean visible = false
integer x = 3986
integer y = 256
integer width = 165
...
type pb_close from w_inheritance`pb_close within w_sale_project_m
integer x = 4247
...
type pb_print from w_inheritance`pb_print within w_sale_project_m
integer x = 3863
integer taborder = 60
end type
...
type pb_cancel from w_inheritance`pb_cancel within w_sale_project_m
boolean visible = false
integer x = 2702
integer taborder = 230
end type

type pb_delete from w_inheritance`pb_delete within w_sale_project_m
integer x = 3675
integer taborder = 40
end type
...
elseif messagebox("삭제","삭제하시겠습니까?",question!,okcancel!,2) = 1 then
	...
	elseif i_tabpage = 2 then
		...
		if is_dw = 'dw_6' then
			...
			l_rowcount = dw8.retrieve( ls_scene_code, ls_personal_id)
...
type pb_insert from w_inheritance`pb_insert within w_sale_project_m
integer x = 3488
integer taborder = 30
...
if i_tabpage = 1 then
	...
	tab_1.tabpage_1.dw_5.setcolumn('scene_name')
	tab_1.tabpage_1.dw_5.object.sale_end_yn[ll_row] = 'Y'
...
elseif i_tabpage = 3 then	
	...
	tab_1.tabpage_3.dw_10.object.project_code[ll_row] = is_project_code // is_equip_code 는 dw_1 click 받는다.
...
type pb_retrieve from w_inheritance`pb_retrieve within w_sale_project_m
integer x = 3296
...
type gb_3 from w_inheritance`gb_3 within w_sale_project_m
integer x = 32
integer y = 224
integer width = 4434
integer height = 160
...
type gb_2 from w_inheritance`gb_2 within w_sale_project_m
boolean visible = false
integer x = 1422
integer y = 216
integer width = 2130
...
type gb_1 from w_inheritance`gb_1 within w_sale_project_m
integer x = 3278
integer width = 1189
integer taborder = 0
integer weight = 400
...
type dw_2 from w_inheritance`dw_2 within w_sale_project_m
boolean visible = false
integer x = 1125
integer y = 12
integer width = 119
...
integer height = 728
integer taborder = 61
string dataobject = "d_salepre_project_m"
...
boolean enabled = false
string title = "none"
string dataobject = "d_personal_connection_scene_m3"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
boolean bringtotop = true
string title = "ITEM"
string dataobject = "d_sale_scene_m1"
borderstyle borderstyle = stylelowered!
end type
...
ls_scene_code = tab_1.tabpage_1.dw_5.object.scene_code[1]


dw_7.retrieve(i_personal_id)
dw_8.retrieve(ls_scene_code, string(i_personal_id))
end event
...
string ls_yc_user_id, ls_connect_level
//open(w_personal_w)
...
if isnull(l_personal_id) = false then
     ...
     if l_personal_id <> 0 then
			...
			select   personal_connection.name,
					  personal_connection.tel,
					  personal_connection.kind,
					  personal_connection.company,
					  personal_connection.company_code,
					  personal_connection.level,
					  personal_connection.job,
					  personal_connection.personal_story,
					  personal_connection.write_date,
					  personal_connection.write_user_id,
					  b.user_name write_user_name,
					  personal_connection.modify_date,
					  personal_connection.modify_user_id,
			  ...
			  into  :ls_name, 
					...
					:ls_modify_user_id, 
					:ls_modify_user_name				  
			from personal_connection left outer join login b on personal_connection.write_user_id = b.user_id
											  left outer join login c on personal_connection.write_user_id = c.user_id
...
integer taborder = 80
string title = "none"
string dataobject = "d_personal_connection_scene_m2"
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
...
boolean bringtotop = true
string title = "ITEM"
string dataobject = "d_sale_project_history_m"
