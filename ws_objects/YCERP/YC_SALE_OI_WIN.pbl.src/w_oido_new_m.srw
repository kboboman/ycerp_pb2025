$PBExportHeader$w_oido_new_m.srw
$PBExportComments$거래명세서작성(2019/02/12,조원석)
forward
global type w_oido_new_m from w_inheritance
end type
type gb_5 from groupbox within w_oido_new_m
end type
type gb_4 from groupbox within w_oido_new_m
end type
type dw_3 from datawindow within w_oido_new_m
end type
type dw_inaudit from datawindow within w_oido_new_m
end type
type dw_dodet from datawindow within w_oido_new_m
end type
type dw_8 from datawindow within w_oido_new_m
end type
type dw_5 from datawindow within w_oido_new_m
end type
type em_3 from editmask within w_oido_new_m
end type
type st_2 from statictext within w_oido_new_m
end type
type st_4 from statictext within w_oido_new_m
end type
type cbx_10 from checkbox within w_oido_new_m
end type
type cbx_source from checkbox within w_oido_new_m
end type
type dw_source from datawindow within w_oido_new_m
end type
type cbx_1 from checkbox within w_oido_new_m
end type
type cbx_2 from checkbox within w_oido_new_m
end type
type cbx_temp from checkbox within w_oido_new_m
end type
type st_1 from statictext within w_oido_new_m
end type
type cbx_3 from checkbox within w_oido_new_m
end type
type cbx_4 from checkbox within w_oido_new_m
end type
type pb_1 from picturebutton within w_oido_new_m
end type
type cbx_5 from checkbox within w_oido_new_m
end type
type cbx_6 from checkbox within w_oido_new_m
end type
type dw_4 from datawindow within w_oido_new_m
end type
type cbx_issue from checkbox within w_oido_new_m
end type
type sle_prt from singlelineedit within w_oido_new_m
end type
type dw_6 from datawindow within w_oido_new_m
end type
type cbx_ship from checkbox within w_oido_new_m
end type
type st_left from statictext within w_oido_new_m
end type
type em_left from editmask within w_oido_new_m
end type
type st_9 from statictext within w_oido_new_m
end type
type em_top from editmask within w_oido_new_m
end type
type ddlb_scene_code from dropdownlistbox within w_oido_new_m
end type
type ddlb_ship_no from dropdownlistbox within w_oido_new_m
end type
type st_3 from statictext within w_oido_new_m
end type
type st_5 from statictext within w_oido_new_m
end type
type st_6 from statictext within w_oido_new_m
end type
type ddlb_cust_no from dropdownlistbox within w_oido_new_m
end type
type ddlb_car_no from dropdownlistbox within w_oido_new_m
end type
type st_7 from statictext within w_oido_new_m
end type
type em_1 from editmask within w_oido_new_m
end type
type st_8 from statictext within w_oido_new_m
end type
type cb_1 from commandbutton within w_oido_new_m
end type
type st_10 from statictext within w_oido_new_m
end type
type st_pick from structure within w_oido_new_m
end type
end forward
...
integer width = 7346
integer height = 2716
string title = "거래명세서 작성(w_oido_new_m)"
gb_5 gb_5
gb_4 gb_4
dw_3 dw_3
...
ddlb_ship_no ddlb_ship_no
st_3 st_3
st_5 st_5
st_6 st_6
ddlb_cust_no ddlb_cust_no
ddlb_car_no ddlb_car_no
st_7 st_7
em_1 em_1
st_8 st_8
cb_1 cb_1
...
global w_oido_new_m w_oido_new_m

type variables
public string is_addr
st_print i_print
datawindowchild idwc_scene, idwc_cust, idwc_shipno, idwc_car, idwc_lot, idwc_locno, idwc_car_no
string is_old_time, is_custsql, is_scenesql, is_shipnosql, is_custscenesql, is_locnosql, is_sel_orderno = 'N' , is_carnosql
...
//////////////////////////////////////////////////////////////////////////////
string  ls_left, ls_top, ls_sdate, ls_ship_date, ls_null, ls_req_date
long ll_row
...
dw_3.object.sale_flag[1] = 'D' /*내수*/
//////////////////////////////////////////////////////////////////////////////
sle_prt.enabled = false

dw_3.object.ship_date[1] = today()
em_1.text = string(today(), 'yyyy/mm/dd')

string ls_loc, ls_where, ls_sql, ls_order_no, ls_scene_code
...
ls_ship_date = string(dw_3.object.ship_date[1],'yyyymmdd')

string ls_cust_no, ls_cust_name

DECLARE csr_ct_code01 CURSOR FOR  
select distinct b.cust_no, c.cust_name
 from dbo.V_SHIPDET_PDA a with(nolock) inner join sale b with(nolock) on a.order_no = b.order_no                                          
                                                                        inner join customer c  with(nolock) on b.cust_no  = c.cust_no
where convert(char(8), a.ship_date,112) = :ls_ship_date;
...
//is_custsql = idwc_cust.describe("datawindow.table.select")
//dw_3.insertrow(0)
//
//ls_where  =       " SELECT customer.cust_no,   " + &
//					"         customer.cust_name     " + &
//					"    FROM customer     " + & 
//					"  WHERE customer.open_close   <> ~~'C~~' " + &
//					"     AND (customer.sale_control = ~~'N~~' OR customer.sale_control is null) " + &
//					"    AND customer.cust_no in (   SELECT distinct b.cust_no from dbo.v_shipdet_pda a inner join sale b on a.order_no = b.order_no inner join scene c  on b.scene_code = c.scene_code " + &
//					"   where convert(char(8), a.ship_date,112) = ~~'" + ls_ship_date + "~~' " + " ) "													
////					"   where convert(char(8), a.ship_date,112) = ~~'" + ls_ship_date + "~~' and isnull(a.confirm_flag, ~~'N~~' ) <> ~~'Y~~' " + " ) "								
//ls_sql   = "datawindow.table.select = '" +  ls_where + "'"					

//dw_3.getchild("cust_no", idwc_cust)
//idwc_cust.Modify(ls_sql)
//idwc_cust.SetTransObject(sqlca)
//idwc_cust.Retrieve()
//
//
...
//
//
//dw_3.getchild("ship_no", idwc_shipno)
//idwc_shipno.settransobject(sqlca)
//is_shipnosql = idwc_shipno.describe("datawindow.table.select")
//
//
//dw_3.getchild("car_no", idwc_car_no)
//idwc_car_no.settransobject(sqlca)
//is_carnosql = idwc_car_no.describe("datawindow.table.select")


end event

on w_oido_new_m.create
int iCurrent
call super::create
...
this.ddlb_ship_no=create ddlb_ship_no
this.st_3=create st_3
this.st_5=create st_5
this.st_6=create st_6
this.ddlb_cust_no=create ddlb_cust_no
this.ddlb_car_no=create ddlb_car_no
...
destroy(this.ddlb_ship_no)
destroy(this.st_3)
destroy(this.st_5)
destroy(this.st_6)
destroy(this.ddlb_cust_no)
destroy(this.ddlb_car_no)
destroy(this.st_7)
destroy(this.em_1)
destroy(this.st_8)
destroy(this.cb_1)
...
type pb_save from w_inheritance`pb_save within w_oido_new_m
...
int      li_int,  li_seq,   ll_row,  ll_int,       li_count,  li_saleseq, l_id
...
long  		l_seq, l_seq_no, l_qty, l_intemp, l_count, l_car_seq

string ls_car_no, ls_scene_code, ls_shipamt_case, ls_ship_flag		
string ls_cust_name, ls_req_date, ls_prt
...
if date(ldt_dodate) <> date(ldt_gftoday) then
	...
	if li_today > 0 then
		if MessageBox("경고", "정상적인 작업처리가 아니면 출고일자를 금일자 이후로 발행하면 안됩니다.~n~n" + &
			...
			RETURN
		else	/* 2018.11.06 jowonsuk 익일송장 작성시 가입고 작성 */
			/* 2018.11.7 jowonsuk 1이면 위에서 익일송장발행 메시지 물어봤을때 yes 체크 */		  
			if MessageBox("경고", "익일송장발행으로 (-)가입고를 잡으시겠습니까?~n~n", Exclamation!, OKCancel!, 2) = 1 then
				  l_intemp = 1		  
			end if
...
//			MessageBox("확인", "출고수량은 저장소+장기재고 수량보다 클수 없습니다.") 2018.08.13 jowonsuk 
	...
	////////////////////////////////////////
	ls_fitno  = dw_2.object.customer_fit_no[ll_int]
	...
	if dw_2.object.stock_issue[ll_int] > 0 then
	
		for ll_row = 1 to UpperBound(lstr_where_row[ll_int].qty3)         
			
			l_id = lstr_where_row[ll_int].qty1[ll_row]
	...
	/*2018.11.6 jowonsuk 익일송장 출고시 당일에  수불 미반영됨으로 자동으로 (-)가입고 시킴(일일수불 스케쥴이 다음날 새벽 돌아갈때 이전 가입고 내역 자동 삭제 ) */
	if l_intemp = 1 then /* 2018.11.7 jowonsuk 1이면 위에서 익일송장발행 메시지 물어봤을때 yes 체크 */
		
		li_today = DaysAfter(date(ldt_gftoday), date(ldt_dodate))
		ls_do_no       = dw_1.object.do_no[1]				
		l_today = wf_today()										
					
		ls_dw2_item_no = dw_2.object.item_no[ll_int]
		ls_dw2_qa		 = dw_2.object.qa[ll_int]	
		ls_dw2_uom	 = dw_2.object.uom[ll_int]		
		ls_loc 			 = dw_2.object.loc_no[ll_int]
		l_seq_no          =  dw_2.object.seq_no[ll_int]
		ls_order_no       =  dw_2.object.order_no[ll_int]		
		l_qty                   =  dw_2.object.issue_qty[ll_int]
			
		 SELECT isnull(IDENT_CURRENT ('intemp'), 0) + 1
		 into         :l_seq
		 FROM dual;	
											
		ls_tempno = LeftA(gs_area, 1) + 'T' + string(l_today, 'yyyymmdd') + "-" + RightA( "000" + trim(string(l_seq)),3)
									
		insert into intemp ( intemp_no, 	 user_date, intemp_date,           area_no,    user_id,           item_no, 		   			     qa,        qty,   	 loc_no,  order_no,       seq_no,     type,   flag ,   rem) 
		values                  ( :ls_tempno, 	getdate(),          :l_today,          :gs_area,   :gs_userid,      :ls_dw2_item_no,    :ls_dw2_qa,      :l_qty,    :ls_loc,   :ls_order_no, :l_seq_no, 'DO',    'Z',   :ls_do_no+' 익일송장');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// 2019.02.15 JOWONSUK 상차내역이 송장 완료되면  처뢰 표시 'Y'
////////////////////////////////////////////////////////////////////////////////////////////////////////
string ls_ship_date, ls_ship_no

//ls_req_date = mid(em_1.text, 1,4) + mid(em_1.text, 6,2) + mid(em_1.text, 9,2)
ls_ship_date = MidA(em_1.text, 1,4) + MidA(em_1.text, 6,2) + MidA(em_1.text, 9,2)
...
ls_do_date = string(dw_1.object.do_date[1], "yyyymmdd")
ls_ship_no = LeftA(ddlb_ship_no.text,14)

/*2019.05.16 jowonsuk */
if isnull(ls_ship_no)  or ls_ship_no = '' then
	ls_ship_no = '%'
end if
...
/*	
		2019.05.22 jowonsuk
		라벨사용체크, 
				1. 상차차량 조회, 상차번호 없으면, 차량에 해당하는 상차번호 전부조회 연계된 라벨은 전부 사용된것.
				2. 상차차량 조회, 상차번호 있으면, 해당하는 상차번호 연계된 라벨은 전부 사용된것.
				
		2019.05.24 jowonsuk 수정 order_no 없을때 문제 발생
...
for ll_int = 1 to dw_2.rowcount()
	...
	update label_item     
	 ...
	 where label_no in    (                
									  select b.label_no
										 from shipdet_pda a with(nolock) inner join shipdet_label b on a.ship_no = b.ship_no
									  where convert(char(8), a.ship_date,112) = :ls_ship_date
											and a.car_no = :ls_car_no
											and a.ship_no in (
																		select distinct ship_no
																		from shipdet_pda with(nolock)   
																		where convert(char(8), ship_date,112) = :ls_ship_date
																			 and car_no = :ls_car_no
																			 and order_no = :ls_order_no
																			 and isnull(confirm_flag, 'N') = 'N'
																  )
							);	 
	...
	*/
	 update car_carriage
			set  st_car_amt = :ld_ship_amount,
				...
				do_no 		= :ls_do_no,					/*송장이 두번이상이면 마지막이 덮어 씌움 사용됨 */
				car_status = '03'
	  where car_carriage.cardate in (  
												select distinct convert(char(8),req_date,112) /* 출고의뢰일  jowonsuk 2019.05.22*/
												from shipdet_pda    
												where convert(char(8), ship_date,112) = :ls_ship_date /*실상차일 = 송장발행일 */
													 and car_no = :ls_car_no
													 and order_no = :ls_order_no
													 and isnull(confirm_flag, 'N') = 'N'
											)
		 ...
		 and car_no = :ls_car_no
		 and car_carriage.car_status = '01';											 
		 
	
	update shipdet_pda 
			set confirm_flag = 'Y',										/*N: 안가져온거, Y:가져온거, C:취소*/
				 do_no = :ls_do_no
	where convert(char(8), ship_date,112) = :ls_ship_date
		 and car_no = :ls_car_no
		 and ship_no in (
...
if lb_chk then
	...
	if MessageBox("확인", "생성된 출고번호는 " + ls_do_no1 + " 입니다.~r~n" + &
		...
		if cbx_1.checked = true OR cbx_2.checked = true OR cbx_3.checked = true OR cbx_4.checked = true then
			...
			/*2019-09-09 jowonsuk 프린터 봉투출력시, 기본이 봉투로 바뀌는 현상 방지 로직 */
				...
				if ls_s_sms_yn = 'Y' then
					...
					choose case ls_sms_custno
						case "U968"		// (주)송천건업 동부(성남)
							...
							GF_MMSSEND( ls_template_cd, ls_callback, ls_s_hp_no, '영업사원', ls_s_msg )	// 출고지
					...
					if ls_s_hp_no1 = '' or isnull(ls_s_hp_no1) then
						...
						GF_MMSSEND( ls_template_cd, ls_callback, ls_s_hp_no1, '영업사원', ls_s_msg )	// 출고지
					end if						
	
					// 현장담당
					if ls_s_scene_tel = "" OR isnull(ls_s_scene_tel) then
					else
						GF_MMSSEND( ls_template_cd, ls_callback, ls_s_scene_tel, '현장담당', ls_s_msg )	// 차주정보
					end if
	
					// 구매담당
					if ls_s_cust_tel = "" OR isnull(ls_s_cust_tel) OR LeftA(ls_s_cust_tel,1) <> '0' then
					else
						GF_MMSSEND( ls_template_cd, ls_callback, ls_s_cust_tel,  '구매담당', ls_s_msg )	// 출고지
					end if
				end if
...
type dw_1 from w_inheritance`dw_1 within w_oido_new_m
...
string dataobject = "d_oigiving_m_new"
...
string ls_date, ls_scene_code, ls_company, ls_car_info, ls_car_gbn
...
choose case dwo.name
	case "driver"			// 차량NO
		...
		ls_car = this.object.do_car_no[1]
		
		if isnull(ls_car) or ls_car = '' then
			
			/* 2019.04.25 jowonsuk 유창일경우 반복해서 나오지 않도록 */
			if  idwc_car.getitemstring(idwc_car.getrow(),"driver") = idwc_car.getitemstring(idwc_car.getrow(),"name") then
					this.object.rem[1] = ls_temp + trim(idwc_car.getitemstring(idwc_car.getrow(),"driver")) /*+ " " &
											 + idwc_car.getitemstring(idwc_car.getrow(),"hp"), 43)*/				
			else
			...
			/* 2019.04.25 jowonsuk 배차내역을 조회 한다. */
			  select ( case when car_company = '유창' then '유창' 
										  when car_company = '유로직스' then '유로직스' else '' end ) company, 		  
							 car_no + ' ' + driver_name /*+ ' ' + driver_tel*/ car_info, driver_tel, car_gbn
			  into :ls_company, :ls_car_info, :ls_drhp, :ls_car_gbn
			  from car_carriage
			...
			if isnull(ls_car_info) or trim(ls_car_info) = '' then
				/* 2019.04.25 jowonsuk 차량정보 car_info. */			
				  select  driver + ' ' + name /*+ ' ' + hp*/ car_info, hp, car_kind
			...
			if isnull(ls_company) or ls_company = '' then
				ls_company = dw_1.object.driver[1]
			end if								
			
			if isnull(ls_company) = false or ls_company <> '' then
				ls_car_info = ls_company + ' ' + ls_car_info
			end if					
...
//		end if
		...
		/*2019-11-21 jowonsuk 택배 자동으로 내용 등록 되도록 */
		if trim(data) = '대신(서시화)' then
				this.object.do_dr_hp[1] = '031-431-9112'
				this.object.rem[1] = '대신(서시화) 서시화점'			
				this.object.do_car_no[1] = ''
				this.object.sms_yn[1] = 'N'	
				this.object.do_car_gbn[1] = '01'
				this.object.time_arrival[1] = gf_today()
		end if
			
	case "do_car_no"
				
		ls_date = string(this.object.do_date[1], 'yyyymmdd')
		...
		if data = "Y" then
			for ll_row = 1 to ll_cnt
				...
				if ls_case <> "C" then
					...
					ld_janqoh = dw_2.object.ship_qty[ll_row]
...
event dw_1::rbuttondown;call super::rbuttondown;string ls_car_kind
long   l_amt


if dwo.name  = 'ship_amount' then
	
	ls_car_kind = this.object.do_car_gbn[row]
	
	if isnull(ls_car_kind) or ls_car_kind = '' then
		ls_car_kind = '%'
	end if

	openwithparm(w_whareacar_w, ls_car_kind)
	
	l_amt = long(message.stringparm)
	
	this.object.ship_amount[row] = l_amt
	
end if	
end event

type st_title from w_inheritance`st_title within w_oido_new_m
integer y = 28
integer width = 1010
string text = "거래명세서 작성"
end type

type st_tips from w_inheritance`st_tips within w_oido_new_m
end type

type pb_compute from w_inheritance`pb_compute within w_oido_new_m
integer x = 1605
integer y = 72
integer width = 174
integer height = 124
...
string ls_scene_name, ls_cust_name
string ls_cust_no, ls_req_date, ls_null, ls_scene_no
long   ll_row, ll_cnt
// 거래처, 현장 조회
...
SELECT a.cust_no, b.cust_name,   a.scene_code, c.scene_desc,     a.sale_flag
    INTO :ls_cust,      :ls_cust_name, :ls_scene,        :ls_scene_name, :ls_sale_flag 
  FROM sale a with(nolock) inner join customer b with(nolock) on a.cust_no = b.cust_no
								     inner join scene c with(nolock) on a.scene_code = c.scene_code
...
ls_req_date =  MidA(em_1.text, 1,4) + MidA(em_1.text, 6,2) + MidA(em_1.text, 9,2)

DECLARE csr_ct_code01 CURSOR FOR  
//select distinct convert(char(8), a.req_date,112) req_date, a.order_no, b.cust_no, b.scene_code, a.ship_no
select distinct b.cust_no, c.cust_name
 from dbo.V_SHIPDET_PDA a with(nolock) inner join sale b with(nolock) on a.order_no = b.order_no                                          
                                                                        inner join customer c  with(nolock) on b.cust_no  = c.cust_no
where convert(char(8), a.ship_date,112) = :ls_req_date;
...
select distinct b.scene_code, c.scene_desc scene_name
 from dbo.V_SHIPDET_PDA a with(nolock) inner join sale b with(nolock) on a.order_no = b.order_no                                          
                                                                        inner join scene c  with(nolock) on b.scene_code  = c.scene_code
where convert(char(8), a.ship_date,112) = :ls_req_date;
...
do while sqlca.sqlcode = 0	
	FETCH csr_ct_code02 INTO :ls_scene_no, :ls_scene_name;
	if SQLCA.SQLCODE < 0 then		
		return
	elseif SQLCA.SQLCODE = 100 then
		exit
...
ddlb_cust_no.text = ls_cust + ' ' + ls_cust_name
ddlb_scene_code.text = ls_scene + ' ' +  ls_scene_name
...
type pb_print_part from w_inheritance`pb_print_part within w_oido_new_m
...
type pb_close from w_inheritance`pb_close within w_oido_new_m
integer x = 4325
...
type pb_print from w_inheritance`pb_print within w_oido_new_m
boolean visible = false
integer x = 2295
integer y = 68
...
type pb_cancel from w_inheritance`pb_cancel within w_oido_new_m
integer x = 3941
...
event pb_cancel::clicked;string  ls_ship_date, ls_cust_no, ls_cust_name
...
type pb_delete from w_inheritance`pb_delete within w_oido_new_m
boolean visible = false
integer x = 2569
integer y = 68
...
type pb_insert from w_inheritance`pb_insert within w_oido_new_m
integer x = 3410
integer width = 325
...
type pb_retrieve from w_inheritance`pb_retrieve within w_oido_new_m
integer x = 3749
...
string   ls_custno,   ls_scene_code, ls_where, ls_sql, ls_ship_no, ls_curr_code, ls_null, ls_sdate, ls_car_no, ls_date, ls_company
string   ls_custname, ls_addr, ls_scenename, ls_addr2, ls_order_no, ls_shipto_addr, ls_telno, ls_telnocap, ls_pre_sale_yn
dec      ld_curr_rate
datetime ldt_dodate
long     ll_dw6row, ll_find, ll_row
string ls_car_info, ls_drhp, ls_car_gbn, ls_select_car, ls_sqlerrtext, ls_carno, ls_select_ship_no, ls_select_car_no
string ls_scene, ls_itemno, ls_uom
string ls_case, ls_area, ls_loc, ls_item, ls_qa
decimal l_ship_amount, ld_amount
decimal ld_itemloc_qoh, ld_janqoh
...
//ls_custno    = left(ddlb_cust_no.text,4)
//if isnull(ls_custno) OR trim(ls_custno) = "" then 
//	ls_custno = '%'
//end if 	

//ls_scene_code    = dw_3.object.scene_code[1]
//ls_scene_code = left(ddlb_scene_code.text,11)
//if  isnull(ls_scene_code) or trim(ls_scene_code) = "" then	
//	ls_scene_code = '%'
//end if	

//ls_ship_no    = dw_3.object.ship_no[1]

//ls_car_no    = dw_3.object.car_no[1]

ls_custno  = LeftA(ddlb_cust_no.text,4)
ls_scene_code  = LeftA(ddlb_scene_code.text,11)
ls_car_no = trim(ddlb_car_no.text)
ls_ship_no = LeftA(ddlb_ship_no.text,14)

if isnull(ls_car_no)  or ls_car_no = '' then
	ls_car_no = '%'
end if
...
//ls_ship_no  = trim(ddlb_ship_no.text)
//if  isnull(ls_ship_no) or trim(ls_ship_no) = "" then	
//	ls_ship_no = '%'
//end if	
...
if is_sel_orderno = 'Y' then
	...
	ls_where = 			 "  and convert(char(8), pda.ship_date,112) = ~~'" + ls_sdate + "~~'" + &
							 "  and sale.cust_no = ~~'" + ls_custno + "~~'" + &
							 "  and sale.scene_code =  ~~'" + ls_scene_code + "~~'" + &
							 "  and pda.car_no like  ~~'" +  ls_car_no + "~~'" 					 
end if


ls_sql = is_custscenesql + ls_where
...
if dw_2.rowcount() > 0 then
	...
	for ll_dw6row = 1 to dw_2.rowcount()
		...
		dw_2.object.issue_qty[ll_dw6row]  = dw_2.object.ship_qty[ll_dw6row]
		
		dw_2.object.source[ll_dw6row] = "Y"		
		
		if ls_case <> "C" then
			ld_itemloc_qoh = 0

			dw_2.object.loc_no[ll_dw6row] = ls_loc
			ls_item   = dw_2.getitemstring(ll_dw6row, "item_no")
			ls_qa     = dw_2.getitemstring(ll_dw6row, "qa")
			
			ld_janqoh = dw_2.object.ship_qty[ll_dw6row]
			
			SELECT qoh INTO :ld_itemloc_qoh 
			  FROM itemloc
			 WHERE loc_no  = :ls_loc AND item_no = :ls_item	AND qa = :ls_qa;
			 
			/*jowonsuk 2019-08-26 이론은 맞지만 여기서는 실재 출고수량을 표시한다 그래서 여기서는 아래로직을 주석처리 */ 
			/*
					if ld_itemloc_qoh > ld_janqoh then
						dw_2.setitem(ll_dw6row, "issue_qty",    ld_janqoh )
					else
						if ld_itemloc_qoh <= 0 then
							dw_2.object.loc_no[ll_dw6row]       = ls_loc
							dw_2.object.issue_qty[ll_dw6row]    = 0		
						else
							dw_2.setitem(ll_dw6row, "issue_qty",    ld_itemloc_qoh)	
						end if
					end if
			*/
		else
...
/*		2019.02.07 jowonsuk, 여기서 걸러내는것도 해야함.
		select sum(isnull(issue_qty,0))
		from v_issuereqdet_pda
		where req_date = convert(datetime, convert(char(10), getdate(), 121) )
			and  order_no = :as_order_no
			and  seq_no = :as_seq_no	
*/			
...
//		end if
	...
	dw_1.object.do_chk[1] = 'Y'
//	dw_1.object.driver[1] = dw_2.object.car_no[1]
   ...
   //	dw_1.object.tel_no[1]     = dw_2.object.tel_no[1]
	...
	// 수주폰번호 사용 체크
	SELECT scene_phone INTO :ls_scene_phone  FROM customer WHERE cust_no = :ls_custno;
...
//	dw_1.object.sale_flag[1]   = ls_sale_flag
	...
	//dw_1.object.do_date[1] = dw_3.object.ship_date[1] /*datetime(date(string(today(),'yyyy/mm/dd'))) 2019.05.6 jowonsuk 주석 처리*/
	dw_1.object.do_date[1] = datetime(date(  MidA(ls_sdate, 1,4) + '/' + MidA(ls_sdate, 5,2) + '/' + MidA(ls_sdate, 7,2) )) /*2019.05.15 jowonsuk 수정 */ 
	
	ldt_dodate = datetime(today())
	
	dw_1.object.curr_rate[1] = 1
	if dw_1.object.curr_code[1] <> "WON" then
		SELECT exp_rate INTO :ld_curr_rate FROM currency 
		 WHERE curr_code = :ls_curr_code AND curr_date = :ldt_dodate;
		dw_1.object.curr_rate[1] = ld_curr_rate		  
	end if
	
	/*2019.05.06 jowonsuk 내용추가*/
	ls_date = string(dw_1.object.do_date[1], 'yyyymmdd')
	//ls_car_no = dw_3.object.car_no[1]
		
	  select ( case when car_company = '유창' then '유창' 
					   when car_company = '유로직스' then '유로직스' 
                             else '' end ) company, 
			  ( case when car_company = '유창' then '유창' 
					   when car_company = '유로직스' then '유로직스' 
                             else '' end )  + ' ' + car_no + ' ' + driver_name /*+ ' ' + driver_tel*/ car_info, driver_tel, car_gbn, ship_amount
	...
	dw_1.object.rem[1] = ls_car_info
	dw_1.object.do_car_no[1] = ls_car_no
	dw_1.object.driver[1] = ls_company	

	
	if isnull(ls_drhp) or ls_drhp = "" then
		dw_1.object.do_dr_hp[1] = "00"
	else
		dw_1.object.do_dr_hp[1] = ls_drhp
	end if		
	
	dw_1.object.do_car_gbn[1] = ls_car_gbn	
...
///*2019.04.12 jowonsuk 배차정보 가지고 오기 */
DECLARE carno_cursor CURSOR FOR  		
//	select car_carriage.car_no
//	from car_carriage with(nolock) left outer join car_info with(nolock) on car_carriage.car_no = car_info.driver
//	where car_carriage.cardate = :ls_date
//		and car_carriage.scene_code = :ls_scene_code
//		and car_carriage.car_status = '01';
	SELECT distinct a.car_no
	from dbo.V_SHIPDET_PDA a inner join sale b on a.order_no = b.order_no
										  inner join scene c  on b.scene_code = c.scene_code
	  where convert(char(8), a.ship_date,112) = :ls_date
								  and b.cust_no = :ls_custno
								  and b.scene_code =  :ls_scene_code;
	
	OPEN carno_cursor;

		FETCH carno_cursor INTO :ls_car_no;

		Do While sqlca.sqlcode = 0
	
			ls_select_car_no = ls_select_car_no + "'" + ls_car_no + "'" + ', '
	...
	ls_select_car_no = "driver in ( " + MidA(ls_select_car_no, 1, LenA(ls_select_car_no) - 2) +  " )"


	idwc_car_no.setfilter("")
	idwc_car_no.filter()
	
	idwc_car_no.setfilter(ls_select_car_no)
	idwc_car_no.filter()
	 
	idwc_car_no.SetSort("driver asc")
	idwc_car_no.Sort( )	
	
	/*2019-11-21 jowonsuk 택배는 자동으로 반영 */	
	string ls_ship_case, ls_ship_flag
	
	select ship_case, ship_flag
	into :ls_ship_case, :ls_ship_flag
	from sale
	where order_no = :ls_order_no;
	
	if ls_ship_case = 'T' then
		dw_1.object.driver[1] = '대신(서시화)'
		dw_1.object.rem[1] = '대신(서시화) 서시화점'
		dw_1.object.do_car_no[1] = ''
		dw_1.object.do_dr_hp[1] = '031-431-9112'
		dw_1.object.ship_flag[1] = ls_ship_flag
		dw_1.object.sms_yn[1] = 'N'	
		dw_1.object.do_car_gbn[1] = '01'
		dw_1.object.time_arrival[1] = gf_today()		
	end if	
...
//			if ls_uom = "M" then
//				ls_qa  = left(dw_2.object.qa[ll_row],13)
//
//
...
//									FROM dodet 
//										  LEFT OUTER JOIN do ON dodet.do_no = do.do_no
...
//			end if
			...
			ld_amount = dw_2.object.doprice[ll_row] 
...
type gb_3 from w_inheritance`gb_3 within w_oido_new_m
integer x = 32
integer y = 224
integer width = 5723
integer height = 140
...
type gb_2 from w_inheritance`gb_2 within w_oido_new_m
integer y = 352
integer width = 4219
integer height = 616
...
type gb_1 from w_inheritance`gb_1 within w_oido_new_m
integer x = 3383
integer width = 1166
integer taborder = 90
long backcolor = 79220952
...
type dw_2 from w_inheritance`dw_2 within w_oido_new_m
event ue_1 pbm_custom01
integer x = 55
integer y = 1016
integer width = 4773
...
boolean enabled = false
string dataobject = "d_oidodet_new_m"
...
choose case dwo.name
	case "source"	
		...
		if data = "Y" then
			...
			if isnull(dw_1.object.ship_flag[1]) or dw_1.object.ship_flag[1] = '' then 
			
					SELECT top 1 ship_flag INTO :ls_shipflag FROM issuereqdet WHERE sale_no = :ls_saleno ORDER BY req_date desc;
					
					/* jowonsuk 2019-10-16 출고의뢰에서 운반비 지정을 안하면 거래명세서에서 그냥 넘어가게 수정 */
					if isnull(ls_shipflag) or ls_shipflag = '' then
		...
		if GF_PERMISSION("거래명세서_가입고대체", gs_userid) = "Y" then			
			...
			choose case gs_print_control
				...
				case "longstock"
					...
					// 다중추가 화면								  //
					///////////////////////////////////////////////////////////////////
					lstr_where2.str1[1]    = this.object.item_no[row]
					lstr_where2.str2[1]    = this.object.qa[row]
					lstr_where2.str3[1]    = this.object.order_no[row]		
					lstr_where2.qty1[1]    = this.object.seq_no[row]
					lstr_where2.chk	      = this.object.loc_no[row]
					
					ls_do_no				 =   dw_1.object.do_no[1]		
					ls_sale_no = this.object.order_no[row]
					l_seq_no = this.object.seq_no[row]
			
					SELECT top 1 a.oi_no
					into        :l_oi_no
					FROM dodet a inner join do b on a.do_no = b.do_no
					where convert(char(8),b.do_date,112)  = convert(char(8),getdate(),112)
					and a.sale_no = :ls_sale_no
					and a.seq_no =  :l_seq_no
					and a.do_no = :ls_do_no;
			
					lstr_where2.str4[1]    = ls_do_no
					lstr_where2.qty2[1]    = l_oi_no
			
			
					OpenWithParm( w_itemstockdet_w, lstr_where2 )
					...
					this.object.stock_issue[row] = l_stock_qty								
			end choose
		else
...
choose case dwo.name
	case "stock_issue"	
		...
		lstr_where.str4[1]    = ls_do_no
		lstr_where.qty2[1]    = l_oi_no
...
boolean enabled = false
string dataobject = "d_oicustgiving_new_s"
boolean border = false
boolean livescroll = true
end type

event itemchanged;// 거래처, 현장 조회
string ls_cust_no, ls_scenehp, ls_where, ls_sql, ls_null, ls_req_date, ls_scene_code, ls_ship_date, ls_ship_no, ls_cust_name
long   ll_cnt
...
ls_req_date = string(dw_3.object.ship_date[1],"yyyymmdd")
...
select distinct b.cust_no, c.cust_name
 from dbo.V_SHIPDET_PDA a with(nolock) inner join sale b with(nolock) on a.order_no = b.order_no                                          
                                                                    inner join customer c  with(nolock) on b.cust_no  = c.cust_no
where convert(char(8), a.req_date,112) = :ls_req_date;
...
//	case "ship_date"
//			
//		//ls_ship_date =  string(data, "yyyymmdd")
//		
//		idwc_cust.reset()
//		idwc_scene.reset()
//		idwc_shipno.reset()
...
//		this.object.car_no[1] = ls_null
//		this.object.ship_no[1] = ls_null				
//		// 선택한 거래처의 현장만 보여주기
//		
//		ls_where  =       " SELECT customer.cust_no,   " + &
//							"         customer.cust_name     " + &
...
//
//		
//		ls_where  =       " SELECT scene.scene_code,  " + &
//							"                 scene.scene_desc  " + &
//							"    FROM scene " + &
//							"   where scene.scene_code in (   SELECT distinct b.scene_code from dbo.V_SHIPDET_PDA a inner join sale b on a.order_no = b.order_no inner join scene c  on b.scene_code = c.scene_code " + &							
//							"   where convert(char(8), a.ship_date,112) = ~~'" + ls_ship_date + "~~'" + &
//							"   and b.cust_no = ~~'" + ls_cust_no + "~~'" + &
//							" ) "								
////							"   and isnull(a.confirm_flag, ~~'N~~') <> ~~'Y~~'" + &							
//		ls_sql   = "datawindow.table.select = '" +  ls_where + "'"					
//		
//		dw_3.getchild("scene_code", idwc_scene)
...
//		// 선택한 거래처의 현장만 보여주기
//
//   		ls_where = 	 " SELECT distinct a.ship_no  " + &
//	 					 "	from v_SHIPDET_PDA a inner join sale b on a.order_no = b.order_no " + &
//                                "   inner join scene c on b.scene_code = c.scene_code " + &
//						 "   where convert(char(8), a.ship_date,112) = ~~'" + ls_ship_date + "~~'" + &
//						 "   and b.cust_no = ~~'" + ls_cust_no + "~~'" + &														  
//						 "   and b.scene_code = ~~'" + ls_scene_code + "~~'" /*+ &
...
//		
//		dw_3.getchild("ship_no", idwc_shipno)
//		idwc_shipno.modify(ls_sql)
//		idwc_shipno.settransobject(sqlca)
//		idwc_shipno.retrieve()
//		
//	case "ship_no"		
//		
//		idwc_car_no.reset()		
//		
//		ls_cust_no = this.object.cust_no[1]		
//		ls_scene_code = this.object.scene_code[1]		
//		ls_ship_no = data			
//		this.object.car_no[1] = ls_null		
//
//		// 선택한 거래처의 현장만 보여주기
//
//   		ls_where = 	 " SELECT distinct a.car_no  " + &
//	 					 "	from v_SHIPDET_PDA a inner join sale b on a.order_no = b.order_no " + &
//                                "   inner join scene c on b.scene_code = c.scene_code " + &
//						 "   where convert(char(8), a.ship_date,112) = ~~'" + ls_ship_date + "~~'" + &
//						 "   and b.cust_no = ~~'" + ls_cust_no + "~~'" + &														  
//						 "   and b.scene_code = ~~'" + ls_scene_code + "~~'" + &
//						 "   and a.ship_no = ~~'" + ls_ship_no + "~~'"
//								
//		ls_sql   = "datawindow.table.select = '" +  ls_where + "'"					
//		
//		dw_3.getchild("car_no", idwc_car_no)
//		idwc_car_no.modify(ls_sql)
//		idwc_car_no.settransobject(sqlca)
...
event selectionchanged;string ls_car_no, ls_req_date, ls_cust_no, ls_scene_code
...
ls_req_date = MidA(em_1.text, 1,4) + MidA(em_1.text, 6,2) + MidA(em_1.text, 9,2)
ls_cust_no  = LeftA(ddlb_cust_no.text, 4)
ls_scene_code  = LeftA(this.text,11)

OPEN csr_ct_code01;

do while sqlca.sqlcode = 0	
	FETCH csr_ct_code01 INTO :ls_car_no;
	if SQLCA.SQLCODE < 0 then		
		return
	elseif SQLCA.SQLCODE = 100 then
		exit
...
event selectionchanged;//string ls_ship_no, ls_req_date, ls_cust_no, ls_scene_code, ls_car_no
//
//DECLARE csr_ct_code01 CURSOR FOR  
////select distinct convert(char(8), a.req_date,112) req_date, a.order_no, b.cust_no, b.scene_code, a.ship_no
...
//
//ls_req_date = mid(em_1.text, 1,4) + mid(em_1.text, 6,2) + mid(em_1.text, 9,2)
//ls_cust_no  = left(ddlb_cust_no.text, 4)
//ls_scene_code  = left(ddlb_scene_code.text,11)
//ls_car_no  = trim(this.text)
//
//OPEN csr_ct_code01;
//
//do while sqlca.sqlcode = 0	
//	FETCH csr_ct_code01 INTO :ls_ship_no;
//	if SQLCA.SQLCODE < 0 then		
//		return
//	elseif SQLCA.SQLCODE = 100 then
//		exit
//	end if
//	
//	ddlb_ship_no.AddItem( ls_ship_no)
//
//loop
//
//close csr_ct_code01;
...
// 거래처, 현장 조회
string ls_cust_no, ls_cust_name, ls_req_date, ls_null
long   ll_cnt

setnull(ls_null)

ls_req_date =  MidA(this.text, 1,4) + MidA(this.text, 6,2) + MidA(this.text, 9,2)
...
event clicked;
string  ls_ship_date, ls_cust_no, ls_cust_name
