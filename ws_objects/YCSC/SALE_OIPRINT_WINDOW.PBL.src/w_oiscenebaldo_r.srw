...
If gs_level = 'B' Then
	...
	string ls_custname
		...
		If ist_code.order_flag <> '' Then
			select cust_name into :ls_custname from customer where cust_no = :ist_code.order_flag;
		Else
			ls_custname = '전체'
		End If
		ls_desc = '출고기간:' + string(ist_code.start_date) + '-' + string(ist_code.end_date) + '판매거래처:' + ls_custname
