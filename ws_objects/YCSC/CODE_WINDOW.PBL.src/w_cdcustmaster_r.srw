...
this.Control[iCurrent+43]=this.gb_6
end on
...
if ll_dw4cnt < 1 then
	...
	dw_2.getchild("dw_1", idwc_opeom)	// 전전년 
	idwc_opeom.insertrow(0)
	idwc_opeom.SetItem(1, "customer_cust_name", ls_custnm)
	idwc_opeom.SetItem(1, "user_name", ls_username)
	idwc_opeom.SetItem(1, "eom_year",  date(string(li_year) + '/01/01'))
end if
...
//if currentrow < 1 Or currentrow > this.rowcount() then RETUR
...
dw_9.SetSort("customer_salesman A, toplevel A, cust_sort A, custname_sort A")	
dw_9.Sort()
dw_9.groupcalc()
///////////////////////////////////////////////////////////////////
	...
	for ll_col = 1 to 2
		...
		if isnull(ls_custno) OR ls_custno = "" then
			...
			if isnull(ll_found) OR ll_found < 1 then
				...
				if LeftA(dw_9.object.customer_cust_name[ll_found],1) = "*" then
					//
				else
					dw_9.object.customer_cust_name[ll_found] = "*" + dw_9.object.customer_cust_name[ll_found]
				end if
...
boolean vscrollbar = true
integer limit = 7
string item[] = {"%  전체","A大 등급","A中 등급","A小 등급","B大 등급","B中 등급","B小 등급","C大 등급","C中 등급","C小 등급"}
borderstyle borderstyle = stylelowered!
end type
