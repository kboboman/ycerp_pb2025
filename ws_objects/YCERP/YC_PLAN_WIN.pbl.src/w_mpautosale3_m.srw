...
type cb_4 from commandbutton within w_mpautosale3_m
end type
type st_1 from statictext within w_mpautosale3_m
end type
type st_info from statictext within w_mpautosale3_m
end type
type st_2 from statictext within w_mpautosale3_m
end type
type st_vertical from u_splitbar_vertical within w_mpautosale3_m
end type
end forward
...
for ll_row = 1 to arg_cnt
	...
	if isnull(dw_1.object.itemjaego_item_no[ll_row]) OR trim(dw_1.object.itemjaego_item_no[ll_row]) = "" then
		...
		// 재고 물품인경우에는 saledet.sol_qty에 가용량을 넣지 않고 
		...
		dw_4.object.area_no[ll_ins]   = "T0001"
...
//	ll_cnt = dw_3.retrieve(ls_item, ls_like)
	...
	if ll_cnt < 1 then
		...
		dw_4.object.area_no[ll_ins]   = "T0001"
		dw_4.object.user_id[ll_ins]   = gs_userid
		dw_4.object.rout_msg[ll_ins]  = "오류  - [" + ls_itemnm + "][" + ls_qa1 + "] 물품의 공정이 등록되지 않았습니다. " + STRING(now())
		CONTINUE
	else
	   for j = 1 to ll_cnt
			lr_qpa       = dw_3.object.qpa[j]     
					...
					if sqlca.sqlcode < 0 then
						...
						if ll_int3 > 0 then
							...
							if sqlca.sqlcode < 0 then
								...
								dw_4.object.area_no[ll_ins]   = "T0001"
								dw_4.object.user_id[ll_ins]   = gs_userid
								dw_4.object.rout_msg[ll_ins]  = "오류5-1  - routtempnew DELETE 중 오류가 발생하였습니다 전산실로 연락바랍니다." + STRING(now())
							...
							if sqlca.sqlcode < 0 then
								...
								dw_4.object.area_no[ll_ins]   = "T0001"
								dw_4.object.user_id[ll_ins]   = gs_userid
								dw_4.object.rout_msg[ll_ins]  = "오류5-2  - routtempnew INSERT 중 오류가 발생하였습니다 전산실로 연락바랍니다" + STRING(now())
				...
				choose case sqlca.sqlcode
					case 0
						...
						if lr_planqty <= lr_qoh then
							...
							// 중간공정의 물품이 재고물품관리(itemjaeho)에 있으면서 재고수량이 부족하면 전체생산계획 취소
...
dw_4.object.area_no[ll_dw4row]   = "T0001"
...
if dw_1.rowcount() > 0 then
	// 재고물품(itemjaego) 이면 무조건 생산계획을 생성하지 않는다.
...
// 작업내역
///////////////////////////////////////////////////
dw_4.retrieve( "T0001", ls_sdate, ls_edate )
///////////////////////////////////////////////////
