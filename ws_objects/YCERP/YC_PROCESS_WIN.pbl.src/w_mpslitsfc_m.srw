...
long ll_row, ll_losscnt, l_len, ll_count, l_seq
int li_col
string ls_coil, ls_date, ls_loc, ls_item, ls_code1, ls_date_pre
...
if ll_losscnt > 0 then
   ...
   END CHOOSE
	...
	ls_date_pre = ls_code1 + 'L' + MidA(string(dw_2.object.work_date[1], 'yyyymmdd'), 3)
	li_col = 11
else
	ls_date = "YS" + MidA(string(dw_2.object.work_date[1], 'yyyymmdd'), 3) + "%"
	ls_date_pre = "YS" + MidA(string(dw_2.object.work_date[1], 'yyyymmdd'), 3)
	li_col = 12
end if
...
if dw_1.object.sheet[arg_dw1row] = 'S' then
	...
	SELECT :ls_date_pre + ( case when max(convert(decimal, substring(sheet_no, 9, len(sheet_no))))  < 10 then convert(char(1), max(convert(decimal, substring(sheet_no, 9, len(sheet_no)))) )
	  ...
	  FROM sheetmst
	where substring(sheet_no,1,8) = :ls_date_pre
	...
	SELECT :ls_date_pre + ( case when max(convert(decimal, substring(coil_no, 9, len(coil_no))))  < 10 then convert(char(1), max(convert(decimal, substring(coil_no, 9, len(coil_no))))  )
	...
	where flag = 'S'
	   AND loc_no = :ls_loc
	   AND substring(coil_no,1,8) = :ls_date_pre
...
for ll_row = 1 to tab_1.tabpage_2.dw_4.rowcount()
	if not isnull(tab_1.tabpage_2.dw_4.object.coil_no[ll_row]) then
		if LeftA(ls_coil, 2) = LeftA(tab_1.tabpage_2.dw_4.object.coil_no[ll_row], 2) &				
			and long(MidA(ls_coil,9,4)) < long(MidA(tab_1.tabpage_2.dw_4.object.coil_no[ll_row],9, 4)) then //				and ls_coil < left( tab_1.tabpage_2.dw_4.object.coil_no[ll_row], li_col -1 ) then				
...
for ll_row = 1 to ll_rowcount
	...
	if ls_oc = "C" then
		...
		VALUES (getdate(), :ld_sfc_date, :ls_witem_no, :ls_wqa, :ls_wloc, 'IW', :ls_order,
			    0, :li_wunit_qty * :li_wcnt * -1, 0, :ls_wc_no, :ls_w_coil_no, "스리팅(ERP)삭제", :li_wunit_qty, :li_seq, :li_wunit_qty, :gs_userid);
		if sqlca.sqlcode < 0 then
	...
	VALUES (getdate(), :ld_sfc_date, :ls_item_no, :ls_qa, :ls_loc, 'RC', :ls_order,
		    :ld_unit_qty * :li_cnt * -1, 0, 0,	:ls_wc_no, :ls_coil,"스리팅(ERP)삭제", :ld_unit_qty, :li_seq, :ld_unit_qty * :li_cnt, :gs_userid);
	if sqlca.sqlcode < 0 then
