...
event clicked;// 확인
string ls_req_date, ls_rem, ls_req_text
...
/* 2019.09.02 jowonsuk 한미란 차장 요청으로 삭제 */
...
SELECT :ls_req_date, :ls_old_userid, :li_req_seq, accept_user, rem, req_text, accept_area, 'N' 
  FROM issuereq 
...
SELECT :ls_req_date, :ls_old_userid, :li_req_seq, req_flag, seq, cust_no, scene_code, add_scene, sale_no, item_name, uom, issue_flag, issue_schedule, ship_flag, ship_case, tel, rem 
  FROM issuereqdet
 WHERE req_date = :ls_old_req_date
   AND salesman = :ls_old_userid
   AND req_seq  = :li_old_req_seq
...
COMMIT;
MessageBox('확인', '복사완료.', exclamation!)

w_oiissuereq_m.em_1.text = em_1.text		// 변경출고일자
w_oiissuereq_m.em_2.text = em_3.text		// 변경차수
