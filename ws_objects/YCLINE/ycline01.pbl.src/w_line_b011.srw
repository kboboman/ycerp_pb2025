...
ldt_date = w_line.dw_r.object.date[1]  /*jowonsuk 주석처리 work_date 반영 2018-05-29 */

dw_1.accepttext()

lt_Time          = time(MidA(string(today(),'yyyy/mm/dd hh:mm:ss:fff'),11))
//ldt_date 		 = datetime(date(dw_1.object.work_date[al_row]), lt_Time)				 /*jowonsuk 주석처리 work_date 반영 2018-05-29 */
...
VALUES (:ldt_sfc_dt,:ldt_date,:ls_assy_item,:ls_assy_qa,:ls_plan_no,:ldb_work_qty,0,:ls_loc,'RP',0,:ls_wc_no,"현장작업일보[YSP]");
if sqlca.sqlcode < 0 then
	ls_sqlerrtext = sqlca.sqlerrtext
	rollback;
...
//
//	if li_cnt < 2 then
//		ls_qa        = left(trim(w_line.dw_1.object.qa[ll_selrow]), 13)
...
//for ll_row = 1 to ll_dw1cnt
//	ls_itemno   = dw_1.object.item_no[ll_row]
//	ls_qa13     = left(dw_1.object.qa[ll_row], 13) + "X00000"
//	
