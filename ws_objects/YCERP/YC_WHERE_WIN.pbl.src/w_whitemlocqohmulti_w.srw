$PBExportHeader$w_whitemlocqohmulti_w.srw
$PBExportComments$물품선택다중(저장소재고)선택(2003/09/02,이인호)
forward
global type w_whitemlocqohmulti_w from window
end type
type cb_23 from commandbutton within w_whitemlocqohmulti_w
end type
type dw_7 from datawindow within w_whitemlocqohmulti_w
end type
type tab_1 from tab within w_whitemlocqohmulti_w
end type
...
type cb_9 from commandbutton within w_whitemlocqohmulti_w
end type
type cbx_2 from checkbox within w_whitemlocqohmulti_w
end type
type cb_8 from commandbutton within w_whitemlocqohmulti_w
end type
type cb_6 from commandbutton within w_whitemlocqohmulti_w
end type
type cb_2 from commandbutton within w_whitemlocqohmulti_w
end type
type cb_1 from commandbutton within w_whitemlocqohmulti_w
end type
type dw_1 from datawindow within w_whitemlocqohmulti_w
end type
type gb_4 from groupbox within w_whitemlocqohmulti_w
end type
type dw_loc from datawindow within w_whitemlocqohmulti_w
end type
end forward
...
string title = "저장소별품목선택(w_whitemlocqohmulti_w)"
...
global w_whitemlocqohmulti_w w_whitemlocqohmulti_w
...
on w_whitemlocqohmulti_w.create
...
on w_whitemlocqohmulti_w.destroy
destroy(this.cb_23)
destroy(this.dw_7)
destroy(this.tab_1)
destroy(this.cb_9)
...
//dw9.object.item_no.TabSequence = 0
...
if UpperBound(istr_where.str1) > 0 then
	if not isnull(istr_where.str1[1]) and trim(istr_where.str1[1]) <> '' then
		...
		tab_1.tabpage_1.cb_4.triggerevent(clicked!)		// 조회
...
dw_7.accepttext()
ls_name = trim(UPPER(dw_7.object.cust_no[1]))
...
ELSE
	...
	ls_filter = '%' + ls_name + '%'
	is_where = " qa like '" + ls_filter + "'"
	
end if	

dw3.SetFilter(is_where)
dw3.Filter( )
end event
...
tab_1.tabpage_1.dw_3.SetSort("qa A ")
tab_1.tabpage_1.dw_3.Sort( )

ll_found = tab_1.tabpage_1.dw_3.Find("qa > '" + data + "'", 1, tab_1.tabpage_1.dw_3.RowCount())
...
event selectionchanged;// 품목, 수주품목, 출고품목, 일위대가, 물품요청
...
if this.object.item_no[row] = 'M2Z0LIQKZZ' or this.object.item_no[row] = 'M2Z0POWZZZ' then
	dw_7.object.cust_no[1] = ''
	dw3.SetFilter("")
	dw3.Filter( )		
	dw_7.visible = true
	cb_23.visible = true
...
//if ls_locno = 'WS00000011' THEN
//	ls_locno = 'WS00000011'
//ELSE
//	ls_locno = '%'
//END IF	
dw_3.retrieve(is_loc, ls_name )
//dw_3.retrieve(ls_locno, ls_name )
dw_3.setredraw(true)
...
End If
if messagebox("확인","  규격 = [" + ls_new_qa + "]을 저장하시겠습니까?", &
...
//	  dw_3.retrieve(ls_locno, ls_item_no )
	ll_row = dw_3.Find( "qa = '" + ls_qa + "'", 1, dw_3.RowCount())
	IF ll_row > 0 THEN
		dw_3.ScrollToRow(ll_row)
	end if
...
event constructor;em_5.text = 'HD' + string(today(),'yyyyMMdd')
em_5.SelectText(9, 2)
em_5.setfocus()
...
for ll_row = 1 to dw10.rowcount()				
	...
	SELECT isnull(qoh,0) INTO :ld_qoh FROM itemloc WHERE loc_no = :is_loc AND item_no = :ls_itemno AND qa = :ls_qa;
	...
	if dw10.object.uom[ll_row] = 'M' then
		...
		ld_temp      = dec(RightA(dw10.object.asc_qa[ll_row], 5)) * 0.001
...
/*  물품요청 헤드
  SELECT ask_head.ask_no     
       , ask_head.ask_date   
       , ask_head.proc_flag  /* Y(완료), N(요청중), P(부문) */
       , ask_head.ask_user
       , login.user_name
    FROM ask_head  
         LEFT OUTER JOIN login  ON ask_head.ask_user = login.user_id
   WHERE ( convert(char(10),ask_head.ask_date,111)  between :as_sdate and :as_edate ) 
     AND ( ask_head.t_area         = :as_area )   
   ORDER BY ask_head.ask_date desc,   
            ask_head.ask_no   asc   
*/
end event
	...
	case 2		// 수주품목
		for ll_row = 1 to dw5.rowcount()
			if dw5.IsSelected(ll_row) = true then
				...
				dw_1.object.item_name[ll_cnt] = dw5.object.item_name[ll_row]
				dw_1.object.qoh[ll_cnt]       = dw5.object.qoh[ll_row]
				dw_1.object.rem[ll_cnt]       = dw5.object.rem[ll_row]
