...
this.Control[iCurrent+26]=this.pb_add
this.Control[iCurrent+27]=this.cbx_2
this.Control[iCurrent+28]=this.dw_7
this.Control[iCurrent+29]=this.dw_5
this.Control[iCurrent+30]=this.tab_1
this.Control[iCurrent+31]=this.pb_del
end on
...
string   ls_rem1, ls_rem2, ls_rem3, ls_rem4, ls_rem5
...
for ll_row = 1 to dw_5.rowcount()
	...
	if isnull(dw_5.object.rem[ll_row]) then
		MessageBox('확인','산출근거가 누락되었습니다.~n~n확인 후 다시 등록하여 주십시요.',Stopsign!)
		dw_5.setcolumn("rem")
		dw_5.setfocus()
		return
	end if
	...
	dw_5.object.item_no[ll_row]    = 'PZZZPP4ZZZ'				// 픽셀
...
if isNULL(ls_groupno) or ls_groupno = "" then	// 신규모드
	...
	// 수정모드	(할인일자 변경한 경우)
	if ld_date <> date(MidA(ls_groupno,3,8)) then
