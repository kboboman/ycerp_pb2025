...
type dw_12 from datawindow within w_apinspupdate_m
end type
type cbx_inupt from checkbox within w_apinspupdate_m
end type
type cb_inupt from commandbutton within w_apinspupdate_m
end type
type gb_4 from groupbox within w_apinspupdate_m
end type
type pb_1 from picturebutton within w_apinspupdate_m
end type
type st_vertical from u_splitbar_vertical within w_apinspupdate_m
end type
type st_horizontal from u_splitbar_horizontal within w_apinspupdate_m
end type
type cb_comuom from commandbutton within w_apinspupdate_m
end type
type st_2 from statictext within w_apinspupdate_m
end type
type ddlb_fld from dropdownlistbox within w_apinspupdate_m
end type
type ddlb_dwtitles from dropdownlistbox within w_apinspupdate_m
end type
type st_5 from statictext within w_apinspupdate_m
end type
type ddlb_op from dropdownlistbox within w_apinspupdate_m
end type
type sle_value from singlelineedit within w_apinspupdate_m
end type
type cb_6 from commandbutton within w_apinspupdate_m
end type
type cb_7 from commandbutton within w_apinspupdate_m
end type
type cb_8 from commandbutton within w_apinspupdate_m
end type
type cb_9 from commandbutton within w_apinspupdate_m
end type
type st_6 from statictext within w_apinspupdate_m
end type
type cb_10 from commandbutton within w_apinspupdate_m
end type
...
string is_dw, is_supp_no, is_enable_unit_qty = 'N', is_filename, is_rem
int    ii_row, ii_Sheet
...
public function boolean wf_jajae_out_delete (long arg_cnt)
end prototypes
...
string ls_item, ls_qa, ls_type, ls_destination, ls_serial_id, ls_rem, ls_order_no, ls_newloc
long   l_wlen, l_origin_wlen
decimal L_rcpt_qty, L_issue_qty, l_cost, l_unit_qty, l_com_qty, l_cnt
datetime ldt_insp_date
string ls_subcon, ls_coilsliting_coil_no, ls_ingubun
...
//				AND cnt = :li_seq;
			     ls_subcon    = trim(dw_2.object.subcon[li_int])
				/*2020-07-20 jowonsuk purdet에 입고시 자재사용 구분자를 넣음. 입고시 ingubun을 체크하면 자재사용처리를 해야함. 즉 자재출고를 일으킴. ingubun이  'Y' 일때  */
			     ls_ingubun    = trim(dw_2.object.ingubun[li_int])
				  
			     ls_item    = trim(dw_2.object.item_no[li_int])
			     ls_qa      = trim(dw_2.object.qa[li_int])

				l_unit_qty = dw_2.getitemdecimal(li_int, 'unit_qty', primary!, true)
				ld_qty  = l_unit_qty * dw_2.object.cnt[li_int]				
				L_rcpt_qty = ld_qty*(-1)
				L_issue_qty = ld_qty*(-1)
				l_cost = dw_2.object.price[li_int]
				ls_destination = dw_2.object.supp_no[li_int]
				ls_newloc  = trim(dw_2.object.loc_no[li_int])			
				ls_serial_id = dw_2.object.coil_no[li_int]
				l_cnt = dw_2.object.oi_no[li_int]
				l_com_qty = dw_2.object.comqty[li_int]
				ls_rem = '단중수정(기존값처리)'						/*2019-08-08 jowonsuk  행삭제인지, 전체삭제인지 rem에 등록하기 위해서 wf_delete 함수 호출 직전에 is_rem에 텍스트 지정 */
																			/* 행삭제: 행입고삭제(erp발주입고수정), 전체삭제: 입고삭제(erp발주입고수정)*/
																																										
				ldt_insp_date = dw_2.object.insp_date[li_int]	
				ls_order_no  = ls_inspno
				
				ls_coilsliting_coil_no = dw_2.object.coilsliting_coil_no[li_int]				
				
				/*2019.08.22 jowonsuk 자재가공, 외주가공, 외주스리팅/시트 의 inaudit 처리 기존에 누락, 반영작업.시작  */
				IF ls_ingubun = 'Y' then
					ls_subcon = 'P'									 
				end if
				
				if ls_subcon = 'J' OR ls_subcon = 'P' or ls_subcon = 'C' then	
					/*2020-07-24 jowonsuk 내용 추가 */
					IF ls_ingubun <> 'Y' then					
					
						ls_type = 'IO'
						
						if ls_subcon = 'C' then
							ls_serial_id = ls_coilsliting_coil_no
						end if
						 ...
						 INSERT INTO inaudit ( inaudit_time,    inaudit_date,   inaudit_item,       qa,         order_no,                         loc_no,	      inaudit_type,         rcpt_qty,            issue_qty,     cost,             destination,       serial_id,      rem,       unit_qty,     cnt,     com_qty,      user_id )  
										 VALUES (      getdate(), :ldt_insp_date,         :ls_item, :ls_qa,    :ls_order_no,             :ls_destination,               :ls_type,                   0,        :L_issue_qty,  :l_cost,             :ls_newloc, :ls_serial_id, :ls_rem,   :l_unit_qty, :l_cnt,         :ld_qty, :gs_userid );
				...
				/*2019.08.22 jowonsuk 자재가공, 외주가공의 inaudit 처리 기존에 누락, 반영작업.끝  */				
				
				ls_type = dw_2.object.iotype[li_int]				
				ls_serial_id = dw_2.object.coil_no[li_int]

				INSERT INTO inaudit (             inaudit_time,                inaudit_date,   inaudit_item,           qa,         order_no,        loc_no,              inaudit_type,           rcpt_qty,        issue_qty,     cost,       destination,       serial_id,      rem,       unit_qty,     cnt,     com_qty,      user_id )  
										VALUES (      getdate(),              :ldt_insp_date,         :ls_item,      :ls_qa,    :ls_order_no,  :ls_newloc,           :ls_type,          	 :L_rcpt_qty,                     0,  :l_cost, :ls_destination, :ls_serial_id, :ls_rem,   :l_unit_qty, :l_cnt,        :ld_qty, :gs_userid );
				...
				l_unit_qty = dw_2.object.unit_qty[li_int]
				ld_qty  = l_unit_qty * dw_2.object.cnt[li_int]				
				L_rcpt_qty = ld_qty
				L_issue_qty = ld_qty
				ls_rem = '단중수정(신규)'			
				...
				ls_type = dw_2.object.iotype[li_int]				
				ls_serial_id = dw_2.object.coil_no[li_int]
				/*2020-07-24 jowonsuk 내용 수정 기존 issue_qty가 되어 있었음 */				
				 INSERT INTO inaudit ( inaudit_time,    inaudit_date,   inaudit_item,       qa,         order_no,        loc_no,    inaudit_type,             rcpt_qty,      issue_qty,     cost,       destination,       serial_id,      rem,       unit_qty,     cnt,     com_qty,      user_id )  
								     VALUES (  getdate(), :ldt_insp_date,         :ls_item, :ls_qa,    :ls_order_no,  :ls_newloc,           :ls_type,       :L_rcpt_qty,                    0,  :l_cost,  :ls_destination, :ls_serial_id, :ls_rem,   :l_unit_qty, :l_cnt,        :ld_qty, :gs_userid );										
...
datetime ldt_insp_date
string ls_coilsliting_coil_no
string ls_check_sum, ls_check
string ls_witem_no, ls_oc, ls_etc 
int 		li_wunit_qty
dec{2} ld_length
long l_count = 0
string ls_product_no, ls_ingubun
...
ls_ingubun  = trim(dw_2.object.ingubun[arg_cnt])

ls_inspno  = trim(dw_2.object.insp_no[arg_cnt])		// 입고번호
li_oino    = dw_2.object.oi_no[arg_cnt]

/* 2019-08.08 jowonsuk inaudit inset 부분을 datawindows에서 가져와서 inaudit에 insert 시작   */
if dw_1.object.insp_insp_case[1] = 'A' then
	
	L_rcpt_qty = dw_2.object.rcpt_qty[arg_cnt]*(-1)
	L_issue_qty = dw_2.object.rcpt_qty[arg_cnt]*(-1)
...
//	ls_serial_id = dw_2.object.coil_no[arg_cnt]
	...
	ls_rem = is_rem										/*2019-08-08 jowonsuk  행삭제인지, 전체삭제인지 rem에 등록하기 위해서 wf_delete 함수 호출 직전에 is_rem에 텍스트 지정 */
																/* 행삭제: 행입고삭제(erp발주입고수정), 전체삭제: 입고삭제(erp발주입고수정)*/
	ldt_insp_date = dw_2.object.insp_date[arg_cnt]	
	ls_order_no  = trim(dw_2.object.insp_no[arg_cnt])
	ls_coilsliting_coil_no = dw_2.object.coilsliting_coil_no[arg_cnt]
	
	/*2020-07-22 jowonsuk 자재사용 */
	if ls_ingubun = 'Y' then
		ls_subcon = 'P'
		/*jowonsuk 2020.07.23 추가 */
		UPDATE purdet 
			SET    ingubun          = 'N'													
		 WHERE order_no = :ls_order AND seq_no = :li_seq;			
	end if
	
	/*2019.08.22 jowonsuk 기존 로직에서 자재가공, 외주가공, 외주스리팅/시트  업체의 출고 내역 삭제가 없었음. 시작 */	
	/*2019.12.02 jowonsuk INSP_FLAG = 'Y' 이면 P로 pur.subcon엔 지정을 안했을수도 있음  */	
	if ls_subcon = 'J'  or ls_subcon = 'P' or ls_subcon = 'C' then			

	//		if ls_subcon = 'P' then			
						
					IF ls_subcon = "C" THEN /*외주스리팅/sheet*/
					
						/* 2019-12-04 JOWONSUK 외주가공업체에서 원코일 사용(출고) 삭제로 사용했던 inaudit 변경*/
						L_COUNT = 0
					
						SELECT COUNT(*)
						INTO :L_COUNT
						FROM inaudit
						WHERE inaudit_type = 'IO'
								AND ORDER_NO = :ls_order_no
								AND LOC_NO = :ls_destination
							AND REM = '외주스리팅/sheet';                /* 외주스리팅/sheet(미사용)은 안됨. */
				
				
						IF L_COUNT > 0 THEN
							
							/* 2019-12-02 JOWONSUK 외주가공업체에서 원코일 사용(출고)을 삭제함 */
							INSERT INTO inaudit
							SELECT GETDATE() inaudit_time,    inaudit_date,   inaudit_item,       qa,     loc_no,     inaudit_type,  order_no,    rcpt_qty,   issue_qty*(-1) issue_qty,     cost,       destination,         serial_id,      '외주스리팅/sheet(삭제)' rem,       unit_qty,      cnt,     com_qty,      :gs_userid user_id
							FROM inaudit
							WHERE inaudit_type = 'IO'
									AND ORDER_NO = :ls_order_no
									AND LOC_NO = :ls_destination
							...
							if sqlca.sqlcode < 0 then
								...
								ROLLBACK;				
								MessageBox('오류32','[pb_save][inaudit] IO insert중 DB오류가 발생했습니다.~r~n' + ls_sqlerrtext, exclamation!)
								RETURN false
							end if
							...
							/* 2019-12-04 JOWONSUK 외주가공업체에서 원코일 사용(출고) 삭제로 사용했던 inaudit 변경*/
							UPDATE inaudit
									  SET rem = '외주스리팅/sheet(미사용)'
							WHERE inaudit_type = 'IO'
									AND ORDER_NO = :ls_order_no
									AND LOC_NO = :ls_destination
								AND REM = '외주스리팅/sheet';			/* 외주스리팅/sheet -> 외주스리팅/sheet(미사용) */
...
/*						
						...
						if ls_subcon = 'C' then
						 ...
						 INSERT INTO inaudit ( inaudit_time,    inaudit_date,   inaudit_item,       qa,         order_no,              loc_no,     inaudit_type,         rcpt_qty,       issue_qty,     cost,       destination,         serial_id,      rem,       unit_qty,      cnt,     com_qty,      user_id )  
							                VALUES (      getdate(),  :ldt_insp_date,         :ls_item,  :ls_qa,   :ls_order_no,  :ls_destination,           :ls_type,    			 0, 	 :L_issue_qty, :l_cost,        :ls_newloc,  :ls_serial_id, :ls_rem,   :l_unit_qty,  :l_cnt,  :l_com_qty, :gs_userid );
...
//			end if
	...
	/*2019.08.22 jowonsuk 기존 로직에서 자재가공, 외주가공 업체의 출고 내역 삭제가 없었음. 끝  */	
	
	ls_type = dw_2.object.iotype[arg_cnt]	
	ls_serial_id = dw_2.object.coil_no[arg_cnt]
				
	 INSERT INTO inaudit ( inaudit_time,    inaudit_date,   inaudit_item,       qa,         order_no,        loc_no,    inaudit_type,         rcpt_qty,      issue_qty,     cost,       destination,       serial_id,      rem,       unit_qty,     cnt,     com_qty,      user_id )  
			   	      VALUES (      getdate(), :ldt_insp_date,         :ls_item, :ls_qa,    :ls_order_no,  :ls_newloc,           :ls_type,    :L_rcpt_qty, 				 0, :l_cost,  :ls_destination, :ls_serial_id, :ls_rem,   :l_unit_qty, :l_cnt, :l_com_qty, :gs_userid );
...
/* 2019-08.08 jowonsuk inaudit insert 부분을 datawindows에서 가져와서 inaudit에 insert  끝 */
...
if (ls_subcon = 'N') OR (ls_subcon = 'C') OR (ls_subcon = 'J') OR (ls_subcon = 'W') then
   ...
   choose case ls_subcon
	   case 'C'
			...
			SELECT proc_flag, coilorder_order, coil_no, wid, cnt, loc_no, isnull(rem4, 'N'), isnull(rem5, 'N')
			  INTO :ls_flag, :ls_coilorder, :ls_w_coil, :ld_w_wid, :li_wcnt, :ls_loc, :ls_check_sum, :ls_check
			  FROM coilorderdet
...
//			if ls_flag <> "P" and ls_check= 'Y' then 2020-08-10 jowonsuk 주석처리 
			if ls_flag <> "P" then				
/*			if ls_flag <> "P" then				*/
...
/*                   2020-08-10 jowonsuk 주석처리 */
/*				UPDATE coilorderdet SET proc_flag = 'P', rem4 = 'N', rem5 = 'N' WHERE order_no = :ls_product;								
 */				
				...
				/*2019-09-02 jowonsuk coilsum dw_9 자체가 안돌아감. 이거 로직 수정했음 기존 dataobject가 잘못되어 있음.  */				
				for ll_row = 1 to dw_9.rowcount()
					...
					/*2019-09-02 jowonsuk coilsum 삭제 로직이 없음 로직 추가 */					
					...
					VALUES ( getdate(), :ldt_insp_date, :ls_witem_no, :ls_wqa, :ls_supp, 'IO', :ls_inspno,
							 0, :li_wunit_qty * :li_wcnt*(-1), 0, :ls_supp, :ls_w_coil, 'coilsum_IO', :li_wunit_qty, :li_oino, :li_wunit_qty, :gs_userid);			 
			...
			//스리팅코일 삭제
			if ls_sheet = "S" then
				DELETE FROM sheetmst WHERE sheet_no = :ls_coil AND loc_no = :ls_newloc and cnt = qcnt and qcnt <> 0; // and cnt = qcnt and qcnt <> 0 추가 2021.01.23 jowonsuk 추가 출고가 되면 삭제 안됨. 재고가 그대로 있는것만 삭제 가능.
				...
				DELETE FROM coilmst WHERE coil_no = :ls_coil   AND loc_no = :ls_newloc and cnt = qcnt and qcnt <> 0; // and cnt = qcnt and qcnt <> 0 추가 2021.01.23  jowonsuk 추가 출고가 되면 삭제 안됨. 재고가 그대로 있는것만 삭제 가능.
				if sqlca.sqldbcode <> 0 then
					ROLLBACK;
...
if (ls_subcon = 'W') OR (ls_subcon = 'C') then
	...
	if MidA(ls_item, 1, 1) = 'M' then	/*2021.03.21 jowonsuk 내용 추가*/
			if ls_item4 = '1' then // 1: coil, 2: sheet
				...
				DELETE coilmst WHERE coil_no = :ls_coil AND loc_no = :ls_newloc and cnt = qcnt and qcnt <> 0; // and cnt = qcnt and qcnt <> 0 추가 2021.01.23  jowonsuk 추가 출고가 되면 삭제 안됨. 재고가 그대로 있는것만 삭제 가능.
			...
			elseif ls_item4 = '2' then
				...
				DELETE sheetmst WHERE sheet_no = :ls_coil AND loc_no = :ls_newloc and cnt = qcnt and qcnt <> 0; // and cnt = qcnt and qcnt <> 0 추가 2021.01.23  jowonsuk 추가 출고가 되면 삭제 안됨. 재고가 그대로 있는것만 삭제 가능.
...
public function boolean wf_jajae_out_delete (long arg_cnt);//
string ls_item, ls_supp, ls_qa, ls_order, ls_rem, ls_assy_item, ls_assy_qa, ls_inspno
string ls_destination, ls_temp, ls_suppname, ls_flag, ls_loc, ls_sqlerrtext
string ls_w_coil, ls_item4, ls_oc, ls_old_time, ls_item1, ls_type, ls_tempno
dec{1} ld_w_wid
real   lr_newqty
long   ll_cnt, ll_qcnt, ll_count, l_insp_seq, l_row, l_qa_check, ll_seq
datetime ldt_date

dw_1.accepttext()
dw_2.accepttext()

ldt_date = dw_1.object.insp_date[1]
ls_supp  = trim(dw_1.object.supp_no[1])
ls_inspno = trim(dw_1.object.insp_no[1])
l_insp_seq    = dw_2.object.oi_no[arg_cnt]
ls_qa    = trim(dw_2.object.qa[arg_cnt])
//ls_order = trim(dw_2.object.order_no[arg_cnt])
ls_destination = trim(dw_1.object.supp_no[1])
lr_newqty = dw_2.object.rcpt_qty[arg_cnt]*-1
ls_rem =   '자재사용[삭제]'
...
IF l_row > 0 THEN
		...
		if sqlca.sqlcode = 0 then	
			...
			INSERT INTO inaudit (inaudit_time,inaudit_date,inaudit_item,qa,  loc_no,inaudit_type,order_no,
							...
							VALUES (getdate(),:ldt_date,:ls_assy_item,:ls_assy_qa,  :ls_loc,'IO',:ls_inspno,
										 0,:  lr_newqty,  0,   :ls_supp, null, :l_insp_seq, null, :ls_rem, :gs_userid);   
...
//							  VALUES ( :ls_tempno,      :ldt_date,     :ls_loc,  :ls_assy_item, :ls_assy_qa,    :lr_newqty,                '05',    :gs_userid,          :ldt_date,        :ls_rem);
//			
//			if sqlca.sqlcode < 0 then
//				ls_sqlerrtext = string(sqlca.sqlerrtext)
//				ROLLBACK;
...
ELSE
		...
		if isnull(ls_assy_item) or ls_assy_item = '' then	
			return true
		else
...
this.Control[iCurrent+39]=this.cb_10
end on
...
decimal L_rcpt_qty, L_issue_qty, l_cost, l_unit_qty, l_cnt, l_com_qty
string    ls_insp_flag, ls_jaje_out, ls_subcon
...
ls_insp_flag    = dw_1.object.insp_flag[1]
ls_jaje_out =  dw_1.object.inv_no[1]
/*ldt_user_date  = date(dw_1.GetItemDateTime( 1, 'user_date', Primary!, True)) 	*/			/*2018-05-15 jowonsuk user_date -> insp_date*/

/*2019-08-08 jowonsuk wf_delete에 inaudit 등록 로직 반영*/
is_rem = '입고삭제(erp발주입고수정)'
for ll_cnt = 1 to dw_2.rowcount()
	...
	/*외주가공 2019-11-01 jowonsuk 외주가공 출고된 자재 inaudit 삭제 로직 */
	/*외주가공 2020-04-06 jowonsuk 외주가공, 상품 출고된 자재 inaudit 삭제 로직 */	
	
	ls_subcon =  dw_2.object.ingubun[ll_cnt]
	
	if ls_subcon = 'Y' then
			if wf_jajae_out_delete(ll_cnt) = false then
				ROLLBACK;
				MessageBox('오류','wf_jajae_out_delete 수행중 오류가 발생하였습니다.'&
...
long    ll_row, ll_cnt
decimal 	li_amt, li_price, li_comqty
...
for li_int = 1 to dw_2.rowcount()
	/*2022-06-02 jowonsuk 추가로직.시작 */	
	li_comqty = dw_2.object.comqty[li_int]
	li_price = dw_2.object.price[li_int]	
	if dw_2.object.supp_round[li_int]  = 'Y' then
		select ROUND(:li_comqty * :li_price, 0, 0) into :li_amt from dual;		
	else
		select ROUND(:li_comqty * :li_price, 0, 1) into :li_amt from dual;		
	end if 
	
   dw_2.object.camt[li_int] = li_amt
   /*2022-06-02 jowonsuk 추가로직.끝 */			
   dw_1.object.insp_cash[1] = dw_1.object.insp_cash[1] + dw_2.object.rlse_amount[li_int]
   dw_1.object.insp_rate[1] = dw_1.object.insp_rate[1] + dw_2.object.rlse_rate[li_int]
...
string ls_ITEM_NO, ls_QA, ls_locno, ls_destination, ls_serial_id, LS_TYPE, ls_rem, ls_sqlerrtext
decimal L_rcpt_qty, L_issue_qty, l_cost, l_unit_qty, l_cnt, l_com_qty
string ls_insp_flag, ls_jaje_out, ls_ingubun
...
ls_insp_flag    = dw_1.object.insp_flag[1]
ls_jaje_out =  dw_1.object.inv_no[1]
ls_ingubun = dw_2.object.ingubun[ll_row]

/* 2019-08.08 jowonsuk inaudit inset 부분을 datawindows에서 가져와서 inaudit에 insert 한다. */
is_rem = '행입고삭제(erp발주입고수정)'
...
/* 2020-04-06 jowonsuk 누락 부문 */
if ls_ingubun = 'Y' then
		if wf_jajae_out_delete(ll_row) = false then
			ROLLBACK;
			MessageBox('오류','wf_jajae_out_delete 수행중 오류가 발생하였습니다.'&
						+ "~r~n" + string(sqlca.sqlerrtext) ,exclamation!)
			RETURN							
